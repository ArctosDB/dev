...
concat(source,'::',geog_string) ,
search_terms,
'https://github.com/ArctosDB/arctos/issues/6008'
from
external_gis_data
where
geog_string ilike 
...
'England'  -- <-------------------- there it is
 ...
 select country from geog_auth_rec group by country order by country;
...
<cfif action is "final_load">
    <cfoutput>
        <cftransaction>
            <cfquery name="d" datasource="uam_god">
                select * from temp_pre_geog where load_status is null limit 10
            </cfquery>
            <cfloop query="d">
                ...
                <cfquery name="newGeog" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
                    INSERT INTO geog_auth_rec (
                        ...
                        <cfqueryparam value = "#gr#" CFSQLType="CF_SQL_VARCHAR">,
                        (
                            select 
                                the_shape
                            from external_gis_data where 
