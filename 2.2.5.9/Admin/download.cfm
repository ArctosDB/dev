...
<cfif action is "nothing">
	...
	<cfquery name="ctusername" datasource="cf_dbuser">
		...
		from 
		cf_users
		inner join cf_download on cf_users.user_id = cf_download.user_id
...
<cfif action is "table">
	...
	<cfquery name="dl" datasource="cf_dbuser">
		...
		<cfif isdefined("username") and len(username) gt 0>
			and cf_users.username=<cfqueryparam value="#username#" CFSQLType="cf_sql_varchar">
		</cfif>
		<cfif isdefined("bdate") and len(bdate) gt 0>
			AND cf_download.DOWNLOAD_DATE between '#dateformat(bdate,"yyyy-mm-dd")#' and'#dateformat(edate,"yyyy-mm-dd")#'
		</cfif>
	</cfquery>
