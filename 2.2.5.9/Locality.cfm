...
<cfif action is "massEditCollEvent">
	<cfquery name="locality" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
		select
		SPEC_LOCALITY,
		DEC_LAT,
		DEC_LONG,
...
-------------->
	...
	<cfif action is "reallyMassEditCollEvent">
		<cfquery name="reallyMassEditCollEvent" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
...
<cfif action is "findCollEventIdForSpecDetail">
	...
	<cfquery name="ceid" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
...
<cfoutput>
	...
	<form name="getHG" method="post" action="Locality.cfm">
...
<cfheader name="Location" value="/place.cfm?sch=locality">
...
<cfheader statuscode="301" statustext="Moved">
<cfheader name="Location" value="/place.cfm?sch=collecting_event">
...
<cfif action is "editGeog">
	...
	<script>
		...
		function openOutsidePoints(){
	    	...
	    	if (opa.length>0){
				...
				$("#fo_geog_auth_rec_id").val($("#geog_auth_rec_id").val());
				$("#fo_coordslist").val(opastr);
				$('#findoutside').submit();
	    		//window.open(theURL);
			} else {
	...
	<div style="display:none">
		<form id="findoutside" method="post" target="_blank" action="/SpecimenResults.cfm">
	...
	<cfoutput>
		<cfquery name="geogDetails" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
		 select 
		 	...
		 	higher_geog,
		 	geog_remark,
		 	ST_AsText(spatial_footprint) as wkt
		 from 
		 	geog_auth_rec 
		 where 
		...
		<cfquery name="localities" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			select count(*) c from locality where geog_auth_rec_id=#geog_auth_rec_id#
		</cfquery>
		<cfquery name="collecting_events" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
	...
	<cfoutput query="geogDetails">
					...
					<td rowspan="20" valign="top">
						...
						<div id="wktinstr">
							...
							<ol>
								...
								<li><span class="likeLink" id="mediaUpClickThis">Create Media</span>; copy the media_id of the media record</li>
								<li>Enter the Media ID here</li>
							</ol>
						</div>

	                	---->
						<input type="hidden" id="wkt_poly_data" value="#wkt#">
		...
		<form id="records_form" method="get" action="/SpecimenResults.cfm" target="_blank">
		...
		<cfif session.roles contains "manage_geography" and session.roles contains "global_admin">
			<div class="importantNotification">
				...
				<form name="atts" method="post" enctype="multipart/form-data" action="Locality.cfm">
					<input type="hidden" name="geog_auth_rec_id" value="#geog_auth_rec_id#">
					<input type="hidden" name="Action" value="getFileDBF">
					<label for="FiletoUpload">Upload DBF</label>
...
<script>
	...
	function sbmtNewPublication(){
		var theSURL="/Locality.cfm?action=newEventPub&new_publication_id=" + $("#new_publication_id").val() + '&collecting_event_id=' + $("#collecting_event_id").val();
		...
		<table width="100%"><tr><td valign="top">
			<h4>Edit this Collecting Event:</h4>
				...
				<div style="font-size:x-small;">
					NOTE: Clone operations do not bring name or verbatim coordinates
				</div>
		...
		<td valign="top"><!---------- right side ------------>
			...
			<cfloop query="evt_pub">
				<li>
					...
					<ul>
						<li>
							<a href="Locality.cfm?action=removeEvtPub&collecting_event_publication_id=#collecting_event_publication_id#&collecting_event_id=#locDet.collecting_event_id#">[ remove ]</a>
						</li>
						<li>
...
<cfif action is "removeEvtPub">
	...
	<cflocation url="Locality.cfm?Action=editCollEvnt&collecting_event_id=#collecting_event_id#" addtoken="false">
</cfif>
<!---------------------------------------------------------------------------------------------------->
...
<cfif action is "cloneEventAndLocality">
	<cfoutput>
		...
		<!----
			...
			See https://github.com/ArctosDB/arctos/issues/3718 - makes no sense to bring "verbatim" across in cloning process
			Ditto name
			COLLECTING_EVENT_NAME,
					LAT_DEG,
					DEC_LAT_MIN,
...
<cfoutput>
	...
	<!----
		see https://github.com/ArctosDB/arctos/issues/3718, don't bring verbatim
		and name
	---->
...
<cfif action is "saveCollEventEdit">
	<cfoutput>
		<cftransaction>
			...
			<!----- old, allows edit to verbatim, see https://github.com/ArctosDB/arctos/issues/2930 ----
			<cfquery name="upColl" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
				UPDATE
					collecting_event
				SET
					...
					COLL_EVENT_REMARKS = <cfqueryparam value = "#COLL_EVENT_REMARKS#" CFSQLType="CF_SQL_VARCHAR" null="#Not Len(Trim(COLL_EVENT_REMARKS))#">,
					collecting_event_name = <cfqueryparam value = "#collecting_event_name#" CFSQLType="CF_SQL_VARCHAR" null="#Not Len(Trim(collecting_event_name))#">,
					orig_lat_long_units = <cfqueryparam value = "#orig_lat_long_units#" CFSQLType="CF_SQL_VARCHAR" null="#Not Len(Trim(orig_lat_long_units))#">,
					<cfif orig_lat_long_units is "degrees dec. minutes">
						LAT_DEG = <cfqueryparam value = "#dmLAT_DEG#" CFSQLType="CF_SQL_NUMERIC" null="#Not Len(Trim(dmLAT_DEG))#">,
						LONG_DEG = <cfqueryparam value = "#dmLONG_DEG#" CFSQLType="CF_SQL_NUMERIC" null="#Not Len(Trim(dmLONG_DEG))#">,
						DEC_LAT_MIN = <cfqueryparam value = "#DEC_LAT_MIN#" CFSQLType="CF_SQL_NUMERIC" null="#Not Len(Trim(DEC_LAT_MIN))#">,
						dec_long_min = <cfqueryparam value = "#dec_long_min#" CFSQLType="CF_SQL_NUMERIC" null="#Not Len(Trim(dec_long_min))#">,
						LAT_DIR = <cfqueryparam value = "#dmLAT_DIR#" CFSQLType="CF_SQL_VARCHAR" null="#Not Len(Trim(dmLAT_DIR))#">,
						LONG_DIR = <cfqueryparam value = "#dmLONG_DIR#" CFSQLType="CF_SQL_VARCHAR" null="#Not Len(Trim(dmLONG_DIR))#">,
						LAT_MIN=NULL,
						LAT_SEC=NULL,
						LONG_MIN=NULL,
						LONG_SEC=NULL,
					...
					<cfelseif orig_lat_long_units is "UTM">
						...
						DEC_LAT_MIN=NULL,
						dec_long_min=NULL,
						UTM_EW = <cfqueryparam value = "#UTM_EW#" CFSQLType="CF_SQL_NUMERIC" null="#Not Len(Trim(UTM_EW))#">,
						UTM_NS = <cfqueryparam value = "#UTM_NS#" CFSQLType="CF_SQL_NUMERIC" null="#Not Len(Trim(UTM_NS))#">,
						UTM_ZONE = <cfqueryparam value = "#UTM_ZONE#" CFSQLType="CF_SQL_VARCHAR" null="#Not Len(Trim(UTM_ZONE))#">,
					...
					<cfelseif orig_lat_long_units is "deg. min. sec.">
						LAT_DEG = <cfqueryparam value = "#LAT_DEG#" CFSQLType="CF_SQL_NUMERIC" null="#Not Len(Trim(LAT_DEG))#">,
						LAT_MIN = <cfqueryparam value = "#LAT_MIN#" CFSQLType="CF_SQL_NUMERIC" null="#Not Len(Trim(LAT_MIN))#">,
						LAT_SEC = <cfqueryparam value = "#LAT_SEC#" CFSQLType="CF_SQL_NUMERIC" null="#Not Len(Trim(LAT_SEC))#">,
						LONG_DEG = <cfqueryparam value = "#LONG_DEG#" CFSQLType="CF_SQL_NUMERIC" null="#Not Len(Trim(LONG_DEG))#">,
						LONG_MIN = <cfqueryparam value = "#LONG_MIN#" CFSQLType="CF_SQL_NUMERIC" null="#Not Len(Trim(LONG_MIN))#">,
						LONG_SEC = <cfqueryparam value = "#LONG_SEC#" CFSQLType="CF_SQL_NUMERIC" null="#Not Len(Trim(LONG_SEC))#">,
						dec_lat=NULL,
						DEC_LONG=NULL,
						DEC_LAT_MIN=NULL,
						dec_long_min=NULL,
			...
			<cfif iscrap.recordcount gt 0>
				...
				<form name="editHG" id="editHG" method="post" action="Locality.cfm">
			        ...
			        <cfloop list="#form.FieldNames#" index="f">
						...
						<input type="hidden" name="#f#" id="#f#" value="#stripQuotes(thisVal)#" size="60">
					</cfloop>
					<p>
		...
		<cftransaction>
			<cfquery name="edGe" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
				...
				SET
					...
					drainage = <cfqueryparam value="#drainage#" CFSQLType="CF_SQL_VARCHAR" null="#Not Len(Trim(drainage))#">,
					...
					<!----,
					wkt_media_id=<cfqueryparam value="#wkt_media_id#" CFSQLType="cf_sql_int" null="#Not Len(Trim(wkt_media_id))#">
					---->
				where
			...
			<cfloop from ="1" to="#numGeogSrchTerms#" index="i">
				...
				<cfif len(thisTerm) gt 0>
					<cfquery name="ist1" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
						insert into geog_search_term (geog_auth_rec_id,search_term) values (#val(geog_auth_rec_id)#,trim('#escapeQuotes(thisTerm)#'))
					</cfquery>
				</cfif>
			</cfloop>
			<cfloop list="#form.FieldNames#" index="f">
...
<table border>
	...
	<cfloop query="localityResults">
		...
		<tr>
			...
			<td>
				 <div class="smaller">
				 	...
				 	<cfif len(LOCALITY_REMARKS) gt 0><br>Remark: #LOCALITY_REMARKS#</cfif>
				 	 <cfif len(localityAtts) gt 0><br>#localityAtts#</cfif>
			...
			<td>#verbatim_date#</td>
			<td nowrap>
				<cfquery name="spc" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
				...
				<cfquery name="mc" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
...
<cfif action is "massMoveCollEvent">
	<cfoutput>
		...
		<cfquery name="cd" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
...
<cfif action is "mmCollEvnt2">
	<cfoutput>
		...
		<cfloop list="#collecting_event_id#" index="ceid">
			<cfquery name="upCollLoc" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
...
<tr>
	...
	<td>#geog_remark#</td>
	<cfquery name="searchterm" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
