...
<cfif action is "nothing">
	...
	<cfoutput>
		...
		<form name="details" method="post" action="editBiolIndiv.cfm">
			...
			<label for="coll_object_remarks">Collection Object Remarks</label>
				...
				<table id="mammatttab" class="newRec">
					<tr>
						...
						<td>
							...
							<input type="text" name="mammgrid_determiner" id="mammgrid_determiner" class="reqdClr"
...
<cfif action is "save">
	<cfoutput>
		<cftransaction>
			...
			<cfquery name="upCollObj" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
				UPDATE coll_object SET
					last_edited_person_id = #session.myAgentId#
					,last_edit_date = current_date
					,flags=<cfqueryparam value = "#flags#" CFSQLType="CF_SQL_VARCHAR" null="#Not Len(Trim(flags))#">
			...
			<cfquery name="isCORem" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
				select collection_object_id from coll_object_remark where
				collection_object_id = #collection_object_id#
			</cfquery>
			<cfif len(isCORem.collection_object_id) gt 0>
				<cfquery name="upCoRem" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			...
			<cfelse>
				<cfif len(coll_object_remarks) gt 0 or len(associated_species) gt 0>
					<cfquery name="newBIRem" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
						INSERT INTO coll_object_remark (
							...
							<cfqueryparam value="#associated_species#" CFSQLType="cf_sql_varchar">
						)
					</cfquery>
				</cfif>
			</cfif>
