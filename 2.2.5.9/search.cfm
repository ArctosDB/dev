...
<script>
	jQuery(document).ready(function() {
		...
		// not sure this is useful but maybe takes some load off browsers??
		//document.querySelectorAll('.noshow').forEach(e => e.remove());
	...
	function customize_section(cid,cv){
		var theCategory=cid.replace('_widget','');
		$("#" + cid).val('');
		//console.log(theCategory);
		//console.log(cv);
		customizeCatRecordSearch(theCategory,cv);
	...
	function customizeCatRecordSearch(cat='',opn=''){
		var guts = "/form/customizeCatRecSearch.cfm?cat=" + cat + "&opn=" + opn;
		$("<iframe src='" + guts + "' id='dialog' class='popupDialog' style='width:1200px;height:600px;'></iframe>").dialog({
			autoOpen: true,
			closeOnEscape: true,
			height: 'auto',
	...
	function catRecordSearchProfile(){
		var sp=$("#sp").val();
		openOverlay('/form/catRecordSearchProfile.cfm?pn='+encodeURIComponent(sp),'Customize search and results; create and use search profiles.');
	}
	var map;
	var bounds;
	var all_overlays = [];
	...
	function customizeCatRecordResults(){
		var guts = "/form/customizeCatRecResults.cfm";
		$("<iframe src='" + guts + "' id='dialog' class='popupDialog' style='width:1200px;height:600px;'></iframe>").dialog({
			autoOpen: true,
			closeOnEscape: true,
			height: 'auto',
	...
	function initExpandJSON(){
		...
		var b='<input type="button" class="lnkBtn" id="procCpJsonClk" value="Collapse JSON" onclick="collapseJSON();">';
		$("#jsonCtlBtn").html(b);
	}
	function expandJSON(){
		...
		var b='<input type="button" class="lnkBtn" id="procJsonClk" value="Collapse JSON" onclick="collapseJSON();">';
		$("#jsonCtlBtn").html(b);
	}
	...
	function collapseJSON(){
		...
		var b='<input type="button" class="lnkBtn" id="procJsonClk" value="Expand JSON" onclick="expandJSON();">';
		$("#jsonCtlBtn").html(b);
	}
...
<cfquery name="IslGrp" datasource="cf_codetables" cachedwithin="#createtimespan(0,0,60,0)#">
...
<cfquery name="cttaxonomy_source" datasource="cf_codetables" cachedwithin="#createtimespan(0,0,60,0)#">
	SELECT DISTINCT(source) FROM cttaxonomy_source ORDER BY source
</cfquery>
...
<cfquery name="getCount" datasource="user_login" username="#lcase(session.dbuser)#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#" cachedwithin="#createtimespan(0,0,60,0)#">
	select count(cataloged_item.collection_object_id) as cnt from cataloged_item
	inner join filtered_flat on  cataloged_item.collection_object_id=filtered_flat.collection_object_id
</cfquery>
...
<cfoutput>
	<div class="cat_rec_sch_frm_ctl_ctr">
		...
		<div class="cat_rec_sch_frm_ctl_itm">
			<input form="cat_rec_sch_frm"  type="button" class="cstmBtn" value="Customize Search" onclick="customizeCatRecordSearch();">
		</div>
		<div class="cat_rec_sch_frm_ctl_itm">
			<input form="cat_rec_sch_frm" type="button" class="cstmBtn" value="Profiles" onclick="catRecordSearchProfile();">
			<cfif len(sp) gt 0>
				(Using #sp#)
			</cfif>
		</div>
	...
	<form name="cat_rec_sch_frm" id="cat_rec_sch_frm">
		...
		<div id="cat_rec_srch_frm_wrap">
			<div id="identifier" class="srch_section">
				<div class="section_label">
					...
					<div class="section_label_control">
						<select class="control_widget" id="identifier_widget" onchange="customize_section(this.id,this.value)">
							<option value="">[ customize ]</option>
							<option value='customize'>Customize (choose individual options)</option>
							<option value='forceall'>Show All Options</option>
							<option value='forcebasic'>Reset to Basic</option>
						</select>
						<!-----
						<input type="button" class="cstmBtn" value="Customize"  onclick="customizeCatRecordSearch('identifier');">
						<input type="button" class="cstmBtn" value="All"  onclick="customizeCatRecordSearch('identifier','forceall');">
						<input type="button" class="cstmBtn" value="Reset"  onclick="customizeCatRecordSearch('identifier','forcebasic');">
						----->
					</div>
				</div>
				...
				<div class="section_content">
					<div class="pglayout">
						...
						<div class="schitem noshow" id="si_related_id_references">
							<label for="related_id_references">
								<span class="helpLink" id="_biol_indiv_relationship">Relationship</span>							
			...
			<div id="identification" class="srch_section">
				<div class="section_label">
					...
					<div class="section_label_control">
						<select class="control_widget" id="identification_widget" onchange="customize_section(this.id,this.value)">
							<option value="">[ customize ]</option>
							<option value='customize'>Customize (choose individual options)</option>
							<option value='forceall'>Show All Options</option>
							<option value='forcebasic'>Reset to Basic</option>
						...
						<!----
						<input type="button" class="cstmBtn" value="Add/Remove"  onclick="customizeCatRecordSearch('identification');">
						----->
					</div>
				</div>
				<div class="section_content">
					<div class="pglayout">
						...
						<cfparam name="identification_score" default="">
						<div class="schitem noshow" id="si_identification_score">
							<label class="helpLink" id="_identification_score" for="identification_score" title="1, >0, <1">ID Score</label>
							<input type="text" name="identification_score" id="identification_score" value="#identification_score#" placeholder="ID Score">
						</div>
						...
						<cfparam name="nature_of_id" default="">
						<div class="schitem noshow" id="si_nature_of_id">
							<label for="nature_of_id">
								<span class="helpLink" id="_nature_of_id">Nature of ID</span>
								<span class="likeLink" onclick="getCtDoc('ctnature_of_id', cat_rec_sch_frm.nature_of_id.value);">Define</span>
							</label>
							<cfset x=nature_of_id>
							<select name="nature_of_id" id="nature_of_id" size="1">
								<option value="">[ Nature of ID ]</option>
								<cfloop query="ctnature_of_id">
									<option <cfif ctnature_of_id.nature_of_id is x> selected="selected" </cfif> value="#ctnature_of_id.nature_of_id#">#ctnature_of_id.nature_of_id#</option>
								</cfloop>
							</select>
						</div>
						<cfparam name="identification_confidence" default="">
						<div class="schitem noshow" id="si_identification_confidence">
							<label for="identification_confidence">
								<span class="helpLink" id="_identification_confidence">ID Confidence</span>
								<span class="likeLink" onclick="getCtDoc('ctidentification_confidence', cat_rec_sch_frm.identification_confidence.value);">Define</span>
							</label>
							<cfset x=identification_confidence>
							<select name="identification_confidence" id="identification_confidence" size="1">
								<option value="">[ ID Confidence ]</option>
								<cfloop query="ctidentification_confidence">
									<option <cfif ctidentification_confidence.identification_confidence is x> selected="selected" </cfif> value="#ctidentification_confidence.identification_confidence#">#ctidentification_confidence.identification_confidence#</option>
								</cfloop>
							</select>
						</div>
						...
						<div class="schitem twocols tworows">
							<table id="si_taxonomysearchtable" class="wideCollapseTable noshow">
								<thead>
									<tr>
										<th scope="col">Taxon Term</th>
										<th scope="col">Operator</th>
										<th scope="col">Type or Rank</th>
								...
								<tbody>
									...
									<cfloop from="1" to="#numberOfTaxonTermSearch#" index="i">
										...
										<cfparam name="tax_opr_#i#" default="">
										<cfparam name="tax_rnk_#i#" default="">
										<cfparam name="tax_src_#i#" default="">
										<cfset thisTT=evaluate("tax_trm_" & i)>
										<cfset thisTO=evaluate("tax_opr_" & i)>
										<cfset thisTR=evaluate("tax_rnk_" & i)>
										<cfset thisTS=evaluate("tax_src_" & i)>
										<tr>
											<td data-label="Term">
												<input type="text" name="tax_trm_#i#" id="tax_trm_#i#" size="30" placeholder="term" value="#thisTT#">
											</td>
											<td data-label="Operator">
												<select name="tax_opr_#i#" id="tax_opr_#i#" size="1" style="max-width:4em;">
													<option <cfif thisTO is "is"> selected="selected" </cfif> value="is">is</option>
													<option <cfif thisTO is "starts"> selected="selected" </cfif> value="starts">starts with</option>
													<option <cfif thisTO is "contains"> selected="selected" </cfif> value="contains">contains</option>
												</select>    	
											</td>
											<td data-label="Rank">
											...
											<td data-label="Source">
												<select name="tax_src_#i#" id="tax_src_#i#" class="constrained_select">
													<option value="">[ Source ]</option>
													<cfloop query="cttaxonomy_source">
														<option <cfif thisTS is cttaxonomy_source.source> selected="selected" </cfif>  value="#cttaxonomy_source.source#">#cttaxonomy_source.source#</option>
													</cfloop>
												</select>
											</td>
										</tr>
			...
			<div id="catalog_record" class="srch_section">
				<div class="section_label">
					...
					<div class="section_label_control">
						<!----
						<input type="button" class="cstmBtn" value="Add/Remove"  onclick="customizeCatRecordSearch('catalog_record');">
						---->
						<select class="control_widget" id="record_widget" onchange="customize_section(this.id,this.value)">
							<option value="">[ customize ]</option>
							<option value='customize'>Customize (choose individual options)</option>
							<option value='forceall'>Show All Options</option>
							<option value='forcebasic'>Reset to Basic</option>
				...
				<div class="section_content">
					<table id="si_recordattributesearchtable" class="wideCollapseTable noshow">
						<thead>
							<tr>
								...
								<th scope="col"><span class="helpLink" id="_attribute_date">Attribute Date (earlies)</span></th>
					...
					<div class="pglayout">
						...
						<cfparam name="last_edit_by" default="">
						<div class="schitem noshow" id="si_last_edit_by">
							<label class="helpLink" id="_last_edit_by" for="last_edit_by">Last Edited By</label>
							<input type="text" name="last_edit_by" id="last_edit_by" value="#last_edit_by#" placeholder="Last Edited By">
						</div>
						<cfparam name="beg_last_edit_date" default="">
						<cfparam name="end_last_edit_date" default="">
						<div class="schitem noshow" id="si_last_edit_date">
							<div class="dontwrap">
								<label class="helpLink" id="_last_edit_date" for="beg_last_edit_date">Edited Date</label>
								<input type="datetime" name="beg_last_edit_date" id="beg_last_edit_date" size="12" value="#beg_last_edit_date#" placeholder="earliest">
								<input type="datetime" name="end_last_edit_date" id="end_last_edit_date" size="12" value="#end_last_edit_date#" placeholder="latest">
							</div>
						</div>
						...
						<cfparam name="coll_obj_flags" default="">
						<div class="schitem noshow" id="si_coll_obj_flags">
							<label for="coll_obj_flags">
								<spam class="helpLink" id="_coll_obj_flags" >Missing (flags)</spam>
								<span class="likeLink" onclick="getCtDoc('ctFlags', cat_rec_sch_frm.coll_obj_flags.value);">Define</span>
							</label>
							<cfset x=coll_obj_flags>
							<select name="coll_obj_flags" id="coll_obj_flags" size="1">
								<option value=""></option>
								<cfloop query="ctFlags">
									<option <cfif ctFlags.flags is x> selected="selected" </cfif> value="#flags#">#flags#</option>
								</cfloop>
							</select>
						</div>
						...
						<cfparam name="edited_by_id" default="">
						<div class="schitem noshow" id="si_edited_by_id">
							<label class="helpLink" id="_edited_by_id" for="edited_by_id">edited_by_id</label>
							<input type="text" name="edited_by_id" id="edited_by_id" value="#edited_by_id#" placeholder="edited_by_id">
						</div>
			...
			<div id="parts" class="srch_section">
				<div class="section_label">
					...
					<div class="section_label_control">
						<!----
						<input type="button" class="cstmBtn" value="Add/Remove"  onclick="customizeCatRecordSearch('parts');">
						---->
						<select class="control_widget" id="parts_widget" onchange="customize_section(this.id,this.value)">
							<option value="">[ customize ]</option>
							<option value='customize'>Customize (choose individual options)</option>
							<option value='forceall'>Show All Options</option>
							<option value='forcebasic'>Reset to Basic</option>
				...
				<div class="section_content">
					<div class="pglayout">
						...
						<cfparam name="partname" default="">
						<div class="schitem noshow" id="si_partname">
							<label for="partname">
								<span class="helpLink" id="_partname">Part Name</span>
								<span class="helpLink" onclick="getCtDoc('ctspecimen_part_name',cat_rec_sch_frm.partname.value);">Define</span>
								<span class="helpLink" onclick="var e=document.getElementById('partname');e.value='='+e.value;" title="Add = for exact match">[ = ]</span>
							</label>
						...
						<div class="schitem noshow" id="si_part_container_type">
							<label for="part_container_type">
								<span class="helpLink" id="_part_container_type">Part-holding container type</span>
						...
						<cfparam name="part_disposition" default="">
						<div class="schitem noshow" id="si_part_disposition">
							<label for="part_disposition">
								<span class="helpLink" id="_part_disposition">Part Disposition</span>
								<span class="likeLink" onclick="getCtDoc('ctCollObjDisp', cat_rec_sch_frm.part_disposition.value);">Define</span>
							</label>
							<cfset x=part_disposition>
							<select name="part_disposition" id="part_disposition" size="1">
								<option value="">[ Part Disposition ]</option>
								<cfloop query="ctCollObjDisp">
									<option <cfif ctCollObjDisp.coll_obj_disposition is x> selected="selected" </cfif> value="#ctCollObjDisp.coll_obj_disposition#">#ctCollObjDisp.coll_obj_disposition#</option>
								</cfloop>
							</select>
						</div>
						<cfparam name="part_condition" default="">
						<div class="schitem noshow" id="si_part_condition">
							<label class="helpLink" id="_part_condition" for="part_condition">Part Condition</label>
							<input type="text" name="part_condition" id="part_condition" value="#part_condition#" placeholder="Part Condition">
						</div>
					</div>
					...
					<table id="si_partattributesearchtable" class="wideCollapseTable noshow">
						<thead>
							<tr>
								...
								<th scope="col"><span class="helpLink" id="_part_attribute_value">Part Attribute Value</span></th>
								<th scope="col"><span class="helpLink" id="_part_attribute_units">Part Attribute Units</span></th>
								<th scope="col"><span class="helpLink" id="_part_attribute_determiner">Part Attribute Determiner</span></th>
								<th scope="col"><span class="helpLink" id="_part_attribute_method">Part Attribute Method</span></th>
								<th scope="col"><span class="helpLink" id="_part_attribute_remark">Part Attribute Remark</span></th>
								<th scope="col"><span class="helpLink" id="_part_attribute_date">Part Attribute Date (earliest)</span></th>
								<th scope="col"><span class="helpLink" id="_part_attribute_date">Part Attribute Date (latest)</span></th>
							</tr>
						</thead>
						<tbody>
							<tr>
								...
								<td data-label="Part Attr. Value">
									<input type="text" name="part_attribute_value" id="part_attribute_value" placeholder="value; prefix with = for exact" value="#part_attribute_value#">
								</td>
								<td data-label="Part Attr. Units">
									<input type="text" name="part_attribute_units"  id="part_attribute_units" placeholder="units" value="#part_attribute_units#">     	
								</td>
								<td data-label="Part Attr. Determiner">
									<input type="text" name="part_attribute_determiner"  id="part_attribute_determiner" placeholder="determiner" value="#part_attribute_determiner#">
								</td>
								<td data-label="Part Attr. Method">
									<input type="text" name="part_attribute_method"  id="part_attribute_method" placeholder="method" value="#part_attribute_method#">
								</td>
								<td data-label="Part Attr. Remark">
									<input type="text" name="part_attribute_remark"  id="part_attribute_remark" placeholder="remark" value="#part_attribute_remark#">
								</td>
								<td data-label="Part Attr. Date">
									<input type="text" name="part_attribute_date_min"  id="part_attribute_date_min" placeholder="min" value="#part_attribute_date_min#">
								</td>
								<td data-label="Part Attr. Date">
									<input type="text" name="part_attribute_date_max"  id="part_attribute_date_max" placeholder="max" value="#part_attribute_date_max#">
								</td>
							</tr>
						</tbody>
					</table>
			...
			<div id="event" class="srch_section">
				<div class="section_label">
					...
					<div class="section_label_control">
						<!-----
						<input type="button" class="cstmBtn" value="Add/Remove"  onclick="customizeCatRecordSearch('event');">
						---->
						<select class="control_widget" id="event_widget" onchange="customize_section(this.id,this.value)">
							<option value="">[ customize ]</option>
							<option value='customize'>Customize (choose individual options)</option>
							<option value='forceall'>Show All Options</option>
							<option value='forcebasic'>Reset to Basic</option>
				...
				<div class="section_content">
					<div class="pglayout">
						...
						<div class="schitem noshow" id="si_began_date">
							...
							<div class="dontwrap">
								<select name="begdateoper" id="begdateoper" size="1" class="constrained_select">
						...
						<div class="schitem noshow" id="si_ended_date">
							...
							<div class="dontwrap">
								<select name="enddateoper" id="enddateoper" size="1"  class="constrained_select">
			...
			<div id="locality" class="srch_section">
				<div class="section_label">
					...
					<div class="section_label_control">
						<!----
						<input type="button" class="cstmBtn" value="Add/Remove"  onclick="customizeCatRecordSearch('locality');">
						---->
						<select class="control_widget" id="place_widget" onchange="customize_section(this.id,this.value)">
							<option value="">[ customize ]</option>
							<option value='customize'>Customize (choose individual options)</option>
							<option value='forceall'>Show All Options</option>
							<option value='forcebasic'>Reset to Basic</option>
				...
				<div class="section_content">
					<div class="pglayout">
						...
						<div class="schitem noshow dontwrap" id="si_place_term">
							<label class="helpLink" id="_coll_obj_flags" for="coll_obj_flags">
								Place Terms
							</label>
			...
			<div id="spatial" class="srch_section">
				<div class="section_label">
					...
					<div class="section_label_control">
						<!----
						<input type="button" class="cstmBtn" value="Add/Remove"  onclick="customizeCatRecordSearch('spatial');">
						---->
						<select class="control_widget" id="map_widget" onchange="customize_section(this.id,this.value)">
							<option value="">[ customize ]</option>
							<option value='customize'>Customize (choose individual options)</option>
							<option value='forceall'>Show All Options</option>
							<option value='forcebasic'>Reset to Basic</option>
				...
				<div class="section_content">	
					<div class="noshow" id="si_map">
						...
						<input type="hidden" id="poly_coords" name="poly_coords">
					</div>
					<div class="pglayout">
						...
						<div class="schitem noshow dontwrap" id="si_geog_srch_type">
							...
							<select name="geog_srch_type" id="geog_srch_type">
								<option <cfif geog_srch_type is 'contains'> selected="selected" </cfif> value="contains">Find records contained by the polygon</option>
		...
		<div class="cat_rec_sch_frm_ctl_ctr">
			...
			<div class="cat_rec_sch_frm_ctl_itm">
				<input type="button" class="cstmBtn" value="Customize Search" onclick="customizeCatRecordSearch();">
			</div>
		</div>
	</form>
	...
	<cfset results_flds="">
	<cfset jsoncols='partdetail,json_locality,attributedetail,identifiers'>
	<cfset hasjson=false>
	...
	<!--- if remove_row is on in results, force it into search as well ---->
		...
		$(document).ready(function() {
			...
			$('##crsdtable').DataTable({
			    ...
			    "drawCallback": function (settings) { 
			       ...
			       // } 
			        if (response.hasOwnProperty('tbl')){
			        	$("##tbl").val(response.tbl);
			        } else {
			        	$("##tbl").val('');
			        }
			        ...
			        if (response.hasOwnProperty('row_limit')){
			        	$("##dv_record_limit").html('record limit: ' + response.row_limit);
			        } else {
			        ...
			        if (response.hasOwnProperty('recordsTotal') && response.recordsTotal > 0){
			        	$('html, body').animate({
						    scrollTop: $('##bcrbst').offset().top
						}, 1000);
			        }			        
			    },
				columns: [	
					<cfloop list="#results_flds#" delimiters="," index="i">
						...
						<cfelseif thisColumnName is 'remove_row'>
								...
								"title":"#trim(listgetat(i,2,'|'))#",
								"render": function ( data, type, row, meta ) {
									var result='<input id="rrbtn_' + data + '" type="button" onclick="removeRow(' + "'" + data + "'" + ');" value="remove">';
									return result;
								}
							}
							/*

						{
								...
								"title":"#trim(listgetat(i,2,'|'))#",
								"render": function ( data, type, row, meta ) {
									var result='<input form="cat_rec_sch_frm" name="remove_row" type="checkbox" value="' + data + '" >';
									return result;
								}
							}
							*/
	...
	<input id="bcrbst" form="cat_rec_sch_frm" type="button" class="cstmBtn" id="customCatRecRslt" value="Customize Results"  onclick="customizeCatRecordResults();">
	<cfif hasjson>
		<span id="jsonCtlBtn">
			<input form="cat_rec_sch_frm"  type="button" class="lnkBtn" id="procJsonClk" value="Format JSON"  onclick="initExpandJSON();">
		</span>
	</cfif>
