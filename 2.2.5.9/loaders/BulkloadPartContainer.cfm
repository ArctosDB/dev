<!------------------------------
	...
	drop table cf_temp_barcode_parts;
	...
	create table cf_temp_barcode_parts (
		...
		part_name varchar,
		barcode varchar,
		new_container_type varchar,
		ignore_subsamples varchar,
		username varchar not null default session_user,
		last_ts timestamp default current_timestamp,
		status varchar
	);

	grant select, insert, update, delete on cf_temp_barcode_parts to manage_records;
	grant select, usage on cf_temp_barcode_parts_key_seq to public;
...
<cfset thisFormFile=replace(GetCurrentTemplatePath(),Application.webDirectory,'')><!----cachedwithin="#createtimespan(0,0,60,0)#"---->
<cfquery name="cf_component_loader" datasource="uam_god" >
	select * from cf_component_loader where ui_template=<cfqueryparam value="#thisFormFile#" CFSQLType="CF_SQL_varchar">
</cfquery>
<cfdump var="#cf_component_loader#">
...
<cfoutput>
	...
	<cfset templateHeader="guid,guid_prefix,other_id_type,other_id_number,part_id,part_name,ignore_subsamples,barcode,new_container_type">
</cfoutput>
...
<cfsavecontent variable = "defDocTableGuts">
	<tr>
		...
		<td>conditionally</td>
		<td>
			DWC Triplet ("UAM:Mamm:12") identifying the catalog record. Case-sensitive, check https://arctos.database.museum/home.cfm for correct value.
			<br>(guid, part_name) or (part_id) or (other_id_type,other_id_number,part_name) must be provided.
		</td>
	</tr>
	...
	<tr>
		...
		<td>conditionally</td>
		<td>
			(guid, part_name) or (part_id) or (guid_prefix,other_id_type,other_id_number,part_name) must be provided.
		</td>
	</tr>
	...
	<tr>
		...
		<td>
			...
			<br>(guid, part_name) or (part_id) or (guid_prefix,other_id_type,other_id_number,part_name) must be provided.
		</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td>conditionally</td>
		<td>
			Part identifier (available from various downloads). This is the prefered and unambiguous way to identify parts.
			<br>(guid, part_name) or (part_id) or (guid_prefix,other_id_type,other_id_number,part_name) must be provided.
		</td>
	</tr>
	<tr>
		...
		<td>conditionally</td>
		<td>
			Name of the part to move; must resolve to a single part.
			<br>(guid, part_name) or (part_id) or (guid_prefix,other_id_type,other_id_number,part_name) must be provided.
		</td>
	</tr>
	<tr>
		<td>ignore_subsamples</td>
		<td>no</td>
		<td>
			"yes"==>Filter for only parts where sampled_from_obj_id is NULL
			<br>any other value==>consider all parts
		</td>
	</tr>
	<tr>
		<td>barcode</td>
		<td>yes</td>
		<td>
			Barcode of the container into which the part will be moved.
		</td>
	</tr>
	<tr>
		<td>new_container_type</td>
		<td>no</td>
		<td>
			NULL==>do nothing
			<br>Value from <a href="/info/ctDocumentation.cfm?table=ctcontainer_type">ctcontainer_type</a>==>change container type of container identified by barcode.
		</td>
	</tr>
