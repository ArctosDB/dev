...
function addMoreStuff(mode){
	...
	var guts = "/form/DataEntryExtras.cfm?collection_object_id=" + cid + "&uuid=" + uuid + '&action=' + mode + '&guid_prefix=' + guid_prefix;
	$("<div id='dialog' class='popupDialog'><img src='/images/indicator.gif'></div>").dialog({
		autoOpen: true,
		closeOnEscape: true,
		height: 'auto',
...
jQuery(document).ready(function() {
	...
	*/
	 ...
	 $.each($("input[id^='locality_attribute_detr_date_']"), function() {
		$("#" + this.id).datepicker();
    });
...
function saveNewRecord () {
	//console.log('I am saveNewRecord');
		...
		$.ajax({
			...
			success: function(r) {
				...
				if (status){
					...
					//console.log('inserted ' + coid + ' good');
	...
	$.getJSON("/component/Bulkloader.cfc",
		...
		function(r) {
			...
			for (i=0;i<columns.length;i++) {
				...
				var cVal=eval("r.DATA." + columns[i].toUpperCase());
...
function loadedEditRecord(){
	...
	//
	$("select[id^='locality_attribute_type_']").each(function(e){
		var gid='locality_attribute_type_' + String(e+1);
		populateGeology(gid);
	});
...
function loadRecordEnter(collection_object_id){
	//console.log('init loadRecordEnter');
	...
	$.getJSON("/component/Bulkloader.cfc",
		...
		function(r) {
			...
			var ptl="/form/DataEntryAttributeTable.cfm?useCustom=true&guid_prefix=" + r.DATA[0].GUID_PREFIX;
			var tab=document.getElementById('attributeTableCell');
			jQuery.get(ptl, function(data){
				jQuery(tab).html(data);
				for (i=0;i<columns.length;i++) {
				...
				msg('record ' + r.DATA[0].COLLECTION_OBJECT_ID + ' loaded as template','good');
				...
				$.getJSON("/component/Bulkloader.cfc",
					...
					function(r) {
						...
						for (i=0;i<columns.length;i++) {
							var cName=columns[i];
							var uprcname=cName.toUpperCase();
							var cVal=eval("r.DATA." + uprcname);
							var eName=cName.toLowerCase();
				...
				$("#browseThingy").hide();
				$("#enterMode").show();
				// UUID should always start off NULL
				$("#uuid").val('');
...
function unpickLocality () {
	...
	for (i=0;i<6;i++) {
		...
		var thisVal=$("#locality_attribute_value_" + eNum).val();
		$("#locality_attribute_type_" + eNum).attr("readOnly", false).removeClass();

		populateGeology('locality_attribute_type_' + eNum);
		$("#locality_attribute_value_" + eNum).attr("readOnly", false).removeClass().val(thisVal);

		$("#locality_attribute_determiner_" + eNum).attr("readOnly", false).removeClass();
		$("#locality_attribute_remark_" + eNum).attr("readOnly", false).removeClass();
		$("#locality_attribute_units_" + eNum).attr("readOnly", false).removeClass();
		$("#locality_attribute_detr_meth_" + eNum).attr("readOnly", false).removeClass();
		$("#locality_attribute_detr_date_" + eNum).attr("readOnly", false).removeClass();
			...
			jQuery.getJSON("/component/DataEntry.cfc",
				...
				function (r) {
					...
					if (result.LOCALITY_ID[0] < 0) {
						...
						if (r.ROWCOUNT > 6) {
							alert('Whoa! That is a lot of locality attribtues. They will not all be displayed here, but the locality will still have them.');
						}
						try {
						for (i=0;i<6;i++) {
								var eNum=parseInt(i+1);
								$("#locality_attribute_type_" + eNum).attr("readOnly", true).removeClass().addClass('readClr').val('');
								$("#locality_attribute_value_" + eNum).attr("readOnly", true).removeClass().addClass('readClr').val('');
								$("#locality_attribute_units_" + eNum).attr("readOnly", true).removeClass().addClass('readClr').val('');
								$("#locality_attribute_remark_" + eNum).attr("readOnly", true).removeClass().addClass('readClr').val('');
								$("#locality_attribute_determiner_" + eNum).attr("readOnly", true).removeClass().addClass('readClr').val('');
								$("#locality_attribute_detr_meth_" + eNum).attr("readOnly", true).removeClass().addClass('readClr').val('');
								$("#locality_attribute_detr_date_" + eNum).attr("readOnly", true).removeClass().addClass('readClr').val('');
							}
							for (i=0;i<r.ROWCOUNT;i++) {
								if (i<5) {
									var eNum=parseInt(i+1);
									//console.log(eNum + '---' + i);
									//console.log(result.LOCALITY_ATTRIBUTE_TYPE[i]);
									$("#locality_attribute_type_" + eNum).val(result.LOCALITY_ATTRIBUTE_TYPE[i]);
									$("#locality_attribute_value_" + eNum).append('<option value="' + result.LOCALITY_ATTRIBUTE_VALUE[i] + '">' + result.LOCALITY_ATTRIBUTE_VALUE[i] + '</option>').val(result.LOCALITY_ATTRIBUTE_VALUE[i]);
									$("#locality_attribute_units_" + eNum).val(result.LOCALITY_ATTRIBUTE_UNITS[i]);
									$("#locality_attribute_remark_" + eNum).val(result.LOCALITY_ATTRIBUTE_REMARK[i]);

									$("#locality_attribute_determiner_" + eNum).val(result.LOCALITY_ATTRIBUTE_DETERMINER[i]);
									$("#locality_attribute_detr_meth_" + eNum).val(result.LOCALITY_ATTRIBUTE_DETR_METH[i]);
									$("#locality_attribute_detr_date_" + eNum).val(result.LOCALITY_ATTRIBUTE_DETR_DATE[i]);
								}
							}
						} catch(err) {
							// whatever
