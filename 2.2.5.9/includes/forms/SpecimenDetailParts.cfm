...
<cfif format is "full">
	<cfquery name="rparts" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
		...
		from
			...
			inner join coll_obj_cont_hist on coll_object.collection_object_id=coll_obj_cont_hist.collection_object_id
			left outer join coll_object_remark on coll_object.collection_object_id=coll_object_remark.collection_object_id
	...
	<table border class="guidPageTable">
		...
		<tbody>
			<cfloop query="orderedparts">
				...
				<cfif p.part_disposition is "deaccessioned" or p.part_disposition is "discarded" or p.part_disposition is "missing" or p.part_disposition is "transfer of custody" or p.part_disposition is "used up">
				...
				<tr class="#thisCls#" id="pid#part_id#">
					...
					<td data-label="Condition: ">#p.part_condition#</td>
					<td data-label="Disposition: ">
						<span class="ctDefLink" onclick="getCtDoc('ctcoll_obj_disp','#p.part_disposition#')">#p.part_disposition#</span>
					</td>
					<td data-label="Count: ">#p.lot_count#</td>
					<cfif oneOfUs is 1>
						...
						<cfquery dbtype="query" name="tlp">
							select * from ploan where transaction_id is not null and collection_object_id=<cfqueryparam value = "#part_id#" CFSQLType="cf_sql_int">
						</cfquery>
						<td data-label="Loan: ">
							<cfloop query="tlp">
								<div>
					...
					<td data-label="Remark: ">#p.part_remarks#</td>
					<cfif dPattrs.recordcount gt 0>
		...
		<tbody>
			<cfloop query="orderedparts">
				...
				<tr class="#thisCls#" id="pid#part_id#">
					...
					<td data-label="Condition: ">#p.part_condition#</td>
					<td data-label="Disposition: ">#p.part_disposition#</td>
					<td data-label="Qty: ">#p.lot_count#</td>
					<td data-label="Remarks: ">#p.part_remarks#</td>
					<td data-label="ID: "><div class="partIdDispDiv">#application.serverRootURL#/guid/#p.guid#/PID#p.part_id# (not stable)</div></td>
				</tr>
			</cfloop>
		</tbody>
