...
function openOverlay(iurl,title='No title provided.'){
	...
	bgDiv.id = 'bgDiv';
	bgDiv.className = 'bgDiv';
	bgDiv.setAttribute('onclick','closeOverlay()');
	document.body.appendChild(bgDiv);
	...
	$("#overlayDiv").append(overlayDivHeader);

	var closeOverlay=document.createElement('span');
	closeOverlay.className = 'closeOverlay';
	closeOverlay.id='closeOverlay';
	closeOverlay.setAttribute('onclick','closeOverlay()');
	closeOverlay.append('×');
	$("#overlayDivHeader").append(closeOverlay);

	$("#overlayDiv").append('<img id="overlayLoadingImg" src="/images/loadingAnimation.gif" class="centeredImage">');
	...
	$("#overlayDiv").append(overlayiFrame);
	$("#overlayLoadingImg").remove();
}
function closeOverlay(){
	/* pairs with openOverlay */
	$('#overlayDiv').remove();
	$('#overlayDiv', window.parent.document).remove();
	$('#bgDiv').remove();
}
...
function customizePlaceSearchForm(typ){
	var guts = "/form/customizePlaceSearch.cfm?typ=" + typ;
...
function customizePlaceForm(sch){
	var guts = "/form/customizePlace.cfm?sch=" + sch;
	$("<iframe src='" + guts + "' id='dialog' class='popupDialog' style='width:1200px;height:600px;'></iframe>").dialog({
		autoOpen: true,
		closeOnEscape: true,
		height: 'auto',
