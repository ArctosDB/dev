...
<cfoutput>
	...
	<!-----------
	 and len (
		len(guid_prefix) gt 0 or
		len(scientific_name) gt 0 or
		len(kingdom) gt 0 or
		len(guid_prefix) gt 0 or
		len(guid_prefix) gt 0 or
	...
	<div id="qcolsoc">
		<cfloop query="cf_cat_rec_summary_cols">
			<div class="showhideitem">
				<label for="#obj_name#">
					<div>
						#obj_name#: <input type="checkbox" name="selected_col" value="#obj_name#" id="ckbx#obj_name#" <cfif listfind(show_columns,obj_name)> checked="checked" </cfif> >
					</div>
				</label>
			</div>
		</cfloop>
