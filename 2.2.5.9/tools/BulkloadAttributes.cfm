...
create sequence cf_temp_attributes_key_seq;
...
alter table cf_temp_attributes add last_ts timestamp default current_timestamp;

alter table cf_temp_attributes drop column collection_object_id;
alter table cf_temp_attributes drop column determined_by_agent_id;


alter table cf_temp_attributes ALTER column determiner TYPE varchar(255);


grant insert,select on cf_temp_attributes to data_entry;
...
<cfoutput>
	...
	<cfset title="Bulkload Attributes">
	<cfset thisFormFile="BulkloadAttributes.cfm">
	<cfset thisFormName="Bulkload Attributes">
	<cfset thisFormPurpose='This tool allows Arctos operators with the <a href="/Admin/user_roles.cfm">Manage Collection</a> role to create and review Attributes.'>
	<cfset thisTable="cf_temp_attributes">
	<cfset thisTemplateName="BulkloadAttributesTemplate.csv">
	<cfset thisDownloadName="BulkloadAttributesData.csv">
	<cfset templateHeader="guid,guid_prefix,other_id_type,other_id_number,attribute,attribute_value,attribute_units,attribute_date,attribute_meth,determiner,remarks">
...
<cfsavecontent variable = "defDocTableGuts">
	<tr>
		...
		<td>conditionally</td>
		<td>
			Either GUID or (guid_prefix,other_id_type,other_id_number) must be provided and resolve to one catalog record. "DWC Triplet," UAM:Mamm:12.
		</td>
	</tr>
	<tr>
		<td>guid_prefix</td>
		<td>conditionally</td>
		<td>
			Required if GUID is not provided, unless when other_id_type is UUID.
		</td>
	</tr>
	<tr>
		<td>other_id_type</td>
		<td>conditionally</td>
		<td>
			Required if GUID is not provided. <a href="/info/ctDocumentation.cfm?table=CTCOLL_OTHER_ID_TYPE">CTCOLL_OTHER_ID_TYPE</a>
		</td>
	</tr>
	<tr>
		...
		<td>conditionally</td>
		<td>
			Required if GUID is not provided.
		</td>
	</tr>
	<tr>
		<td>attribute</td>
