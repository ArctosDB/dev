...
<cfoutput>
	...
	<cftransaction>
		<cfinvoke component="/component/utilities" method="uploadToTable">
	    	<cfinvokeargument name="tblname" value="cf_temp_delete_oids">
		</cfinvoke>
	</cftransaction>
...
<cfoutput>
	<cfquery name="presetstatus" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
		update
			cf_temp_delete_oids
		set
			(status)=(
...
<cfif action is "delete">
	<cfoutput>
		...
		<cfloop query="d">
			<cfquery name="buhBye" result="dq" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
