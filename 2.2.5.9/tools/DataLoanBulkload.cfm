...
<cfif #action# is "getFile">
	...
	<cftransaction>
		<cfinvoke component="/component/utilities" method="uploadToTable">
	    	<cfinvokeargument name="tblname" value="cf_temp_data_loan_item">
		</cfinvoke>
	</cftransaction>
...
<cftransaction>
	...
	<cfquery name="data" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
		...
		<cfloop query="data">
			...
			<cfelse>
				no CI
				<cfquery name="BooCollObj" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
					update
						cf_temp_data_loan_item
					set
						status='no item'
