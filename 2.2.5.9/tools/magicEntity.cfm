...
<cfif action is "nothing">
	<cfoutput>
		<h3>Entity Magic</h3>
		<p>
			This tool will try to find and create Entities used as Organism ID for records in the search. Records must share an ID for this to work.
		</p>
		<p>
...
<cfif action is "findem">
	<cfoutput>
		...
		<cfloop query="u_l_id">
			<br>Looking for #issuedBy#: #display_value#
		...
		<cfif problems.recordcount gt 0>
			...
			<cfdump var="#problems#">
			<p>
				<a href="/search.cfm?table_name=#table_name#&remove_row=#valuelist(problems.collection_object_id)#">Click here</a> to perform another search excluding problematic records. DO NOT try to re-use this tab; the link will perform a new search, pick manage/etity magic from it.
			</p>
		<cfelse>
			<p>
				...
				<ul>
					<li>
						...
						<ul>
							...
							<li>Identification: Group's current shared scientific name, attributed to you, today, nature_of_id=relationship</li>
						</ul>
					</li>
		...
		<cfloop query="u_l_id">
			...
			<cfquery name="iddata" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
				...
				where
					...
					identification.collection_object_id=<cfqueryparam cfsqltype="int" value="#one_rec.mcid#">
			</cfquery>
			...
			<cfquery name="identification" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
				insert into identification (
					...
					<cfqueryparam value = "#source.nextCID#" CFSQLType="cf_sql_int">,
					<cfqueryparam value = "relationship" CFSQLType="CF_SQL_varchar">,
					1,
					null,
					'A',
