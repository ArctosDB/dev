...
<cfoutput>
	<!-----------
		...
		create table temp_cf_morphosource_bot_log (
	...
	<cfquery name="ms_coln_recs" datasource="uam_god">
		...
		FROM cte
			...
			inner join coll_obj_other_id_num on flat.collection_object_id=coll_obj_other_id_num.collection_object_id and 
				coll_obj_other_id_num.other_id_type='Morphosource'
		where 
			coll_obj_other_id_num.display_value not in (
				select morphosource_id from temp_cf_morphosource_bot_log where last_check_date > current_date -  interval '6 months'
			) limit 10
	</cfquery>
	...
	<cfloop query="ms_coln_recs">
		...
		<cfquery name="temp_cf_morphosource_bot_log_clean" datasource="uam_god">
			delete from temp_cf_morphosource_bot_log where morphosource_id=<cfqueryparam value='#display_value#' CFSQLType="cf_sql_varchar">
		</cfquery>
		<cfquery name="temp_cf_morphosource_bot_log_log" datasource="uam_god">
			insert into temp_cf_morphosource_bot_log (morphosource_id) values (<cfqueryparam value='#display_value#' CFSQLType="cf_sql_varchar">)
		</cfquery>

		<cfif debug>
			<cfquery name="is_the_damned_thing_there" datasource="uam_god">
				select * from  temp_cf_morphosource_bot_log where morphosource_id=<cfqueryparam value='#display_value#' CFSQLType="cf_sql_varchar">
			</cfquery>
