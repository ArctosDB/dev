...
<cfloop query="d">
	...
	<cfif len(d.collecting_event_name) gt 0>
		<cfquery name="x" datasource="uam_god">
	...
	<cfif len(locid) is 0>
		<!--- didn't get a locality, didn't skip, check locality-stuff ---->
		...
		<cfif len(d.ORIG_LAT_LONG_UNITS) gt 0>
			...
			<cfif len(d.datum) is 0 or len(d.GEOREFERENCE_SOURCE) is 0 or len(d.GEOREFERENCE_PROTOCOL) is 0>
