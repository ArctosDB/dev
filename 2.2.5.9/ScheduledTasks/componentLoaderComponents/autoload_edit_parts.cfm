	...
	<cfloop query="d">
		...
		<cfif getGuidPrefix.recordcount neq 1>
			<cfquery name="fail" datasource="uam_god">
				update cf_temp_bulk_edit_parts set status='bad part_id' where key=<cfqueryparam value="#d.key#" CFSQLType="cf_sql_int">
			</cfquery>
			<cfcontinue />
		</cfif>
			...
			<cftransaction>
				<cfif len(d.part_name) gt 0>
					<cfquery name="upp" datasource="uam_god">
						update specimen_part set part_name=<cfqueryparam value="#d.part_name#" CFSQLType="CF_SQL_VARCHAR">
						where collection_object_id=<cfqueryparam value="#d.part_id#" CFSQLType="cf_sql_int">
					</cfquery>
				</cfif>
				<cfif len(d.condition) gt 0 or len(d.disposition) gt 0 or len(d.lot_count) gt 0>
					<cfset addacomma=false>
					<cfquery name="upco" datasource="uam_god">
						update coll_object set
							<cfif len(d.condition) gt 0>
								condition=<cfqueryparam value="#d.condition#" CFSQLType="cf_sql_varchar">
								<cfset addacomma=true>
							</cfif>
							...
							<cfif len(d.lot_count) gt 0>
								<cfif addacomma>,</cfif>
								lot_count=<cfqueryparam value="#d.lot_count#" CFSQLType="cf_sql_int">
							</cfif>
						where collection_object_id=<cfqueryparam value="#d.part_id#" CFSQLType="cf_sql_int">
					</cfquery>
				</cfif>
				<cfif len(d.remarks) gt 0>
					<cfquery name="has_rmk" datasource="uam_god">
						select count(*) c from coll_object_remark where collection_object_id=<cfqueryparam value="#d.part_id#" CFSQLType="cf_sql_int">
					</cfquery>
					<cfif has_rmk.c gt 0>
						<cfquery name="up_rmk" datasource="uam_god">
							update coll_object_remark set coll_object_remarks=<cfqueryparam value="#d.remarks#" CFSQLType="cf_sql_varchar">
							where collection_object_id=<cfqueryparam value="#d.part_id#" CFSQLType="cf_sql_int">
						</cfquery>
					<cfelse>
						<cfquery name="ins_rmk" datasource="uam_god">
							insert into coll_object_remark(
								collection_object_id,
								coll_object_remarks
							) values (
								<cfqueryparam value="#d.part_id#" CFSQLType="cf_sql_int">,
