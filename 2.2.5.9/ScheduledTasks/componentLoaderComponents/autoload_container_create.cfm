		...
		<cftransaction>
			...
			<cfif len(d.barcode) gt 0>
				<cfquery name="is_dup_barcode" datasource="uam_god">
					select count(*) c from container where barcode=<cfqueryparam value="#d.barcode#" CFSQLType="CF_SQL_VARCHAR" null="#Not Len(Trim(d.barcode))#">
				</cfquery>
				<cfif debug>
				...
				<cfquery name="is_claimed_barcode" datasource="uam_god">
					select is_claimed_barcode(<cfqueryparam value="#d.barcode#" CFSQLType="CF_SQL_VARCHAR" null="#Not Len(Trim(d.barcode))#">) cb
				</cfquery>
				<cfif debug>
			...
			<cfquery name="insctr" datasource="uam_god">
				 insert into container (
					...
					'autoload_container_creaete'
				)
			</cfquery>

			<cfif debug is true>
				<br>delete from cf_temp_container_create where key=<cfqueryparam value="#d.key#" CFSQLType="cf_sql_int">
			</cfif>
