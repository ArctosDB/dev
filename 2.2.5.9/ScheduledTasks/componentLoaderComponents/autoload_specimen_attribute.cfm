...
<cfloop query="d">
	...
	<cfelseif d.other_id_type is "UUID">
	...
	<cfquery name="x" datasource="uam_god">
		select isValidAttribute(
				...
				<cfqueryparam value="#collObj.collection_cde#" CFSQLType="CF_SQL_VARCHAR" null="#Not Len(Trim(collObj.collection_cde))#">
			) v
	</cfquery>
	<cfif debug>
	...
	<cfif x.v neq 1>
		<cfquery name="fail" datasource="uam_god">
			update cf_temp_attributes set status='invalid attribute' where key=#val(d.key)#
		</cfquery>
		<cfcontinue />
	</cfif>
	...
	<cfif len(d.ATTRIBUTE_DATE) gt 0>
		<cfquery name="x" datasource="uam_god" cachedwithin="#createtimespan(0,0,60,0)#">
			select is_iso8601('#d.ATTRIBUTE_DATE#') v
		</cfquery>
		<cfif x.v neq 'valid'>
			<cfquery name="fail" datasource="uam_god">
	...
	<cfif len(d.determiner) gt 0>
		<cfquery name="x" datasource="uam_god" cachedwithin="#createtimespan(0,0,60,0)#">
			select getAgentID('#d.determiner#') v
		</cfquery>
		<cfif len(x.v) lt 1>
			<cfquery name="fail" datasource="uam_god">
	...
	<cfelse>
		<cfquery name="fail" datasource="uam_god">
			update cf_temp_attributes set status='determiner is required' where key=#val(d.key)#
		</cfquery>
		<cfcontinue />
	</cfif>
	<cftry>
		<cftransaction>
			<cfquery name="x" datasource="uam_god">
				...
				VALUES (
					...
					<cfqueryparam value="#dtrID#" CFSQLType="cf_sql_int">,
					<cfqueryparam value="#d.attribute#" CFSQLType="CF_SQL_VARCHAR" null="#Not Len(Trim(d.attribute))#">,
					<cfqueryparam value="#d.attribute_value#" CFSQLType="CF_SQL_VARCHAR" null="#Not Len(Trim(d.attribute_value))#">,
					<cfqueryparam value="#d.attribute_units#" CFSQLType="CF_SQL_VARCHAR" null="#Not Len(Trim(d.attribute_units))#">,
