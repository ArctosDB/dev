...
<cfoutput>
	...
	<cfloop query="d">
		...
		<cfquery name="donor_container" datasource="uam_god">
			select * from container where container_type like '% label' and barcode=<cfqueryparam value="#d.replacement_barcode#" CFSQLType="cf_sql_varchar">
		</cfquery>
		<cfif debug>
		...
		<cfif donor_container.parent_container_id neq 0>
			<cfset errs="bad donor: not in parent0">
			<cfif debug>
				<cfdump var=#errs#>
			</cfif>
			<cfset x=update_record(d.key,errs)>
		...
		<cftry>
			<cftransaction>
				<cfquery name="die_old" datasource="uam_god">
					delete from container where barcode=<cfqueryparam value="#d.replacement_barcode#" CFSQLType="CF_SQL_VARCHAR">
				</cfquery>
