...
<hr>
		<cfquery name="shipment" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			select
				shipment_id,
				PACKED_BY_AGENT_ID,
			...
			where
				...
				shipment.transaction_id=<cfqueryparam value = "#transaction_id#" CFSQLType="cf_sql_int">
		</cfquery>
		...
		<form name="newshipment" method="post" action="borrow.cfm">
			...
			<input type="text" name="packed_by_agent" id="packed_by_agent" class="reqdClr" size="50"
				  onchange="pickAgentModal('packed_by_agent_id',this.id,this.value); return false;"
				  onKeyPress="return noenter(event);">
		...
		<h3>Edit Shipment</h3>
			<form name="shipment#i#" method="post" action="borrow.cfm">
				...
				<label for="packed_by_agent">Packed By Agent</label>
				<input type="text" name="packed_by_agent" id="packed_by_agent_#i#" class="reqdClr" size="50" value="#packed_by_agent#"
					onchange="pickAgentModal('packed_by_agent_id_#i#',this.id,this.value);"
					onKeyPress="return noenter(event);">
				<input type="hidden" name="packed_by_agent_id" id="packed_by_agent_id_#i#" value="#packed_by_agent_id#">
