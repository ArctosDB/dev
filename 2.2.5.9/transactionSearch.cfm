...
<cfif action is "srch">
	...
	<cfoutput>
		...
		<!------ group by is slow, don't do this
		<cfif loanQueryTbls does not contain " loan_item ">
			<cfset loanQueryTbls=loanQueryTbls & " left outer join loan_item on trans.transaction_id=loan_item.transaction_id ">
		</cfif>
		----------->
		...
		<cfif isdefined("loaned_part_id") and len(loaned_part_id) gt 0>
			...
			<cfset thisrow.d="cf_sql_int">
			<cfset thisrow.o="in">
			<cfset thisrow.t="loan_item.collection_object_id">
		...
		<cfif isdefined("loaned_part_name") and len(loaned_part_name) gt 0>
			...
			<cfif loanQueryTbls does not contain " loan_srch_part ">
				<cfset loanQueryTbls=loanQueryTbls & " inner join specimen_part loan_srch_part on loan_item.collection_object_id=loan_srch_part.collection_object_id ">
			</cfif>
		...
		<cfif isdefined("loaned_part_dispn") and len(loaned_part_dispn) gt 0>
			...
			<cfif loanQueryTbls does not contain " loan_srch_part_co ">
				<cfset loanQueryTbls=loanQueryTbls & " inner join coll_object loan_srch_part_co on loan_srch_part.collection_object_id=loan_srch_part_co.collection_object_id ">
			</cfif>
			<cfset thisrow={}>
			<cfset thisrow.l="true">
			<cfset thisrow.d="cf_sql_varchar">
			<cfset thisrow.o="in">
			<cfset thisrow.t="loan_srch_part_co.coll_obj_disposition">
		...
		<cfif isdefined("loan_has_items") and len(loan_has_items) gt 0>
			<cfif loan_has_items is "true">
				...
				<cfset thisrow.o="">
				<cfset thisrow.t="loan_item_null.collection_object_id">
				<cfset thisrow.v="">
				<cfset arrayappend(loanQueryParams,thisrow)>
			<cfelse>
				<cfif loanQueryTbls does not contain " loan_item_null ">
										...
										<cfif not isNull(a.short_citation) and  not isNull(a.publication_id)>
											<div class="itemSubSubTitle">
												...
												<cfif not isNull(a.doi)>
													(<a class="external" href="https://doi.org/#a.doi#">#a.doi#</a>)
												</cfif>
											</div>
										</cfif>
									</cfloop>
