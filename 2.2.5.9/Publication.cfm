...
<cfoutput>
	...
	<form name="editPub" method="post" action="Publication.cfm">
		...
		<cfif len(pub.datacite_doi) gt 0>
			<a class="infoLink external" target="_blank" href="https://doi.org/#pub.datacite_doi#">[ open DOI ]</a>
		<cfelse>
...
<cfoutput>
	<cftransaction>
		<cfif len(doi) gt 0>
			<cfinvoke component="component.functions" method="stripDOI" returnvariable="doi">
				<cfinvokeargument name="doi" value="#doi#">
			</cfinvoke>
			<cfinvoke component="/component/functions" method="checkBareDOI" returnVariable="isok">
				<cfinvokeargument name="doi" value="#doi#">
			</cfinvoke>
		...
		<cfif len(datacite_doi) gt 0>
			<cfinvoke component="component.functions" method="stripDOI" returnvariable="datacite_doi">
	...
	<cfoutput>
		...
		<form name="newpub" method="post" onsubmit="if (!confirmpub()){return false;}" action="Publication.cfm">
			...
			<div id="pleaseusethis">
				...
				<input type="button" class="lnkBtn" id="doilookup" onclick="getPubMeta('DOI');" value="crossref">
