			...
			<div class="summary_row">
				<div id="annotateSpace">
					<cfquery name="existingAnnotations" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
						select
							case when REVIEWER_AGENT_ID is null then 0 else 1 end isreviewed,
							count(*) cnt
						from
							annotations
						where
							collection_object_id = <cfqueryparam value="#detail.collection_object_id#" CFSQLType="cf_sql_int">
						group by
					...
					<button type="button" onclick="openAnnotation('collection_object_id=#detail.collection_object_id#')" class="annobtn">
						<span class="abt">Comment or Report Bad Data&nbsp;<span class="gdAnnoCt">[#gac#]</span><span class="badAnnoCt">[#bac#]</span></span>
					</button>
				</div>
			</div>
			<cfif isdefined("session.roles") and listfindnocase(session.roles,"coldfusion_user")>
		...
		<div id="gp_identifications" class="">
			<cfloop query="identification">
				...
				<cfquery name="thisTaxLinks" dbtype="query">
					select distinct taxsciname from raw_identification where taxsciname is not null and 
	        	...
	        	<div class="one_identification #tdc#" id="iid#identification_id#">
					<cfif thisTaxLinks.recordcount is 1 and thisTaxLinks.taxsciname is identification.scientific_name>
						<a href="/name/#thisTaxLinks.taxsciname#" target="_blank">#identification.scientific_name#</a>
					...
					<cfelse>
						...
						<cfloop query="thisTaxLinks">
							<div style="font-size:smaller; margin-left:1em">
								<a href="/name/#thisTaxLinks.taxsciname#" target="_blank">#thisTaxLinks.taxsciname#</a>
							</div>
						</cfloop>
					</cfif>
					<div class="taxDetDiv">
						<cfif accepted_id_fg is 1>
							<div class="taxaMeta">
								#detail.FORMATTED_SCIENTIFIC_NAME#
							</div>
							<div class="taxaMeta">
								#detail.full_taxon_name#
							</div>
						</cfif>
						...
						<div>
							Nature of ID: <span class="ctDefLink" onclick="getCtDoc('ctnature_of_id','#nature_of_id#')">#nature_of_id#</span>
						</div>
						<cfif len(identification_confidence) gt 0>
							<div>Confidence: 
								<span class="ctDefLink" onclick="getCtDoc('ctidentification_confidence','#identification_confidence#')">
									#identification_confidence#
								</span>
							</div>
						</cfif>
			...
			<div id="gp_citation_container"  class="gp_one_section">
				...
				<cfloop query="citations">
					...
					<div class="one_citation">
						...
						<cfif len(doi) gt 0>
							<a href="https://doi.org/#doi#" target="_blank" class="external sddoi">https://doi.org/#doi#</a>
						</cfif>
		...
		<div class="gp_item_type" id="idsTbl">
			<label for="idsTbl" class="gp_section_label">
				...
				<select name="idViewSelector" id="idViewSelector" onchange="loadIdTable(this.value);">
					...
					<option <cfif session.idsview is "full_separated"> selected="selected" </cfif>value="full_separated">full/split view</option><option <cfif session.idsview is "minimal"> selected="selected" </cfif>value="minimal">minimal view</option>
				</select>
			</label>
				
			<div class="detailBlock">
				<span class="detailData">
					<div id="idTableDiv"></div>
				</span>
			</div>
		</div>
		<!----------------------------------- Identifiers --------------------------------------->
		...
		<div id="cat_rec_core_junk"  class="gp_one_section">
			...
			<cfquery name="isProj" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			  ...
			  <cfquery name="isLoan" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
					SELECT project_name, project.project_id FROM
						...
						specimen_part
					 WHERE
					 	specimen_part.derived_from_cat_item = <cfqueryparam value = "#detail.collection_object_id#" CFSQLType = "cf_sql_int"> AND
					...
					<!---- union dataloans---->
					union
					SELECT project.project_name, project.project_id FROM
						project
					 ...
					 WHERE
					 	loan_item.collection_object_id = <cfqueryparam value = "#detail.collection_object_id#" CFSQLType = "cf_sql_int"> 
					GROUP BY
						project_name, project.project_id
			...
			<cfquery name="isLoanedItem" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
				...
				FROM
					loan_item
				WHERE
					loan_item.collection_object_id=#detail.collection_object_id#
			</cfquery>
			...
			<cfif len(detail.remarks) gt 0>
				<div class="gp_dp">
					<div class="gp_dp_label">Remarks:</div>
					<div class="gp_dp_data">
						#detail.remarks#
					</div>
				</div>
			...
			<cfif oneOfUs is 1>
				...
				<cfif len(detail.flags) gt 0>
					<div class="gp_dp">
						<div class="gp_dp_label">Missing (flags):</div>
						<div class="gp_dp_data">
							#detail.flags#
						</div>
					</div>
				...
				<div class="one_locality #thisClass#" id="SD_#specimen_event_id#">
					<div class="gp_locality_except_attrs">
						<div class="text_loc">
							...
							<cfif len(detail.associated_species) gt 0>
								<div class="gp_dp">
									<div class="gp_dp_label">Associated&nbsp;Species:</div>
									<div class="gp_dp_data">
										#detail.associated_species#
									</div>
								</div>
							</cfif>
						...
						<div class="loc_coordinate">
							<div class="gp_dp">
								...
								<div class="gp_dp_data">
									...
									<cfelseif len(s$dec_lat) gt 0 and len(s$dec_long) gt 0 and oneOfUs is 1 and len(locality_name) is 0>
							...
							<cfif len(georeference_source) gt 0>
								<div class="gp_dp">
									<div class="gp_dp_label">Georeference&nbsp;Source:</div>
									<div class="gp_dp_data">
										#georeference_source#
									</div>
								</div>
