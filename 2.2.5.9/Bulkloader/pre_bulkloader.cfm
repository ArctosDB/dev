...
<cfoutput>
	...
	<cfif action is "nothing">
		...
		<cfquery name="sts" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			select status,enteredby,count(*) numrecs from pre_bulkloader group by status,enteredby
		</cfquery>
		<p>
		...
		<ol>
			...
			<li><a href="pre_bulkloader.cfm?action=checkStatus">checkStatus</a>. The checks are done when ALL status=init_pull_complete</li>

			<cfset tbls="pre_bulk_agent,pre_bulk_taxa,pre_bulk_attributes,pre_bulk_oidt,pre_bulk_date,pre_bulk_parts,pre_bulk_locality_attributes">
			...
			<li><a href="pre_bulkloader.cfm?action=checkStatus">checkStatus</a>. The repatriation is done when ALL status=repatriation_complete</li>
			<li>
				...
				<cfset dfltnls.SPECIMEN_EVENT_TYPE="collection">
				<cfset dfltnls.PART_NAME_1="unknown">
				<cfset dfltnls.VERIFICATIONSTATUS="unverified">
				<cfset dfltnls.NATURE_OF_ID="unknown">
			...
			<li>
				...
				Collection_object_id will be replaced. status will be set to "pushed_from_prebulk."
			</li>
		</ol>
	</cfif>

	<cfif action is "goGoCoordinateMagic">
		<cfquery name="bah" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#" result="myQueryResult">
		...
		<cfquery name="udllu" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#" result="myQueryResult">
			update
				pre_bulkloader
			set
	...
	<cfif action is "ready_for_checkall">
		<cfquery name="uppc" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#" result="myQueryResult">
			UPDATE pre_bulkloader SET status='ready_for_checkall'
		</cfquery>
	...
	<cfif action is "buildCollectionObjectID">
		<cfquery name="uppc" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#" result="myQueryResult">
			UPDATE pre_bulkloader SET collection_object_id=nextval('bulkloader_pkey') WHERE collection_object_id is null
		</cfquery>
	...
	<cfif action is "setNullDefaultsAttribute">
		...
		<cfif len(attribute_determiner_n) gt 0>
			<cfloop from="1" to="#numberOfAttributes#" index="i">
				<cfquery name="uppc" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
					update
						pre_bulkloader
					set
						attribute_determiner_#i#=<cfqueryparam value="#attribute_determiner_n#" CFSQLType="cf_sql_varchar">
					where
			...
			<cfloop from="1" to="#numberOfAttributes#" index="i">
				<cfquery name="uppc" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
					update
						pre_bulkloader
					set
						attribute_date_#i#=<cfqueryparam value="#attribute_date_n#" CFSQLType="cf_sql_varchar">
					where
						attribute_#i# is not null and
			...
			<cfloop from="1" to="#numberOfParts#" index="i">
				<cfquery name="uppc" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
					update
						pre_bulkloader
					set
						PART_CONDITION_#i#=<cfqueryparam value="#PART_CONDITION_n#" CFSQLType="cf_sql_varchar">
					where
			...
			<cfloop from="1" to="#numberOfParts#" index="i">
				<cfquery name="uppc" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
					update
						pre_bulkloader
					set
						PART_LOT_COUNT_#i#=<cfqueryparam value="#PART_LOT_COUNT_n#" CFSQLType="cf_sql_varchar">
					where
						part_name_#i# is not null and
			...
			<cfloop from="1" to="#numberOfParts#" index="i">
				<cfquery name="uppc" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
					update
						pre_bulkloader
					set
						PART_DISPOSITION_#i#=<cfqueryparam value="#PART_DISPOSITION_n#" CFSQLType="cf_sql_varchar">
					where
						part_name_#i# is not null and
	...
	<cfif action is "setNullDefaults">
		...
		<cfloop list="#fldlist#" index="fld">
			...
			<br>update pre_bulkloader set #fld#='#v#' where #fld# is null
			<cfquery name="upnull" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
				update pre_bulkloader set #fld#=<cfqueryparam value="#v#" CFSQLType="cf_sql_varchar"> where #fld# is null
			</cfquery>
		</cfloop>
		<p>
	...
	<cfif action is "repatriate">
		<cfquery name="nullLoaded" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			update pre_bulkloader set status='go_go_gadget_repatriate'
		</cfquery>
		<cflocation url="pre_bulkloader.cfm" addtoken="false">
	</cfif>
	<!------------------------------------------------------->
	<cfif action is "getFile">
	        ...
	        <cfloop query="x">
	            <cfquery name="ins" datasource="uam_god">
		            ...
		            <cfloop list="#cols#" index="i">
		            	<cfqueryparam value="#evaluate(i)#" CFSQLType="cf_sql_varchar">
		...
		status to #table# <a href="pre_bulkloader.cfm?action=nothing##u_#table#">return</a>
	</cfif>
	<!------------------------------------------------------->
	...
	<cfif action is "checkStatus">
		<cfquery name="checkStatus" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			select status,count(*) c from pre_bulkloader group by status
		</cfquery>
	...
	<cfif action is "precheckLoaded">
		<cfquery name="nullLoaded" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			update pre_bulkloader set status='go_go_gadget_precheck'
		</cfquery>
		<cflocation url="pre_bulkloader.cfm" addtoken="false">
	</cfif>
	<!------------------------------------------------------->
	<cfif action is "deleteAll">
		<cfquery name="deleteAll" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
	...
	<cfif action is "instobulk">
		<cfquery name="uppc" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#" result="myQueryResult">
			UPDATE pre_bulkloader SET collection_object_id=nextval('bulkloader_pkey')
		</cfquery>
		<p>
	...
	<cfif action is "setLoadedForLoad">
		<cfquery name="uppc" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#" result="myQueryResult">
			UPDATE pre_bulkloader SET status='pushed_from_prebulk'
		</cfquery>
		<p>
			status updated to pushed_from_prebulk.
	...
	<cfif action is "pushToBL">
		<cfquery name="c" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#" result="myQueryResult">
			select * from pre_bulkloader where 1=2
		</cfquery>
		<cfset cl=c.columnList>
		<cfset cl=listdeleteat(cl,listfindnocase(cl,'collection_cde'))>
		<cfquery name="ibl" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#" result="myQueryResult">
	...
	<cfif action is "pushToBL_SUCCESS">
		<cfquery name="uppc" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#" result="myQueryResult">
			UPDATE pre_bulkloader SET status='pushed to BULKLOADER #dateformat(now(),"yyyy-mm-dd")# by #session.username#'
		</cfquery>
		<p>
