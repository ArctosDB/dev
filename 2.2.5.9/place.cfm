...
<cfif action is "nothing">
	...
	<cfoutput>
					...
					columns: [
						{
							...
							"render": function ( data, type, row, meta ) {
								...
								<cfelseif sch is "geog">
									...
									<cfif isAdmin is true>
										result+='<br><a target="_blank" class="external" href="editGeog.cfm?geog_auth_rec_id=' + row["geog_auth_rec_id"] + '">Edit</a>';
									</cfif>
		...
		<div id="ctlBtns">
			...
			<cfif len(session.username) gt 0>
				<span id="menButton">
					<input type="button" class="cstmBtn" id="hideEmptyColumns" value="Customize Results"  onclick="customizePlaceForm('#sch#');">
				</span>
			</cfif>
		</div>
		<br>
...
<script>
	...
	function initializeLocalityMap(){
		if ($('#locpoly').length) {
			...
			// add geography if theres data
			var gp=$("#geopoly").val();
			if (gp.length>0){
				var geogeojson = JSON.parse(gp);
				loc_map.data.addGeoJson(geogeojson);
			}
	...
	function initializeGeographyMap() {
		...
		if (mst.length>0){
			//console.log('go');
			var geojson = JSON.parse(mst);
			geog_map.data.addGeoJson(geojson);
	...
	<cfoutput>
		...
		<cfif isdefined("locality_id") and len(locality_id) gt 0>
			...
			<cfquery name="loc" datasource="uam_god" >
				select
					...
					ST_AsGeoJSON(ST_ForcePolygonCCW(ST_Buffer(ST_MakePoint(s$dec_long, s$dec_lat)::geometry,coalesce(s$error_meters,0.5),8))) ||
			...
			<!--- map-stuff for JS -------->
			<input type="hidden" id="s_dollar_dec_lat" value="#loc.s$dec_lat#">
			<input type="hidden" id="s_dollar_dec_long" value="#loc.s$dec_long#">
		...
		<cfif isdefined("geog_auth_rec_id") and len(geog_auth_rec_id) gt 0>
			...
			<input type="hidden" id="geopoly" value="#encodeforhtml(geo.spatial_footprint)#">
			...
			<tr>
				<td valign="top"><!--- begin left-side div --->
					...
					<ul>
						...
						<cfif isAdmin is true>
							<li><a href="/editGeog.cfm?geog_auth_rec_id=#geo.geog_auth_rec_id#">Edit Geography</a></li>
						</cfif>
						<cfif len(geo.spatial_footprint) gt 0>
							<!----<li>
								<span class="likeLink" onclick="openOutsidePoints();">
									Find catalog records with coordinates "outside" the geography shape (new window)
								</span>
							</li>
							---->
							<cfif geo.valid_catalog_term_fg is 1>
								<li>
					...
					<table>
				 		<cfif len(geo.start_date) gt 0>
							<tr>
								<td class="titleCell">Valid Dates</td>
								<td class="dataCell">#geo.start_date# - #geo.stop_date#</td>
							</tr>	 			
				 		</cfif>

						<cfif geo.valid_catalog_term_fg neq 1>
							<tr>
								<td class="titleCell">Assertable</td>
								<td class="dataCell">This geography cannot be used for cataloging.</td>
							</tr>
						</cfif>	 	
						<tr>
						...
						<cfif len(geo.continent_ocean) gt 0>
							<tr>
								<td class="titleCell">Continent/Ocean</td>
								<td class="dataCell">#geo.continent_ocean#</td>
							</tr>
						</cfif>
					...
					<cfif isdefined("loc.locality_id") and len(loc.locality_id) gt 0>
						...
						<div class="blockDiv">
							<ul>
								...
								<cfif isAdmin is true>
									...
									<li>
										<a
											target="_blank"
											href="/component/functions.cfc?method=getLocalityCacheStuff&locality_id=#loc.locality_id#&returnformat=plain&debug=false">
											Refresh webservice data
										</a> (close the new tab, refresh this to see results)
									</li>
								</cfif>
								<!----
								<cfif len(loc.wkt_media_id) gt 0>
									<li>has WKT, maybe do something clever here??</li>
								</cfif>
								----->
							...
							<table>
								...
								<cfif len(loc.georeference_source) gt 0>
									<tr>
										<td class="titleCell">Georeference Source</td>
										<td class="dataCell">#loc.georeference_source#</td>
									</tr>
								</cfif>
					...
					<div style="align:right">
						<h4>Geography Map (all localities, error is not displayed)</h4>
							...
							<cfif isdefined("loc.locality_id") and len(loc.locality_id) gt 0>
								...
								<span style="border:3px solid ##43253c;background-color:##43253c;">&nbsp;&nbsp;&nbsp;</span>=geography
							</cfif>
					</div>
				</td>
			</tr>
