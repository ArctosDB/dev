...
<cfif action is "srch">
<cfquery name="loanData" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
	select
	guid_prefix,
	collection_id,
	TRANSACTION_ID,
	...
	from (
		...
		from
			...
			left outer join loan_item on loan.transaction_id=loan_item.transaction_id
			left outer join specimen_part on loan_item.collection_object_id=specimen_part.collection_object_id
			left outer join citation on specimen_part.derived_from_cat_item=citation.collection_object_id
		union
		select
			guid_prefix,
			collection.collection_id,
		...
		from
			...
			inner join collection on trans.collection_id=collection.collection_id
			left outer join loan_item on loan.transaction_id=loan_item.transaction_id
			left outer join specimen_part on loan_item.collection_object_id=specimen_part.derived_from_cat_item
			left outer join citation on loan_item.collection_object_id=citation.collection_object_id
	) als1
	where 1=1
...
<table border id="t" class="sortable">
	...
	<cfloop query="loanData">
		...
		<tr>
			...
			<td>
				<a href="/Ssearch.cfm?loan_trans_id=#loanData.TRANSACTION_ID#&collection_id=#loanData.collection_id#">#CntCatNum#</a>
			</td>
			<td><a href="/Ssearch.cfm?loan_trans_id=#loanData.TRANSACTION_ID#&collection_id=#loanData.collection_id#&type_status=any">
					#cntCited#</a>&nbsp;
			</td>
			<td>#trans_remarks#</td>
		</tr>
