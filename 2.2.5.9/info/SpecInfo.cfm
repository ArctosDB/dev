...
<cfquery name="id" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
		...
		FROM
			...
			left outer join preferred_agent_name enteredPerson on coll_object.entered_person_id = enteredPerson.agent_id
			left outer join preferred_agent_name editedPerson on coll_object.last_edited_person_id = editedPerson.agent_id
		WHERE
