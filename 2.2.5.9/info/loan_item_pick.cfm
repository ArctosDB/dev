...
<cfif action is "nothing">
	...
	<cfoutput>
		...
		<cfset theColumnList="began_date,ended_date,verbatim_date,received_date,coll_obj_disposition,condition,attrcnt,attr1,attr2,attr3,attr4,attr5,part_id,sampled_from_obj_id,bc1,lbl1,coll_object_remarks,higher_geog,spec_locality,item_remarks,item_instructions">
		...
		<cfif loanDetails.loan_type is 'data loan'>
		...
		<div class="top_stuff">
			<div>
				<h4>Add Items - Loan Summary</h4>
				
				<ul>
					<li>
						Loan Number: <strong>#loanDetails.guid_prefix# #loanDetails.loan_number#</strong>
						[ <a href="/Loan.cfm?action=editLoan&transaction_id=#add_to_trans_id#">back to edit loan</a> ]
					</li>
					<li>loan_type: #loanDetails.loan_type#</li>
					<li>loan_status: #loanDetails.loan_status#</li>
					<li>loan_description: #loanDetails.loan_description#</li>
					<li>nature_of_material: #loanDetails.nature_of_material#</li>
					<cfloop query="loanAgents">
						<li>#trans_agent_role#: #agent_name#</li>
					</cfloop>
				</ul>
			</div>
			<div>
				...
				<form name="fltr" method="get" action="loan_item_pick.cfm">
					...
					<label for="filter_dispn">Disposition</label>
					<cfparam name="filter_dispn" default="">
					<select name="filter_dispn" id="filter_dispn" size="5" multiple>
						<cfloop query="ctcoll_obj_disp">
							<option 
								<cfif listcontains(filter_dispn,ctcoll_obj_disp.coll_obj_disposition) > selected="selected" </cfif> 
								value="#ctcoll_obj_disp.coll_obj_disposition#">#ctcoll_obj_disp.coll_obj_disposition#
							</option>
						</cfloop>
					</select>
		...
		from
			...
			<cfif len(filter_dispn) gt 0>
				and coll_object.coll_obj_disposition in (<cfqueryparam value="#filter_dispn#" CFSQLType="cf_sql_varchar" list="true"> )
			</cfif>
		order by
			flat.guid,
			specimen_part.part_name
	...
	<form name="aatl" id="aatl" method="post" action="loan_item_pick.cfm">
		...
		<table border="1" id="pp_outertable">
			<tr>
				...
				<th data-cname="received_date">AccesionedDate</th>	
					...
					<td title="#guid#">
						...
						<table border id="ptbl_#collection_object_id#" class="pp_innertable">
							<tr>
								...
								<th data-cname="coll_object_remarks">PartRemark</th>
							</tr>
							<cfloop query="parts">
								<tr>
									<td>
										<cfset secClass="">
										<cfif coll_obj_disposition is "on loan">
									...
									<td data-cname="coll_object_remarks">
										<div class="pp_ptrmkDiv">#coll_object_remarks#</div>
									</td>
			...
			<cfloop list="#part_id#" index="pid">
				...
				<cfif len(thisSS) gt 0>
					...
					<cfquery name="parentData" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
						...
						WHERE
							coll_object.collection_object_id = <cfqueryparam value="#pid#" CFSQLType="cf_sql_int">
					</cfquery>
					<cfquery name="newCollObj" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
						INSERT INTO coll_object (
							...
							<cfqueryparam value="#session.myAgentId#" CFSQLType="cf_sql_int">,
							current_date,
							<cfqueryparam value="#parentData.coll_obj_disposition#" CFSQLType="cf_sql_varchar">,
							1,
				...
				<cfquery name="setDisp" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
					UPDATE coll_object SET coll_obj_disposition = <cfqueryparam value="#thisDisp#" CFSQLType="cf_sql_varchar">
