	...
	<cfif len(tbl) gt 0>
		...
		<cfquery name="smed" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			...
			where
				...
				media_relations.related_primary_key=#val(kval)#
			order by
				media_id
		</cfquery>
		<style>
		...
		<cfloop query="smed">
			...
			<cfquery name="lbl" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
