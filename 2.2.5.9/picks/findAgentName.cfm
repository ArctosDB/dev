...
<cfif action is "nothing">
	...
	<cfquery name="getAgentId" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
		...
		FROM
			...
			left outer join agent_name dispNames on  searchNames.agent_id = dispNames.agent_id
		WHERE
			UPPER(searchNames.agent_name) LIKE '%#ucase(agentname)#%'
		...
		<cfelse>
			...
			<table border>
				...
				<cfloop query="getAgentId">
					<cfset thisName = EncodeForHTML(agent_name)>
...
<cfif action is "newOtherAgent">
	<cfquery name="ctAgentType" datasource="cf_codetables" cachedwithin="#createtimespan(0,0,60,0)#">
...
<cfif action is "insertPerson">
	<cftransaction>
		<cfquery name="agentID" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			select sq_agent_id.nextval nextAgentId
		</cfquery>
		<cfquery name="agentNameID" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			select sq_agent_name_id.nextval nextAgentNameId
		</cfquery>
		<cfquery name="insPerson" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			INSERT INTO agent (
				agent_id,
				agent_type,
		...
		<cfif not isdefined("ignoreDupChek") or ignoreDupChek is false>
			<cfquery name="dupPref" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
		...
		<cfquery name="insName" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
