...
<cfoutput>
	...
	<cfif action is "nothing">
		...
		<form method="post" action="duplicateLocality.cfm">
			...
			<label for="localityAttrs">
				localityAttrs
			</label>

			<input type="text" name="localityAttrs" size="120" value="#encodeforhtml(localityAttrs)#">

			<label for="locality_footprint">
				locality_footprint
			</label>

			<input type="text" name="locality_footprint" size="120" value="#encodeforhtml(locality_footprint)#">


			<label for="primary_spatial_data">
				primary_spatial_data
			</label>

			<input type="text" name="primary_spatial_data" size="120" value="#encodeforhtml(primary_spatial_data)#">

		


			<br>
			<input readonly="readonly" class="readClr" type="text" size="120" value="#orig.localityAttrs#">
			<br>
		...
		<cfif spec_locality is not "ignore">
			<cfif len(SPEC_LOCALITY) gt 0>
				<upper(SPEC_LOCALITY) like <cfqueryparam value = "#ucase(SPEC_LOCALITY)#" CFSQLType="CF_SQL_VARCHAR">
			<cfelse>
		...
		<cfif DEC_LONG is not "ignore">
			<cfif len(DEC_LONG) gt 0>
				 DEC_LONG=#DEC_LONG# and
			<cfelse>
				 DEC_LONG is null and
			</cfif>
		</cfif>
		<cfif MINIMUM_ELEVATION is not "ignore">
			<cfif len(MINIMUM_ELEVATION) gt 0>
				 MINIMUM_ELEVATION=#MINIMUM_ELEVATION# and
			<cfelse>
				 MINIMUM_ELEVATION is null and
			</cfif>
		</cfif>
		<cfif MAXIMUM_ELEVATION is not "ignore">
			<cfif len(MAXIMUM_ELEVATION) gt 0>
				 MAXIMUM_ELEVATION=#MAXIMUM_ELEVATION# and
			<cfelse>
				 MAXIMUM_ELEVATION is null and
			</cfif>
		</cfif>
		<cfif ORIG_ELEV_UNITS is not "ignore">
			<cfif len(ORIG_ELEV_UNITS) gt 0>
				 ORIG_ELEV_UNITS='#ORIG_ELEV_UNITS#' and
			<cfelse>
				 ORIG_ELEV_UNITS is null and
			</cfif>
		</cfif>
		<cfif MIN_DEPTH is not "ignore">
			<cfif len(MIN_DEPTH) gt 0>
				 MIN_DEPTH=#MIN_DEPTH# and
			<cfelse>
				 MIN_DEPTH is null and
			</cfif>
		</cfif>
		<cfif MAX_DEPTH is not "ignore">
			...
			<cfelse>
				 MAX_DEPTH is null and
			</cfif>
		</cfif>
		<cfif DEPTH_UNITS is not "ignore">
			<cfif len(DEPTH_UNITS) gt 0>
				 DEPTH_UNITS='#DEPTH_UNITS#' and
			<cfelse>
				 DEPTH_UNITS is null and
			</cfif>
		</cfif>
		<cfif MAX_ERROR_DISTANCE is not "ignore">
			<cfif len(MAX_ERROR_DISTANCE) gt 0>
				 MAX_ERROR_DISTANCE=#MAX_ERROR_DISTANCE# and
			<cfelse>
				 MAX_ERROR_DISTANCE is null and
			</cfif>
		</cfif>
		<cfif MAX_ERROR_UNITS is not "ignore">
			<cfif len(MAX_ERROR_UNITS) gt 0>
				 MAX_ERROR_UNITS='#MAX_ERROR_UNITS#' and
			<cfelse>
				 MAX_ERROR_UNITS is null and
			</cfif>
		</cfif>
		...
		<cfif LOCALITY_REMARKS is not "ignore">
			<cfif len(LOCALITY_REMARKS) gt 0>
				 upper(LOCALITY_REMARKS) like <cfqueryparam value="#ucase(LOCALITY_REMARKS)#" CFSQLType="CF_SQL_VARCHAR"> and
			<cfelse>
				 LOCALITY_REMARKS is null and
			</cfif>
		</cfif>
		<cfif GEOREFERENCE_SOURCE is not "ignore">
			<cfif len(GEOREFERENCE_SOURCE) gt 0>
				 upper(GEOREFERENCE_SOURCE) like <cfqueryparam value="#ucase(GEOREFERENCE_SOURCE)#" CFSQLType="CF_SQL_VARCHAR"> and
			<cfelse>
				 GEOREFERENCE_SOURCE is null and
			</cfif>
		</cfif>
		<cfif GEOREFERENCE_PROTOCOL is not "ignore">
			<cfif len(GEOREFERENCE_PROTOCOL) gt 0>
				 GEOREFERENCE_PROTOCOL='#GEOREFERENCE_PROTOCOL#' and
			<cfelse>
				 GEOREFERENCE_PROTOCOL is null and
			</cfif>
		</cfif>
		<cfif LOCALITY_NAME is not "ignore">
			<cfif len(LOCALITY_NAME) gt 0>
				 LOCALITY_NAME='#LOCALITY_NAME#' and
			<cfelse>
		...
		<cfif localityAttrs is not "ignore">
			<cfif len(localityAttrs) gt 0>
				 getLocalityAttributesAsJson(locality_id)::varchar='#localityAttrs#' and
			<cfelse>
				  getLocalityAttributesAsJson(locality_id)::varchar is null and
			</cfif>
		<cfelse>
		...
		<cfif locality_footprint is not "ignore">
			<cfif len(locality_footprint) gt 0>
				 ST_AsText(locality_footprint)='#locality_footprint#' and
			<cfelse>
				  locality_footprint is null and
			</cfif>
		<cfelse>
			<!--- just to keep the SQL valid --->
			 1=1 ">
	...
	<cfif action is "delete">
		<cftransaction>
			...
			<cfquery name="cleardupsBL" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
				update bulkloader set locality_id=#locality_id#::varchar where locality_id::int in (#deleteLocalityID#)
			</cfquery>
			<cfquery name="deleteg" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
                delete from locality_attributes where locality_id in (#deleteLocalityID#)
            </cfquery>
