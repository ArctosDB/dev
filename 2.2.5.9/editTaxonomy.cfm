...
<cfif action is "cloneClassificationNewName">
	...
	<cfoutput>
		...
		<form name="x" id='x' method="post" action="editTaxonomy.cfm">
            ...
            <div class="importantNotification">You will need to edit the classifiation after creation; species and similar will carry over from the source and will be incorrect,</div>
            <p>
...
<cfif action is "cloneClassificationNewName_insert">
    <cfoutput>
        ...
        <cfif not isdefined("forceOverride") or forceOverride is not true>
            ...
            <cfset result=tc.validateName(taxon_name=newName,name_type=taxon_name_type)>
            <cfif result.consensus is not "might_be_valid">
                <h3 class="caution">
                    ...
                    <p>
                        ...
                        <a href="editTaxonomy.cfm?action=cloneClassificationNewName_insert&newName=#newName#&taxon_name_id=#taxon_name_id#&CLASSIFICATION_ID=#URLEncodedFormat(CLASSIFICATION_ID)#&SOURCE=#SOURCE#&forceOverride=true" class="godo">Force-create this Taxon Name
                        </a>
                    </p>
                <cfabort>
            </cfif>
...
<!----------------------Seed Classification with Clone--------------------------------------->
		...
		<cfif isdefined("forceOverride") and forceOverride is true>
			<cfquery name="a" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
				select get_address(#session.myAgentID#, 'email') a
			</cfquery>
			<cfinvoke component="/component/functions" method="deliver_notification">
				<cfinvokeargument name="usernames" value="#usrInfo.invited_by_username#">
				<cfinvokeargument name="subject" value="Account Created: #session.username#">
				<cfinvokeargument name="message" value="#msg#">
				<cfinvokeargument name="email_immediate" value="#usrInfo.invited_by_email#">
			</cfinvoke>
		</cfif>
	...
	<cfoutput>
		...
		<cfquery name="d" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			...
			where
				...
				taxon_name.taxon_name_id=<cfqueryparam value = "#TAXON_NAME_ID#" CFSQLType="cf_sql_int"> and
				(
		...
		<cfquery name="cttaxon_term" datasource="cf_codetables" cachedwithin="#createtimespan(0,0,60,0)#">
			select taxon_term,is_classification,relative_position from cttaxon_term where taxon_term not in ('display_name','scientific_name')
		</cfquery>
		...
		<ul>
			...
			<li>
				Terms of type "display_name" and "scientific_name" are automatically created by Arctos when possible. Using these terms here may result in them being overwritten by automation. <strong>nomenclatural_code</strong> and <strong>author_text</strong> are used to generate display_name and should be provided when relevant.
			</li>
...
<cfif action is "yesReally_forceDeleteNonLocal">
	<cfoutput>
		<cfquery name="insRow" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			delete from taxon_term where source not in (select source from cttaxonomy_source) and taxon_name_id=<cfqueryparam value = "#TAXON_NAME_ID#" CFSQLType="cf_sql_int">
		</cfquery>
