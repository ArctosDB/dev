<cfscript>component{
	function initUserSession (string username="",string pwd=""){
		...
		session.block_suggest=0;
		session.myAgentId='';
		session.taxaPickPrefs='';
		session.taxaPickSource='';
		session.bigsearchbox='';
		session.srmapclass='';
		session.sdmapclass='';
		session.resultsbrowseprefs='';
		session.customoidoper='';
		session.locsrchprefs='';
		...
		if (len(username) is 0 and len(pwd) is 0){
	    	...
	    	if (Argon2CheckHash(pwd, userdata.password, 'argon2id')){
	    	...
	    	if (isOperator.recordcount gt 0){ // BEGIN: they are an operator, set session
				...
				session.result_sort=userdata.result_sort;
				session.fancyCOID=userdata.fancyCOID;
				...
				session.block_suggest=userdata.block_suggest;
				session.taxaPickPrefs=userdata.taxaPickPrefs;
				session.taxaPickSource=userdata.taxaPickSource;
				session.bigsearchbox=userdata.bigsearchbox;
				session.srmapclass=userdata.srmapclass;
				session.sdmapclass=userdata.sdmapclass;
				session.resultsbrowseprefs=userdata.resultsbrowseprefs;
				session.customoidoper=userdata.customoidoper;
				session.locsrchprefs=userdata.locsrchprefs;
	...
	function getIpAddress(){
		...
		variables.localIpAddrs='129.114.52.171,129.114.52.14,129.114.52.13,129.114.60.239,129.114.52.18,127.0.0.1,129.114.60.10';
