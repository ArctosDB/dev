		...
		<cfoutput>
			...
			<cfif isdefined("collection_id") and len(collection_id) gt 0>
				...
				<cfif collnOper is "usedOnlyBy">
					...
					<cfset tbls=tbls & " left outer join (
						select collection.collection_id,locality.locality_id from collection
						inner join cataloged_item on collection.collection_id=cataloged_item.collection_id
						inner join specimen_event on cataloged_item.collection_object_id=specimen_event.collection_object_id
						inner join collecting_event on specimen_event.collecting_event_id=collecting_event.collecting_event_id
						inner join locality on collecting_event.locality_id=locality.locality_id
						) anticolln_join on locality.locality_id=anticolln_join.locality_id ">

					<cfset thisrow={}>
					<cfset thisrow.l="false">
					<cfset thisrow.d="cf_sql_int">
					<cfset thisrow.o="!=">
					<cfset thisrow.t="anticolln_join.collection_id">
					<cfset thisrow.v=collection_id>
					<cfset arrayappend(qp,thisrow)>
					...
					<cfset thisrow.l="false">
					<cfset thisrow.d="cf_sql_int">
					<cfset thisrow.o="isnull">
					<cfset thisrow.t="anticolln_join.locality_id">
					<cfset thisrow.v="">
					<cfset arrayappend(qp,thisrow)>
			...
			<cfif isdefined("coord_serv_diff") and len(coord_serv_diff) gt 0>
				...
				<cfif not listfind("=,<,>",left(coord_serv_diff,1)) or not isnumeric(mid(coord_serv_diff,2,999))>
					<cfset r.status='fail'>
					<cfset r.message="coord_serv_diff format is (=,<, or >) followed by an integer.">
					<cfheader statuscode="500">
					<cfreturn r>
					<cfabort>
				</cfif>
				<cfset opr=left(coord_serv_diff,1)>
				<cfset ivl=right(coord_serv_diff,len(coord_serv_diff)-1)>

				<cfset thisrow={}>
				<cfset thisrow.l="false">
				<cfset thisrow.d="cf_sql_int">
				<cfset thisrow.t="checkLocalityError(locality.locality_id)">
				<cfset thisrow.o="#opr#">
				<cfset thisrow.v=ivl>
				<cfset arrayappend(qp,thisrow)>
		...
		<cfif isdefined("collecting_event_name") and len(collecting_event_name) gt 0>
			...
			<cfif isdefined("hasGeoWKT") and len(hasGeoWKT) gt 0>
				<cfif hasGeoWKT is 1>
					...
					<cfset thisrow.l="false">
					<cfset thisrow.d="notnull">
					<cfset thisrow.t="geog_auth_rec.spatial_footprint">
					<cfset thisrow.o="">
					<cfset thisrow.v="">
					<cfset arrayappend(qp,thisrow)>
			...
			<cfif isdefined("valid_catalog_term_fg") and len(valid_catalog_term_fg) gt 0>
				<cfset thisrow={}>
				<cfset thisrow.l="false">
				<cfset thisrow.d="smallint">
				<cfset thisrow.t="geog_auth_rec.valid_catalog_term_fg">
				<cfset thisrow.o="=">
				<cfset thisrow.v=valid_catalog_term_fg>
				<cfset arrayappend(qp,thisrow)>
			</cfif>
			...
			<cfif isdefined("continent_ocean") and len(continent_ocean) gt 0>
				...
				<cfelseif left(continent_ocean,1) is '='>
					...
					<cfset thisrow.t="geog_auth_rec.continent_ocean">
					<cfset thisrow.o="ilike">
					<cfset thisrow.v="#right(continent_ocean,len(continent_ocean)-1)#">
					<cfset arrayappend(qp,thisrow)>
				<cfelse>
					<cfset thisrow={}>
					<cfset thisrow.l="false">
					...
					<cfset thisrow.t="geog_auth_rec.continent_ocean">
					<cfset thisrow.o="ilike">
					<cfset thisrow.v='%#continent_ocean#%'>
					<cfset arrayappend(qp,thisrow)>
				</cfif>
			</cfif>
			...
			<cfquery name="localityResultsCNT" timeout="10" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
				select
