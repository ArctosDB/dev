...
<cfif isdefined("d")>
	...
	<cfloop query="d">
		<cfif not listcontains(usedAgentIdList,id1) and not listcontains(usedAgentIdList,id2)>
			...
			<tr>
				<td valign="top">
					<cfquery name="one" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
						select
							agent_name,
							agent_name_type,
							agent_type,
				...
				<td valign="top">
					<cfquery name="two" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
						select
							agent_name,
							agent_name_type,
							agent_type,
