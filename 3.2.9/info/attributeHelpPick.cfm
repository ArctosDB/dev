...
<cfquery name="ctCodes" datasource="cf_codetables" cachedwithin="#createtimespan(0,0,60,0)#">
	select
	 ...
	 from ctattribute_code_tables
		...
		<cfquery name="atts" datasource="cf_codetables" cachedwithin="#createtimespan(0,0,60,0)#">
		...
		<cfif isdefined("isValCt.value_code_table") and len(#isValCt.value_code_table#) gt 0>
			...
			<cfquery name="getCols" datasource="uam_god">
				select column_name from information_schema.columns  where table_name='#lcase(isValCt.value_code_table)#'
		...
		<cfif isdefined("isUnitCt.units_code_table") and len(#isUnitCt.units_code_table#) gt 0>
			...
			<!---- there's a code table --->
			<!---- get the data --->
			<cfquery name="unitCT" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
				select * from #isUnitCt.units_code_table#
			</cfquery>
			<!---- get column names --->
			<cfquery name="getCols" datasource="uam_god">
				select column_name from information_schema.columns  where table_name='#lcase(isUnitCt.units_code_table)#'
				and column_name <> 'description'
			</cfquery>
				<cfset collCode = "">
				<cfset columnName = "">
