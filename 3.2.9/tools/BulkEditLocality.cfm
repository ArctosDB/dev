...
drop table cf_temp_edit_locality;

create table cf_temp_edit_locality (
	key serial not null,
...
grant select,insert,update,delete on cf_temp_edit_locality to manage_collection;

grant select, usage on cf_temp_edit_locality_key_seq to public;


grant select, insert, update, delete on cf_temp_edit_locality to manage_records;
...
insert into cf_component_loader (
	...
	'Bulk Edit Locality',
	99,
	'autoload_edit_locality',
	'/tools/BulkEditLocality.cfm',
	'cf_temp_edit_locality',
	10,
	null
);
...
<cfoutput>
	...
	<cfset title="Bulk Edit Locality">
	<cfset thisFormFile="BulkEditLocality.cfm">
	<cfset thisFormName="Bulk Edit Locality Tool">
	<cfset thisFormPurpose='This tool allows Arctos operators with <a href="/Admin/user_roles.cfm">role(s)</a> (#required_roles#) to create and review locality edits.'>
	<cfset thisTable="cf_temp_edit_locality">
	<cfset thisTemplateName="BulkEditLocality.csv">
	<cfset thisDownloadName="BulkEditLocalityData.csv">
	<cfset templateHeader="locality_name,higher_geog,spec_locality,dec_lat,dec_long,minimum_elevation,maximum_elevation,orig_elev_units,min_depth,max_depth,depth_units,max_error_distance,max_error_units,datum,locality_remarks,georeference_protocol,utm_zone,utm_ew,utm_ns,use_geography_polygon">
</cfoutput>
...
<cfsavecontent variable = "defDocTableGuts">
	<tr>
		...
		<td>-</td>
		<td>
			This application replaces all data for specified localities. Locality attributes are not changed in any way. It is very recommended to use DataServices/get Locality Users before using this tool.
		</td>
	</tr>
	<tr>
		<td>locality_name</td>
		<td>yes</td>
		<td>
			Locality Name of an existing locality is required. Find/Locality/ManageNames may be useful in manipulating these. If locality_name is not found, nothing will happen.
		</td>
	</tr>
	...
	<tr>
		<td>use_geography_polygon</td>
		<td>yes</td>
		<td>
			Must be one of two values:
			<ul>
				<li>
					true: Ignore coordinates, use the geography record's polygon.  and georeference_protocol are required, all other spatial fields are ignored
				</li>
				<li>
					false: Use coordinate information if provided.
				</li>
			</ul>
		</td>
	</tr>
