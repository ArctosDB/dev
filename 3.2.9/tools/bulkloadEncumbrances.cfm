<!-------What this is and how to use it-------------
	...
	create table cf_temp_obj_encumbrance (
		key serial not null,
	...
	grant select, insert, update, delete on cf_temp_obj_encumbrance to manage_collection;
	grant select, usage on cf_temp_obj_encumbrance_key_seq to public;
...
<cfset ComponentLoaderVersion="1.1">




<!---------------Settings BEGIN::this section will need customized for individual loaders ----------------------------->
<cfset title="Bulkload Encumbrances">
<cfset thisFormFile="bulkloadEncumbrances.cfm">
<cfset thisFormName="Bulkload Encumbrances">
<cfset thisFormPurpose='This tool allows Arctos operators with the <a href="/Admin/user_roles.cfm">Manage Collection</a> role to create and review encumbrances.'>
<cfset thisTable="cf_temp_obj_encumbrance">
<cfset thisTemplateName="bulkloadEncumbrance.csv">
<cfset thisDownloadName="bulkloadEncumbranceData.csv">
<cfset templateHeader="guid,encumbrance_id">
...
<cfsavecontent variable = "defDocTableGuts">
	<tr>
		...
		<td>encumbrance_id</td>
		<td>yes</td>
		<td>
			Encumbrance ID: Edit encumbrance and get this from the URL
		</td>
	</tr>
	<tr>
		<td>status</td>
...
<cfif action is "table">
	...
	<cfoutput>
		...
		<cfelse>
			<cfset thisStatus=replace(status,'"','&quot;','all')>
		</cfif>
