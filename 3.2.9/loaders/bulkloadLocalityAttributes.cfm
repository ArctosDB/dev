...
delete from cf_component_loader where data_table='cf_temp_locality_attributes';
...
insert into cf_component_loader (
	...
	'Bulkload Locality Attributes', -- title
	'Load attributes to existing named localities. (There is a mechanism to temporarily name localities in bulk.)', -- short description of the purpose
	1, -- run_order is a nonunique integer; 1 is as good at anything
	'autoload_locality_attribute', -- this will resolve to /ScheduledTasks/componentLoaderComponents/{what you enter here}.cfm
	'/loaders/bulkloadLocalityAttributes.cfm', -- this should be /loaders/something.cfm - migration in process
	'cf_temp_locality_attributes', -- the table used by the loader
	10, -- 10 is a nice number; this should NOT tax the server, and must complete in under a minute
	null,
	'manage_collection,manage_locality', -- list of roles required to set autoload, delete, etc.
...
<cfoutput>
	...
	---->
	<cfset templateHeader="locality_name,attribute_type,attribute_value,attribute_units,attribute_determiner,attribute_remark,determination_method,determined_date">
</cfoutput>
