...
update cf_component_loader set rec_per_run=10,remark=concat_ws(' | ',remark, '25 timing out, going 10, can be reevaluated after v3.2') where loader_template='autoload_otherid';
...
drop table cf_temp_oids;
	...
	create table cf_temp_oids (
		...
		guid_prefix varchar,
		uuid varchar,
		existing_other_id_type varchar,
		existing_other_id_number varchar,
	...
	grant select, insert, update, delete on cf_temp_oids to manage_records;
	grant select, usage on cf_temp_oids_key_seq to public;

	grant insert,select on cf_temp_oids to data_entry;



-- https://github.com/ArctosDB/arctos/issues/4604
alter table cf_temp_oids add issued_by varchar(255);
alter table cf_temp_oids add remarks varchar(4000);


update cf_component_loader set purpose='Add identifier and/or relationship to a record. (Agent must already exist or see Identifier IssuedBy Bulkloader.)' where loader_template='autoload_otherid';
...
<cfoutput>
	...
	<cfset templateHeader="guid,uuid,guid_prefix,existing_other_id_type,existing_other_id_number,new_other_id_type,new_other_id_number,new_other_id_references,issued_by,remarks">
...
<cfsavecontent variable = "defDocTableGuts">
	<tr>
			...
			<td>conditionally</td>
			<td>
				(GUID), (UUID), or (guid_prefix,existing_other_id_type,existing_other_id_number) may be used to select a catalog record to which an identifier will be added.
				GUID may be given as "DWC Triplet" ("UAM:Mamm:12") or GUID ("https://arctos.database.museum/guid/UAM:Mamm:12")
			</td>
		</tr>
		<tr>
			<td>uuid</td>
		...
		<tr>
			...
			<td>yes</td>
			<td>
				<a href="/info/ctDocumentation.cfm?table=ctid_references">ctid_references</a>
			</td>
		</tr>
		<tr>
			<td>issued_by</td>
			<td>no</td>
			<td>
				Agent who issued the identifier
			</td>
		</tr>
		<tr>
			<td>remarks</td>
			<td>no</td>
			<td>
				Remarks concerning the identifier
			</td>
		</tr>
		<tr>
			<td>status</td>
