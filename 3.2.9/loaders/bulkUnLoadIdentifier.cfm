...
drop table cf_temp_unload_identifiers;


	create table cf_temp_unload_identifiers (
		key serial not null,
		guid varchar,
		other_id_type varchar,
		other_id_number varchar,
		other_id_references varchar,
		other_id_issued_by varchar,
		username varchar not null default session_user,
		last_ts timestamp default current_timestamp,
		status varchar
	);

	grant select, insert, update, delete on cf_temp_unload_identifiers to manage_records;
	grant select, usage on cf_temp_unload_identifiers_key_seq to public;
...
insert into cf_component_loader (
	...
	'Identifier Unloader', -- title
	'remove record identifiers', -- short description of the purpose
	1, -- run_order is a nonunique integer; 1 is as good at anything
	'autoload_unload_identifiers', -- this will resolve to /ScheduledTasks/componentLoaderComponents/{what you enter here}.cfm
	'/loaders/bulkUnLoadIdentifier.cfm', -- this should be /loaders/something.cfm - migration in process
	'cf_temp_unload_identifiers', -- the table used by the loader
...
<cfoutput>
	...
	<cfset templateHeader="guid,other_id_type,other_id_number,other_id_references,other_id_issued_by">
...
<cfsavecontent variable = "defDocTableGuts">
	<tr>
		...
		<td>yes</td>
		<td>
			Record identifier: The record from which to remove identifiers
		</td>
	</tr>
	<tr>
		<td>other_id_type</td>
		<td>no</td>
		<td>
			Type of identifier. One of:
			<ol>
				<li>
					Value from <a href="/info/ctDocumentation.cfm?table=ctcoll_other_id_type">ctcoll_other_id_type</a>, or
				</li>
				<li>blank/empty/null to ignore (match anything)</li>
			</ol>
		</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td>
			...
			<ol>
				<li>
					Value from <a href="/info/ctDocumentation.cfm?table=ctid_references">ctid_references</a>, or
				</li>
				<li>blank/empty/null to ignore (match anything)</li>
			</ol>
		</td>
	...
	<tr>
		...
		<td>no</td>
		<td>
			Agent issueing.  One of:
			<ol>
				<li>
					Agent name, or
				</li>
				<li>blank/empty/null to ignore (match anything), or</li>
				<li>"NULL" (no quotes, case sensitive) to match identifiers without issued_by</li>
			</ol>
		</td>
	</tr>
	<tr>
