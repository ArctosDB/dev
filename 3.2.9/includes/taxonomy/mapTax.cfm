...
<cfoutput>
	...
	<cfif not fileexists("#internalPath##fn#")>
		<cfquery name="d" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#" cachedwithin="#createtimespan(0,0,60,0)#">
		 	...
		 	where
				dec_lat is not null and
		 		...
		 		<cfif method is "exact">
					scientific_name = <cfqueryparam value = "#scientific_name#" CFSQLType="CF_SQL_VARCHAR">
				<cfelse>
					scientific_name ilike <cfqueryparam value = "#scientific_name#%" CFSQLType="CF_SQL_VARCHAR">
				</cfif>
		 	group by
		 		locality_id,
		 	 	scientific_name,
		...
		<cfscript>
			...
			kmlfile='<?xml version="1.0" encoding="UTF-8"?>' & chr(10) &
		...
		<cfloop query="d">
			<cfscript>
				kml=chr(9) & chr(9) & '<Placemark>' & chr(10) &
			        ...
			        chr(9) & chr(9) & chr(9) & chr(9) & chr(9) & '<a href="#application.serverRootUrl#/search.cfm?locality_id=#locality_id#&scientific_name=#scientific_name#">Arctos Specimen Records</a>' & chr(10) &
			...
			<cfset kmlfile=kmlfile & chr(10) & kml>
		</cfloop>
		<cfscript>
		...
		<cfset kmlfile=kmlfile & chr(10) & kml>
		<cffile action = "write" file = "#internalPath##fn#" output = "#kmlfile#" addNewLine = "no" mode = "644">
	</cfif>
