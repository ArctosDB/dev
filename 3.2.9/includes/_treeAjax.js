...
function expandNode (id) {
	var sb=document.getElementById("sort_by").value;
	post(1);
	jQuery.getJSON("/component/container.cfc",
		{
	...
	if (ok == '-1') {
		...
		for (i = 0; i < r.ROWCOUNT; i++) { 
			...
			if (nE == 0) {
				...
				var LAST_DATE = result.LAST_DATE[i];
				var CONTAINER_REMARKS = result.CONTAINER_REMARKS[i];
				var LABEL = result.LABEL[i];
				var BARCODE = result.BARCODE[i];
				//				thislbl='[ ' + BARCODE + ' ] ' + LABEL + ' (' + CONTAINER_TYPE + ')';		

				var thisIns = 'newTree.insertNewChild("' + PARENT_CONTAINER_ID + '","' + CONTAINER_ID + '","' + '[ ' + BARCODE + ' ] ' + LABEL + ' (' + CONTAINER_TYPE + ')",0,0,0,0,"",1)';
				eval(thisIns);
				didSomething = 'yep';
			}						
		 }
...
function getContDetails_success (result) {
	...
	var idateH = document.getElementById('dis_last_date');
