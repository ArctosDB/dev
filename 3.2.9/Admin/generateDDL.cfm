...
<cfsavecontent variable="sc">
-- generated by /Admin/generateDDL.cfm?action=bulk_check_freetext
...
<cfsavecontent variable="sc">
-- BEGIN: Code generated by /Admin/generateDDL.cfm?action=bulk_check_one
...
DECLARE
	...
	r_collection_cde varchar;
	r_institution_acronym varchar;
...
BEGIN
	...
	select  count(distinct(agent_id)) into STRICT numRecs from agent_name where agent_name_type='login' AND agent_name = rec.enteredby;
	if (numRecs != 1) then
		thisError=concat('ENTEREDBY [ ',rec.ENTEREDBY,' ] matches ',numRecs,' login agents');
		allError=concat_ws('; ',allError,thisError);
	END IF;
	select  collection_cde into STRICT r_collection_cde from collection where guid_prefix = rec.guid_prefix;
	...
	if rec.identification_#i# is not null then
		...
		if coalesce(rec.identification_#i#_order,'') != '' then
			thisError = 'identification_order is required';
		end if;

		BEGIN
			numRecs=rec.identification_#i#_order::int;
		EXCEPTION WHEN others THEN
			numRecs=-1;
		END;
		if numRecs < 0 or numRecs > 10 then
			thisError := 'identification_order must be a number between 0 and 10';
			allError=concat_ws('; ',allError,thisError);
		end if;
	...
	if rec.record_event_type is not null then
		...
		IF ISDATE(rec.record_event_determined_date,1) != 1 OR rec.record_event_determined_date is null THEN
			...
			allError=concat_ws('; ',allError,thisError);
		END IF;
		IF coalesce(rec.record_event_verificationstatus,'NULL') not in ('accepted','unaccepted','unverified') THEN
			thisError=concat('record_event_verificationstatus (' , coalesce(rec.record_event_verificationstatus,'NULL'),') is not one of (accepted,unaccepted,unverified)');
			allError=concat_ws('; ',allError,thisError);
		END IF;
	...
	if rec.part_#i#_name is not null THEN
		...
		if rec.part_#i#_attribute_type_#a# is not null and rec.part_#i#_attribute_value_#a# is not null then
			...
			if COALESCE(bool_val, FALSE)=false then
				thisError :=  'part_#i#attribute_#a# is invalid.';
				allError=concat_ws('; ',allError,thisError);
			end if;
...
SECURITY DEFINER
STABLE;
-- END: Code generated by /Admin/generateDDL.cfm?action=bulk_check_one
...
<cfsavecontent variable="sc">
-- BEGIN: Code generated by /Admin/generateDDL.cfm?action=check_and_load
...
BEGIN
	...
	FOR rec IN (SELECT * FROM bulkloader where status in ('autoload_core','autoload_extras')) LOOP
		...
		begin
			...
			select count(distinct(agent_id)) into num from agent_name where agent_name = rec.ENTEREDBY  AND agent_name_type = 'login';
			if num != 1 then
				RAISE EXCEPTION '%','Bad enteredby (use login)';
			else
				select distinct(agent_id) into l_entered_person_id from agent_name where agent_name = rec.ENTEREDBY AND agent_name_type = 'login';
			end if;
			...
			<cfloop from ="1" to="#bulk_otherid_count#" index="i">
				if rec.identifier_#i#_type is not null and rec.identifier_#i#_value is not null THEN
					select split_other_id(rec.identifier_#i#_value) into v_joid;
...
SECURITY DEFINER
;
-- end code generated by /Admin/generateDDL.cfm?action=check_and_load
</cfsavecontent>
