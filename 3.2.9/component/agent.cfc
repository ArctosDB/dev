...
<cffunction name="checkAgentJson" access="public" returnformat="json">
    ...
    <cfargument name="name_1" required="false" type="string" default="">
    <cfargument name="name_type_1" required="false" type="string" default="">
    <cfargument name="name_2" required="false" type="string" default="">
    <cfargument name="name_type_2" required="false" type="string" default="">
    <cfargument name="name_3" required="false" type="string" default="">
    <cfargument name="name_type_3" required="false" type="string" default="">
    <cfargument name="name_4" required="false" type="string" default="">
    <cfargument name="name_type_4" required="false" type="string" default="">
    <cfargument name="name_5" required="false" type="string" default="">
    <cfargument name="name_type_5" required="false" type="string" default="">
    <cfargument name="name_6" required="false" type="string" default="">
    <cfargument name="name_type_6" required="false" type="string" default="">
    <cfargument name="address_1" required="false" type="string" default="">
    <cfargument name="address_type_1" required="false" type="string" default="">
    <cfargument name="address_2" required="false" type="string" default="">
    <cfargument name="address_type_2" required="false" type="string" default="">
    <cfargument name="address_3" required="false" type="string" default="">
    <cfargument name="address_type_3" required="false" type="string" default="">
    <cfargument name="address_4" required="false" type="string" default="">
    <cfargument name="address_type_4" required="false" type="string" default="">
    <cfargument name="address_5" required="false" type="string" default="">
    <cfargument name="address_type_5" required="false" type="string" default="">
    <cfargument name="address_6" required="false" type="string" default="">
    <cfargument name="address_type_6" required="false" type="string" default="">
    ...
    <cfoutput>
        ...
        <!--- regex preferred name match ---->

        <cfset qname=trim(rereplace(preferred_name,regexStripJunk,"","all"))>
        <!----
            https://github.com/ArctosDB/arctos/issues/6757
            this needs to match exact, minus "stripjunk"
            substrings fire on any "division"
            change param from  %#qname#%" to #qname#
        ---->
        <cfquery name="x" datasource="uam_god">
            select
                agent.agent_id,
            ...
            from
                agent
            where
                trim(regexp_replace(agent.preferred_agent_name,'#regexStripJunk#', '','g')) ilike <cfqueryparam value = "#qname#" CFSQLType="CF_SQL_VARCHAR">
                <cfif len(exclude_agent_id)>
                    and agent.agent_id!=<cfqueryparam value = "#exclude_agent_id#" CFSQLType="cf_sql_int">
                </cfif>
        </cfquery>
        ...
        <cfif x.recordcount gt 0>
            <cfloop query="x">
                <cfset queryaddrow(q,{
                    severity='fatal',
                    message='manipulated preferred name match: #x.preferred_agent_name#',
                    link='#application.serverRootURL#/agents.cfm?agent_id=#x.agent_id#'
                })>
            </cfloop>
        </cfif>
        ...
        <cfloop from="1" to="6" index="i">
            ...
            <cfif len(thisName) gt 0>
                <cfset qname=trim(rereplace(thisName,regexStripJunk,"","all"))>
                 <cfquery name="x" datasource="uam_god">
                    select
                        agent.agent_id,
                        agent.preferred_agent_name
                    ...
                    where
                        regexp_replace(agent_name.agent_name,'#regexStripJunk#', '','g') ilike <cfqueryparam value = "%#qname#%" CFSQLType="CF_SQL_VARCHAR">
                        <cfif len(exclude_agent_id)>
                            and agent.agent_id!=<cfqueryparam value = "#exclude_agent_id#" CFSQLType="cf_sql_int">
                        </cfif>
                </cfquery>
                ...
                <cfif x.recordcount gt 0>
                    <cfloop query="x">
                        <cfset queryaddrow(q,{
                            severity='advisory',
                            message='Agent Name Match: #x.preferred_agent_name#',
                            link='#application.serverRootURL#/agents.cfm?agent_id=#x.agent_id#'
                        })>
                    </cfloop>
                </cfif>
        ...
        <cfloop from="1" to="6" index="i">
            <cfset thisAddress=evaluate("address_" & i)>
            <cfset thisAddressType=evaluate("address_type_" & i)>
            <cfif len(thisAddress) gt 0>
                <cfquery name="ck_address" datasource="uam_god" cachedwithin="#createtimespan(0,0,60,0)#">
                    select isValidAddress(
                        <cfqueryparam value="#thisAddressType#" cfsqltype="cf_sql_varchar">,
                        <cfqueryparam value="#thisAddress#" cfsqltype="cf_sql_varchar">
                    ) as ack
                </cfquery>
                <cfif ck_address.ack neq 'pass'>
                    <cfset queryaddrow(q,{
                        severity='fatal',
                        message='Invalid address: #thisAddressType#==#thisAddress#',
                        link=''
                    })>
                </cfif>
                <cfset qaddr=trim(rereplace(thisAddress,regexStripJunk,"","all"))> 
                <cfquery name="x" datasource="uam_god">
                    select
                        agent.agent_id,
                        agent.preferred_agent_name
                    ...
                    where
                        regexp_replace(address.address,'#regexStripJunk#', '','g') ilike <cfqueryparam value = "%#qaddr#%" CFSQLType="CF_SQL_VARCHAR">
                        <cfif len(exclude_agent_id)>
                            and agent.agent_id!=<cfqueryparam value = "#exclude_agent_id#" CFSQLType="cf_sql_int">
                        </cfif>
                </cfquery>
                ...
                <cfif x.recordcount gt 0>
                    <cfloop query="x">
                        <cfset queryaddrow(q,{
                            severity='advisory',
                            message='Address Match: #x.preferred_agent_name#',
                            link='#application.serverRootURL#/agents.cfm?agent_id=#x.agent_id#'
                        })>
                    </cfloop>
                </cfif>
        ...
        <cfif agent_type is "person">
            ...
            <cfif len(last_name) lt 2>
                <cfset queryaddrow(q,{
                    severity='fatal',
                    message='Person-agents must have a last name.',
                    link=''
                })>
            </cfif>
        <cfelse><!--- not a person --->
            <cfif len(last_name) gt 0 or len(middle_name) gt 0 or len(first_name) gt 0 > 
                <cfset queryaddrow(q,{
                    severity='fatal',
                    message='Non-person agents may not have first, middle, or last name.',
                    link=''
                })>
            </cfif>
