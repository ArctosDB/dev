	...
	<cfif len(taxon_name) gt 0 or len(taxon_term) gt 0 or len(common_name) gt 0 or len(source) gt 0 or len(term_type) gt 0 or len(taxon_name_type) gt 0>
      ...
      <cfset tabls="taxon_name">
			...
			<cfquery name="d" dbtype="query">
				select scientific_name,taxon_name_id from d_raw group by scientific_name,taxon_name_id order by scientific_name 
			</cfquery>
            ...
            ------->
			<cfif d_raw.recordcount is 2000>
				<span class="warningOverflow">Results truncated; refine search for more.</span>
			</cfif>
            <br><br>
...
<cfif isdefined("name") and len(name) gt 0>
	...
	<cfquery name="d" datasource="uam_god">
		...
		from
			taxon_name
		where
			upper(scientific_name)=<cfqueryparam value="#ucase(name)#" cfsqltype="cf_sql_varchar">
	</cfquery>
	...
	<span class="annotateSpace">
		<cfquery name="existingAnnotations" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			...
			where taxon_name_id = <cfqueryparam value="#taxon_name_id.taxon_name_id#" cfsqltype="cf_sql_int">
		</cfquery>
        ...
        <div class="left_indent">
                ...
                <cfquery name="related" datasource="uam_god">
                    ...
                    where
                        ...
                        taxon_relations.taxon_name_id=<cfqueryparam value="#taxon_name_id.taxon_name_id#" cfsqltype="cf_sql_int">
                </cfquery>
                ...
                <cfquery name="wdi" datasource="uam_god">
                    select getPreferredAgentName(created_by_agent_id) cb, to_char(created_date,'YYYY-MM-DD') cd from taxon_name where taxon_name_id=<cfqueryparam value="#taxon_name_id.taxon_name_id#" cfsqltype="cf_sql_int">
                </cfquery>
	...
	<cfset isOperator=false>
	<cfif isdefined("session.roles") and listfindnocase(session.roles,"manage_taxonomy")>
		<cfset isOperator=true>
		<script>
			jQuery(document).ready(function(){
				$.ajax({
    ...
    <cfquery name="sidas" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
		select count(*) c from identification_taxonomy where taxon_name_id=<cfqueryparam value="#taxon_name_id.taxon_name_id#" cfsqltype="cf_sql_int">
	</cfquery>
	<cfif sidas.c gt 0>
        ...
        &nbsp;
        <a href="/search.cfm?taxon_name_id=#taxon_name_id.taxon_name_id#&identification_order=>0" class="schlikeBtn" target="_blank">Used in Accepted Identifications</a>
        &nbsp;
...
<!-------- Maps -------->
    ...
    <div class="left_indent" id="bmrmp">
        <a href="/bnhmMaps/bnhmMapData.cfm&scientific_name=#scientific_name.scientific_name#" class="external" target="_blank">View in BerkeleyMapper</a> 
        <!---- this does nothing that I can see, removing ?showRangeMaps=true   + RangeMaps <span class="caution">Note:</span> This is only available for Amphibia, Aves and Mammalia.---->
    </div>
    <br>
...
<!-------- Related Taxon Names -------->
	<cfquery name="revrelated" datasource="uam_god">
		...
		where
			...
			taxon_relations.related_taxon_name_id=<cfqueryparam value="#taxon_name_id.taxon_name_id#" cfsqltype="cf_sql_int">
	</cfquery>
...
<!--- name concepts --->
	<cfquery name="concept" datasource="uam_god">
		  select
		    ...
		    where
		      ...
		      taxon_concept.taxon_name_id=<cfqueryparam value="#taxon_name_id.taxon_name_id#" cfsqltype="cf_sql_int">
	</cfquery>
	<cfquery name="related_concept" datasource="uam_god">
      ...
      taxon_name
    where
      this_name.taxon_name_id=<cfqueryparam value="#taxon_name_id.taxon_name_id#" cfsqltype="cf_sql_int"> and
				...
				<li class="concept_li" id="concept_#taxon_concept_id#">
					...
					<cfquery name="tcrel" datasource="uam_god">
						...
						where
							...
							from_taxon_concept_id=<cfqueryparam value="#taxon_concept_id#" cfsqltype="cf_sql_int">
					</cfquery>
...
<!-------- Associated Publications -------->
	<cfquery name="tax_pub" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
		...
		where
			...
			taxonomy_publication.taxon_name_id=<cfqueryparam value="#taxon_name_id.taxon_name_id#" cfsqltype="cf_sql_int">
	</cfquery>
...
<!-------- Classifications -------->
	...
	<cfquery name="trms" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
	    ...
	    from
	      taxon_term where taxon_name_id=<cfqueryparam value="#d.taxon_name_id#" cfsqltype="cf_sql_int">
	</cfquery>
	...
	<cfloop query="sources">
		<div class="sourceDiv">
			...
			<cfquery name="source_classification" dbtype="query">
				select classification_id from trms where source=<cfqueryparam value="#source#" cfsqltype="cf_sql_varchar"> group by classification_id
			</cfquery>
			<cfset canEditThis=false>
			<cfif isOperator is true>
				<cfquery name="qry_canEditThis" dbtype="query">
					select count(*) c from cttaxonomy_source where source=<cfqueryparam value="#sources.source#" cfsqltype="cf_sql_varchar"> and  
				...
				<cfif qry_canEditThis.c gt 0>
					<cfset canEditThis=true>
				</cfif>
			</cfif>
			<cfset lpcnt=0>
			<cfloop query="source_classification">
				...
				<div class="classificationDiv">
					<!----
						see https://github.com/ArctosDB/arctos/issues/6708
						the bottleneck is in the QoQ and I can't make it go fast, so limiting
						10-->4s
						20-->5s
						30-->6s 
						40-->8s
					...
					<cfif lpcnt lt 51>
						<cfif isOperator is true>
							<a title="Create a copy of this classification under this name using any local source."
								href="/editTaxonomy.cfm?action=cloneClassification&taxon_name_id=#taxon_name_id.taxon_name_id#&name=#name#&classification_id=#URLEncode(classification_id)#">
									[ Clone Classification ]
							</a>
							<cfif canEditThis is true>
						...
						<cfquery name="qscore" dbtype="query">
							select gn_score,match_type from trms where classification_id=<cfqueryparam value="#classification_id#" cfsqltype="cf_sql_varchar"> and gn_score is not null group by gn_score,match_type
						</cfquery>
						...
						<cfquery name="lastdate" dbtype="query">
							select max(lastdate) as lastdate from trms where classification_id=<cfqueryparam value="#classification_id#" cfsqltype="cf_sql_varchar">
						</cfquery>
					...
					<cfelse><!---- lpcnt lt 11 ---->
						<p>
							Data not displayed: See <a class="external" href="https://github.com/ArctosDB/arctos/issues/6708">https://github.com/ArctosDB/arctos/issues/6708</a>
						</p>
					</cfif>
				</div>				
			</cfloop><!---- source_classification ---->
		</div><!---- sourceDiv ---->
	</cfloop><!---- end sources loop ----->
    <hr>
<!-------- External Links -------->
