	...
	<cfoutput>
		<cftry>
	        ...
	        <cfquery name="init_search" datasource="user_login" username="#usr#" password="#decrypt(pwd,pk,'AES/CBC/PKCS5Padding','hex')#" timeout="50" cachedwithin="#cacheObj#">
	            ...
	            where 1=1
	                ...
	                <cfif isdefined("agent_id") and len(agent_id) gt 0>
	                	<!---- https://github.com/ArctosDB/arctos/issues/7933 - allow list ---->
	                	<cfset aids="">
	                	<cfloop list="#agent_id#" index="i">
	                		<cfset bid=trim(reReplace(i, 'https?:\/\/arctos.database.museum\/agent\/',''))>
	                		<cfset aids=listAppend(aids, bid)>
	                	</cfloop>
						and agent.agent_id in (<cfqueryparam value = "#aids#" CFSQLType="cf_sql_int" list="true">) 
	                </cfif>
	            group by 
	           		agent.agent_id
	        ...
	        <cfif init_search.recordcount is 0>
				...
				<!----
					https://github.com/ArctosDB/arctos/issues/7963 we can't just fail here ugh
	        	<cfreturn result>
	        	---->
	        </cfif>
        	...
        	<cfquery name="raw" result="q_raw" datasource="user_login" username="#usr#" password="#decrypt(pwd,pk,'AES/CBC/PKCS5Padding','hex')#" timeout="50" cachedwithin="#cacheObj#">
				select 
	                ...
	                case 
	                	...
	                	when status_sort.attribute_value = 'flagged' then 9
	            ...
	            where agent.agent_id in ( 
	            	<!---- https://github.com/ArctosDB/arctos/issues/7963 - we have to do something/nothing ---->
	            	<cfif init_search.recordcount gt 0>
	            		<cfqueryparam value="#valuelist(init_search.agent_id)#" CFSQLType="cf_sql_int" list="true"> 
	            	<cfelse>
	            		-1
	            	</cfif>
	            )
	        ...
	        <cfloop query="u_agent">
	        	...
	        	<cfif len(agent_id) gt 0 and deets is true and u_agent.recordcount is 1>
	        		...
	        		<!---- https://github.com/ArctosDB/arctos/issues/7922 - add maybe-dup checks to a more visible place ---->
	        		...
	        		<cfset sobj=[=]>
					<cfset sobj["agent_type"]=u_agent.agent_type>
					<cfset sobj["agent_id"]=u_agent.agent_id>
					<cfset sobj["preferred_agent_name"]=u_agent.preferred_agent_name>
					<cfset sobj["created_by_agent_id"]=u_agent.created_by_agent_id>
