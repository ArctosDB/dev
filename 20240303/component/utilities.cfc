<cfcomponent>
<cffunction name="sanitizesql" returnformat="plain" access="public" output="true">
	<!----
		https://github.com/ArctosDB/dev/issues/99
		no real security here, just make things inconvenient for script kiddies and hopefully fire off some alarms

		from error logs:

		* remove end, case is valid/important
		
	---->
	<cfargument name="inp" type="string" required="true">
	<cfset nogoaway='arctos_keys,api_key,alter,all_tables'>
	<cfset nogoaway=nogoaway & ',begin'>
	<cfset nogoaway=nogoaway & ',cf_users,create,cf_global_settings'>
	<cfset nogoaway=nogoaway & ',delete,drop,declare'>
	<cfset nogoaway=nogoaway & ',execute,exec'>
	<cfset nogoaway=nogoaway & ',functions'>
	<cfset nogoaway=nogoaway & ',information_schema,insert'>
	<cfset nogoaway=nogoaway & ',pg_user,proname,prosrc,pg_proc,pg_addr,pg_database'>
	<cfset nogoaway=nogoaway & ',routines'>
	<cfset nogoaway=nogoaway & ',set,session'>
	<cfset nogoaway=nogoaway & ',table_name,table_type,tables'>
	<cfset nogoaway=nogoaway & ',update'>
	<cfset nogoaway=nogoaway & ',v$session'>
   <cfloop index="i" list="#inp#" delimiters=" .,?!:%$&""'/|[]{}()">
   		<cfloop list="#nogoaway#" index="l">
   			<cfif FindNoCase(l,i)>
				<cfthrow message="sanitizesql fail" detail="#l# in #inp#">
				<cfabort>
			</cfif>
		</cfloop>
	</cfloop>
