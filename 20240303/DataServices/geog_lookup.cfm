<!-------What this is and how to use it-------------
	...
	drop table cf_temp_geog_lookup;

	create table cf_temp_geog_lookup (
		key serial not null,
		higher_geog varchar,
	...
	grant select, insert, update, delete on cf_temp_geog_lookup to data_entry;
	grant select, usage on cf_temp_geog_lookup_key_seq to public;


update cf_component_loader set rec_per_run=10 where data_table='cf_temp_geog_lookup';

delete from cf_temp_geog_lookup where status='lastword: 1 matches';
...
insert into cf_component_loader (
	...
	'Geography Lookup', -- title
	'Geography Lookup: Try to find Arctos Higher Geography from various inputs', -- short description of the purpose
	1, -- run_order is a nonunique integer; 1 is as good at anything
	'autoload_geog_lookup', -- this will resolve to /ScheduledTasks/componentLoaderComponents/{what you enter here}.cfm
	'/DataServices/geog_lookup.cfm', -- this should be /loaders/something.cfm - migration in process
	'cf_temp_geog_lookup', -- the table used by the loader
	1, -- 10 is a nice number; this should NOT tax the server, and must complete in under a minute
	null,
	'data_entry', -- list of roles required to set autoload, delete, etc.
...
Disabled pending resolution of https://github.com/ArctosDB/internal/issues/366; please file an issue for assistance
...
<cfoutput>
	...
	<!----
			...
			select string_agg(column_name,',') from information_schema.columns where table_name='cf_temp_geog_lookup' and column_name not in ('key','last_ts','username','status');
	---->

	<cfset templateHeader="higher_geog,continent_ocean,country,state_prov,county,quad,feature,island,island_group,sea,hg_1,hg_2,hg_3,hg_4,hg_5,hg_6,hg_7,hg_8,hg_9,hg_10">
</cfoutput>
...
<cfsavecontent variable = "defDocTableGuts">
	<tr>
		...
		<td>no</td>
		<td>
			higher_geog-like concatenation; if this is provided components are ignored.
		</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td>no</td>
		<td>
			DO NOT LOAD WITH DATA: Results go here. The loader will return up to 10 possibile matches, ordered (imperfectly) from most general to most specific.
		</td>
	</tr>
	<tr>
		<td>status</td>
