			...
			<tr>
				<td valign="top">
					<table class="newRec">
						...
						<tr>
							<td colspan="6">
								<label for="remarks">Remarks (always private):</label>
...
<cfif action is "edit">
	<cfoutput>
		...
		<cfquery name="getPermits" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			...
			FROM
				permit_trans
				inner join permit on permit_trans.permit_id=permit.permit_id
			WHERE
		...
		<table><tr><td valign="top">
			...
			<table width="100%">
				<cfloop query="getPermits">
					<tr>
						<td>
							<p><strong>Permit ## <a href="/Permit.cfm?Action=editPermit&permit_id=#permit_id#" target="_blank">#permit_Num#</a> (#permit_Type#)</strong>
							<br>issued to #IssuedToAgent# 
							<br>issued by #IssuedByAgent#
							<br>issued_date #dateformat(issued_date,"yyyy-mm-dd")#
							<cfif len(exp_date) gt 0><br> Expires #dateformat(exp_date,"yyyy-mm-dd")# </cfif>
							<cfif len(#permit_remarks#) gt 0><br>Remarks: #permit_remarks#</cfif>
							<cfif len(#use_condition#) gt 0><br>Use Condition: #use_condition#</cfif>
			...
			<p>
				<input type="button" value="Add a permit" class="picBtn" onClick="openOverlay('/picks/PermitPick.cfm?transaction_id=#accnData.transaction_id#&callbackfunction=addNewPermitsPicked','Choose Permits');">
			</p>
