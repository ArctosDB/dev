...
<cfloop query="d">
	...
	<cftry>
		<cftransaction>
			...
			<cfif d.dimension_units is "0">
				<cfset v_dimension_units="">
			<cfelseif len(d.dimension_units) is 0>
				<cfset v_dimension_units=ctr.dimension_units>
			<cfelse>
				<cfset v_dimension_units=d.dimension_units>
			</cfif>
			...
			<cfif d.weight is "0">
				<cfset v_weight="">
			<cfelseif len(d.weight) is 0>
				<cfset v_weight=ctr.weight>
			<cfelse>
				<cfset v_weight=d.weight>
			</cfif>

			<cfif d.weight_units is "0">
				<cfset v_weight_units="">
			<cfelseif len(d.weight_units) is 0>
				<cfset v_weight_units=ctr.weight_units>
			<cfelse>
				<cfset v_weight_units=d.weight_units>
			</cfif>


			<cfif d.weight_capacity is "0">
				<cfset v_weight_capacity="">
			<cfelseif len(d.weight_capacity) is 0>
				<cfset v_weight_capacity=ctr.weight_capacity>
			<cfelse>
				<cfset v_weight_capacity=d.weight_capacity>
			</cfif>

			<cfif d.weight_capacity_units is "0">
				<cfset v_weight_capacity_units="">
			<cfelseif len(d.weight_capacity_units) is 0>
				<cfset v_weight_capacity_units=ctr.weight_capacity_units>
			<cfelse>
				<cfset v_weight_capacity_units=d.weight_capacity_units>
			</cfif>



			<!---- set v_weight junk, then run the check so we can override when demanding NULL ---->

			<cfif d.weight eq "0" and len(d.weight_units) is 0>
				<!--- NULLing --->
				<cfset v_weight="">
				<cfset v_weight_units="">
			<cfelseif d.weight eq "0" and len(d.weight_units) gt 0>
				<cfset errs="invalid (weight,weight_units): zero must be paired with NULL">
				<cfquery name="cleanupf" datasource="uam_god">
					update cf_temp_edit_container set status=<cfqueryparam value="#errs#" CFSQLType="CF_SQL_VARCHAR"> where key=<cfqueryparam value="#d.key#" CFSQLType="cf_sql_int">
				</cfquery>
				<cfcontinue />
			<cfelseif (len(d.weight) gt 0 or len(d.weight_units) gt 0) and ( len(d.weight) eq 0 or len(d.weight_units) eq 0 )>
			...
			<cfif d.weight_capacity eq "0" and len(d.weight_capacity_units) eq 0>
				<!--- spiffy, NULLing ---->
				<cfset v_weight_capacity="">
				<cfset v_weight_capacity_units="">
			<cfelseif d.weight_capacity eq "0" and len(d.weight_capacity_units) gt 0>
				<cfset errs="invalid (weight_capacity,weight_capacity_units): zero must be paired with NULL">
				<cfquery name="cleanupf" datasource="uam_god">
					update cf_temp_edit_container set status=<cfqueryparam value="#errs#" CFSQLType="CF_SQL_VARCHAR"> where key=<cfqueryparam value="#d.key#" CFSQLType="cf_sql_int">
				</cfquery>
				<cfcontinue />
			<cfelseif (len(d.weight_capacity) gt 0 or len(d.weight_capacity_units) gt 0 ) and ( len(d.weight_capacity) eq 0 or len(d.weight_capacity_units) eq 0 )>
			...
			<cfif debug>
				...
				<br>v_dimension_units=#v_dimension_units#
				<br>v_width=#v_width#
				...
				<br>v_weight=#v_weight#
				<br>v_weight_units=#v_weight_units#
				<br>v_weight_capacity=#v_weight_capacity#
				<br>v_weight_capacity_units=#v_weight_capacity_units#
			</cfif>
			...
			<cfquery name="upctr" datasource="uam_god">
				...
				set
					...
					dimension_units=<cfqueryparam value="#v_dimension_units#" CFSQLType="cf_sql_varchar" null="#Not Len(Trim(v_dimension_units))#">,
					...
					weight=<cfqueryparam value="#v_weight#" CFSQLType="cf_sql_numeric" null="#Not Len(Trim(v_weight))#">,
					weight_units=<cfqueryparam value="#v_weight_units#" CFSQLType="cf_sql_varchar" null="#Not Len(Trim(v_weight_units))#">,
					weight_capacity=<cfqueryparam value="#v_weight_capacity#" CFSQLType="cf_sql_numeric" null="#Not Len(Trim(v_weight_capacity))#">,
					weight_capacity_units=<cfqueryparam value="#v_weight_capacity_units#" CFSQLType="cf_sql_varchar" null="#Not Len(Trim(v_weight_capacity_units))#">,
