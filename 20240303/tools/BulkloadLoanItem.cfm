<!-------What this is and how to use it-------------
	...
	***** change this version statement if you change the template *****
	...
	create table cf_temp_loan_item_bak as select * from cf_temp_loan_item;

	-- CSV AND LOAD TO ISSUE!!!!
	...
	create table cf_temp_loan_item (
		...
		other_id_type varchar(30),
		other_id_number  varchar(30),
		part_barcode varchar,
		part_name varchar(255),
		ignore_subsample_in_finding boolean not null default false,
		create_subsample boolean not null default false,
		loan_item_description varchar(255),
		loan_item_remarks varchar(255),
		loan_item_instructions varchar(255),
		loan_guid_prefix varchar(30) not null,
		loan_number varchar(255) not null,
		update_part_disposition varchar(255) not null,
		update_part_condition varchar
	);

	-- no non-demo table should be this liberal
	grant select, insert, update,delete on cf_temp_loan_item to manage_records;
	grant select, usage on cf_temp_loan_item_key_seq to public;

	alter table cf_temp_loan_item alter column guid type varchar(255); 

	--pause

update cf_component_loader  set rec_per_run=0 where data_table='cf_temp_loan_item';
------------->
...
--->
<cfset required_roles="manage_records,manage_transactions">
<cfoutput>
...
<cfset title="Bulkload Loan Items">
<cfset thisFormFile="BulkloadLoanItem.cfm">
<cfset thisFormName="Bulkload Loan Item Tool">
<cfset thisFormPurpose='This tool allows Arctos operators with <a href="/Admin/user_roles.cfm">role(s)</a> (#required_roles#) to create and review loan item loads.'>
<cfset thisTable="cf_temp_loan_item">
<cfset thisTemplateName="bulkloadLoanItemTemplate.csv">
<cfset thisDownloadName="bulkloadLoanItemData.csv">
<cfset templateHeader="partID,guid,guid_prefix,other_id_type,other_id_number,part_barcode,part_name,ignore_subsample_in_finding,create_subsample,loan_item_description,loan_item_remarks,loan_item_instructions,loan_guid_prefix,loan_number,update_part_disposition,update_part_condition">
</cfoutput>
...
<cfsavecontent variable = "defDocTableGuts">
	<tr>
		...
		<td>conditionally</td>
		<td>
			partID, GUID, guid_prefix+other_id_type+other_id_number, or part_barcode may be used to specify the catalog record.
			<br>partID is available from SpecimenResults/Manage/PartsTableDownload.
			<br>When partID is given, guid, guid_prefix, other_id_type, other_id_number, part_barcode, part_name, and ignore_subsample_in_finding are ignored.
		</td>
	</tr>
	<tr>
		<td>guid</td>
	...
	<tr>
		...
		<td>conditionally</td>
		<td>
			Either GUID or guid_prefix+other_id_type+other_id_number may be used to specify the catalog record.
		</td>
	</tr>
	<tr>
		<td>part_barcode</td>
		<td>conditionally</td>
		<td>
			If part_barcode is given, it serves as the single way to specify the catalog record AND the part. If part_barcode is not given, part_name
			(and GUID or guid_prefix+other_id_type+other_id_number) must be. This will only work if there is a single
			part directly contained by the container bearing the supplied barcode.
		</td>
	</tr>
	<tr>
		<td>part_name</td>
		<td>conditionally</td>
		<td>
			If part_barcode is given, it serves as the single way to specify the part. If part_barcode is not given, part_name must be.
		</td>
	</tr>
	<tr>
		<td>ignore_subsample_in_finding</td>
		<td>if included</td>
		<td>
			If part_barcode is not given, a value of "true" will ignore subsamples when finding parts to loan; a value of "false" will also find subsamples.
			Default value is false. This is not considered if barcode is given. NOTE: you may remove this column from the CSV Upload to use the default value.
			If the column is included then a value must be supplied.
		</td>
	</tr>
	<tr>
		<td>create_subsample</td>
		<td>if included</td>
		<td>
			Create a subsample of the specified part and add it to the loan, rather than adding the specified part itself.
			Boolean; acceptable values are "true" and "false"; default value is false. NOTE: you may remove this column from the CSV Upload to use the default value.
			If the column is included then a value must be supplied.
		</td>
	</tr>
	<tr>
		<td>loan_item_description</td>
		<td>no</td>
		<td>
			Text to use in loan_item.item_descr
		</td>
	</tr>
	<tr>
		<td>loan_item_remarks</td>
		<td>no</td>
		<td>
			Text to use in loan_item.loan_item_remarks
		</td>
	</tr>
	<tr>
		<td>loan_item_instructions</td>
		<td>no</td>
		<td>
			Text to use in loan_item.item_instructions
		</td>
	</tr>
	<tr>
		...
		<td>yes</td>
		<td>
			The collection owning the loan to which the item will be added
		</td>
	</tr>
	<tr>
		<td>loan_number</td>
	...
	<tr>
		<td>update_part_disposition</td>
		<td>yes</td>
		<td>
			New part disposition.
			<a href="/info/ctDocumentation.cfm?table=ctdisposition">ctdisposition</a>
		</td>
	</tr>
	<tr>
		<td>update_part_condition</td>
		<td>no</td>
		<td>
			New part condition. NULL (empty) does nothing, leaving existing values as-is; a value updates the part.
		</td>
	</tr>
	<tr>
		<td>status</td>
			...
			<form name="f" method="post" action="#thisFormFile#">
				...
				<table border id="t" class="sortable">
					<tr>
						...
						<th>part_barcode</th>
						<th>part_name</th>
						<th>ignore_subsample_in_finding</th>
						<th>create_subsample</th>
						<th>loan_item_description</th>
						<th>loan_item_remarks</th>
						<th>loan_item_instructions</th>
						<th>loan_guid_prefix</th>
						<th>loan_number</th>
						<th>update_part_disposition</th>
						<th>update_part_condition</th>
							<!--------------- END::this section will need customized for individual loaders ----------------------------->
						</tr>
					<cfloop query="d">
						<tr>
