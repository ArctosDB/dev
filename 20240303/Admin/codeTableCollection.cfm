...
<cfif action is "nothing">
	...
	<cfoutput>
		<cfif len(guid_prefix) is 0>
			<h3>Choose a collection to get started.</h3>
		<cfelse>
			Managing #table# for #guid_prefix#
		</cfif>
		...
		<cfif listFind("ctlife_stage,ctsex_cde", table)>
			...
			<cfquery name="collection_in_use" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
				...
				inner join ctattribute_type on attributes.attribute_type=ctattribute_type.attribute_type
				...
				---->
				and ctattribute_type.value_code_table=<cfqueryparam value="#table#" cfsqltype="cf_sql_varchar">
		...
		<cfelseif table is "ctattribute_type">
			...
			<cfquery name="collection_in_use_att" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			...
			<form name="selector" method="post" action="codeTableCollection.cfm">
				<input type="hidden" name="table" value="#table#">
				<input type="hidden" name="guid_prefix" value="#guid_prefix#">
				<input type="hidden" name="action" value="savepick_ctattribute_type">
				<input type="submit" value="Update Collection Attribute List" class="savBtn">
				<cfset sobj=structNew()>
				<table border id="seltbl" class="sortable">
					<tr>
						<th>Attribute</th>
						<th>Select</th>
					...
					<cfloop query="q">
						<cfquery name="isThisOneUsed" dbtype="query">
							select count(*) c from collection_in_use_att where attribute_type=<cfqueryparam value="#attribute_type#" cfsqltype="cf_sql_varchar">
						</cfquery>
						<cfif isThisOneUsed.c gt 0>
							<cfset thisStatus="In Use">
						<cfelseif listfind(collections,guid_prefix)>
						...
						<cfset sobj[attribute_type]=thisStatus>
						<tr>
							<td>
								#attribute_type#
							</td>
							<td>
								<input <cfif thisStatus is "In Use">  disabled readonly </cfif> type="checkbox" name="selected_value" value="#attribute_type#" <cfif listfind(collections,guid_prefix)> checked </cfif> >
							</td>
							<td>
								#thisStatus#
							</td>
							<td>
								<cfif len(value_code_table) gt 0>
									IN (<a href="/info/ctDocumentation.cfm?table=#value_code_table#">#value_code_table#</a>)
								<cfelseif len(unit_code_table) gt 0>
									number+<a href="/info/ctDocumentation.cfm?table=#unit_code_table#">#unit_code_table#</a>
								<cfelse>
									freetext
								</cfif>
		...
		<cfelseif table is "ctpart_attribute_type">
			...
			<cfquery name="collection_in_use_att" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
				select attribute_type from specimen_part_attribute
			...
			<form name="selector" method="post" action="codeTableCollection.cfm">
				<input type="hidden" name="table" value="#table#">
				<input type="hidden" name="guid_prefix" value="#guid_prefix#">
				<input type="hidden" name="action" value="savepick_ctpart_attribute_type">
				<input type="submit" value="Update Collection Part Attribute List" class="savBtn">
				<cfset sobj=structNew()>
				<table border id="seltbl" class="sortable">
					<tr>
						<th>Attribute</th>
...
<cfif action is "savepick_ctpart_attribute_type">
	<cfoutput>
		<cfquery name="q" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			select
				attribute_type,
			...
			<cfloop collection="#oss#" item="itm">
				<cfif (oss[itm] is 'Recommended' or oss[itm] is 'Available') and listfind(selected_value,itm)>
					<cfquery name="thisCollections" dbtype="query">
						select collections from q where  attribute_type=<cfqueryparam value="#itm#" cfsqltype="cf_sql_varchar">
					</cfquery>
					...
					<cfif (len(gl) is 0 and len(q.collections) is 0) or Compare(gl,q.collections) neq 0>
						<cfquery name="insert_my_collection" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
							update ctpart_attribute_type set 
								...
								where attribute_type=<cfqueryparam value="#itm#" cfsqltype="cf_sql_varchar">
						</cfquery>
					</cfif>
...
<cfif action is "savepick_ctattribute_type">
	<cfoutput>
		<cfquery name="q" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			select
				attribute_type,
	...
	<cfoutput>
		...
		<cftransaction>
			<cfloop collection="#oss#" item="prt">
				<cfif (oss[prt] is 'Recommended' or oss[prt] is 'Available') and listfind(selected_part,prt) gt 0>
