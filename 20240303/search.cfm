		...
		$('#tools_ctl').change(function(){
			...
			if (opn=='downloadRequest'){
				...
				if (size==0){
					...
					openOverlay('/form/catRecordSaveSearch.cfm?tbl='+tbl+'&rc=' + results_columns + '&so=' + search_term_obj,'Save or Archive Search.');
				}
...
<cfquery name="ctpart_attribute_type" datasource="cf_codetables" cachedwithin="#createtimespan(0,0,60,0)#">
	select attribute_type from ctpart_attribute_type group by attribute_type order by attribute_type
</cfquery>
...
<cfquery name="ct_continent" datasource="cf_codetables" cachedwithin="#createtimespan(0,0,60,0)#">
	select continent from geog_auth_rec group by continent ORDER BY continent
</cfquery>
<cfquery name="ct_ocean" datasource="cf_codetables" cachedwithin="#createtimespan(0,0,60,0)#">
	select ocean from geog_auth_rec group by ocean ORDER BY ocean
</cfquery>
...
<cfoutput>
		...
		<div id="cat_rec_srch_frm_wrap">
			...
			<div id="identification" class="srch_section">
				...
				<div class="section_content">
					<div class="pglayout">
						...
						<cfparam name="suborder" default="">
						<div class="schitem noshow" id="si_suborder">
							<label for="suborder">
								<span class="helpLink" data-helplink="suborder">Suborder</span>
								<span class="likeLink" onclick="var e=document.getElementById('suborder');e.value='='+e.value;">[ exact ]</span>
								<span class="likeLink" onclick="var e=document.getElementById('suborder');e.value='NULL';">[ NULL ]</span>
								<span class="likeLink" onclick="var e=document.getElementById('suborder');e.value='!'+e.value;">[ NOT ]</span>
							</label>
							<input type="text" name="suborder" id="suborder" value="#suborder#" placeholder="Collection's classification">
						</div>
			...
			<div id="parts" class="srch_section">
				...
				<div class="section_content">
					...
					<cfparam name="part_attribute" default="">
						...
						<thead>
							<tr>
								<th scope="col">
									...
									<span class="likeLink" onclick="getCtDoc('ctpart_attribute_type',cat_rec_sch_frm.part_attribute.value);">Define</span>
								</th>
									...
									<select name="part_attribute" id="part_attribute" size="1">
										<option value=""></option>
											<cfloop query="ctpart_attribute_type">
												<option <cfif part_attribute is ctpart_attribute_type.attribute_type> selected="selected" </cfif> value="#ctpart_attribute_type.attribute_type#">#ctpart_attribute_type.attribute_type#</option>
											</cfloop>
									  </select>				
								</td>
				...
				<div class="section_content">
					<div class="pglayout">
						...
						<div class="schitem noshow" id="si_collecting_method">
							<label class="helpLink" data-helplink="collecting_method" for="collecting_method">Collecting Method</label>
				...
				<div class="section_content">
					<div class="pglayout">
						...
						<div class="schitem noshow" id="si_any_geog">
							<label class="helpLink" data-helplink="any_geog" for="any_geog">Any&nbsp;Geographic&nbsp;Element</label>
						...
						<div class="schitem noshow dontwrap" id="si_place_term">
							<label class="helpLink" data-helplink="place_term" for="place_term_type">
								Place Terms
							</label>
					...
					<div class="pglayout">
						...
						<div class="schitem noshow" id="si_continent">
							<label class="helpLink" data-helplink="continent" for="continent">Asserted Continent</label>
							<cfset x=continent>
							<select name="continent" id="continent" size="1" >
								<option value=""></option>
								<option value="NULL">NULL</option>
								<cfloop query="ct_continent">
									<option <cfif ct_continent.continent is x> selected="selected" </cfif>  value="#ct_continent.continent#">#ct_continent.continent#</option>
								</cfloop>
							</select>
						</div>
						<cfparam name="ocean" default="">
						<div class="schitem noshow" id="si_ocean">
							<label class="helpLink" data-helplink="ocean" for="ocean">Asserted Ocean</label>
							<cfset x=ocean>
							<select name="ocean" id="ocean" size="1" >
								<option value=""></option>
								<option value="NULL">NULL</option>
								<cfloop query="ct_ocean">
									<option <cfif ct_ocean.ocean is x> selected="selected" </cfif>  value="#ct_ocean.ocean#">#ct_ocean.ocean#</option>
								</cfloop>
							</select>
						</div>
	...
	<!----
		https://github.com/ArctosDB/dev/issues/129
		set autosubmit=false to prevent initial query
	---->
	<cfparam name="autosubmit" default="true">
	<script>
		$(document).ready(function() {
			...
			$('##crsdtable').DataTable({
				...
				//deferLoading: 57,
				<cfif autosubmit is false>"deferLoading": 0,</cfif>
	...
	<select form="cat_rec_sch_frm" name="tools_ctl" id="tools_ctl" size="1">
		...
		<cfif len(session.username) gt 0>
			<optgroup label="Save / Share">
				<!---- reserving this for "us" because there's a forever expense ---->
				<cfif listfindnocase(session.roles,'coldfusion_user')>
					<option value="save_archive">Save or Archive Search</option>
				</cfif>
				<!--- same functionality, no long-term cost ---->
			...
			<cfif listfindnocase(session.roles,'manage_records')>
				<optgroup label="Manage">
					...
					<option value="/tools/movePartsToContainer.cfm">Move Part Containers</option>
