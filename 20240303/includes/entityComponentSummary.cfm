...
<div class="entityExplanation">
	This record represents an <strong>Entity</strong>, which is composed of catalog records that represent <i>something</i> discrete, such as a biological individual or a cultural item, made of many components.
</div>
<cfoutput>
	<!---- replacing #application.serverRootUrl# with hardcoded URL below so this works on test, probably will explode somewhere... --->
	<cfquery name="entities" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
		...
		from
			...
			inner join coll_obj_other_id_num on #session.flatTableName#.collection_object_id=coll_obj_other_id_num.collection_object_id and 
				display_value=<cfqueryparam value="https://arctos.database.museum/guid/#guid#" CFSQLType="CF_SQL_varchar" list="false">
	</cfquery>
