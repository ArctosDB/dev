...
<cfif not listcontainsnocase(session.roles,'coldfusion_user')>
	...
	<cfif currentPath neq "/errors/blocked.cfm">
		...
		<cfif isdefined("request") and len(request) gt 0>
			<!---- javax.servlet.forward.mapping - some complex weird thing introed in some weird version of something - fucks with this
				<cfset structAppend(variables.vstruct, request)>
				0---->
			<cftry>
				<cfset structAppend(variables.vstruct, request.javax.servlet.forward.context_path)>
				<cfcatch><!----------failed appending request-------------></cfcatch>
			</cftry>
			<cftry>
				<cfset structAppend(variables.vstruct, request.javax.servlet.forward.request_uri)>
				<cfcatch><!----------failed appending request-------------></cfcatch>
			</cftry>
			<cftry>
				<cfset structAppend(variables.vstruct, request.javax.servlet.forward.request_uri)>
			...
			<cftry>
				<cfset structAppend(variables.vstruct, request.lucee.forward.request_uri)>
				<cfcatch><!----------failed appending request-------------></cfcatch>
			</cftry>
			<cftry>
				<cfset structAppend(variables.vstruct, request.lucee.forward.request_uri)>
