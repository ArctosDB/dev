...
<cfquery name="ctEncAct" datasource="cf_codetables" cachedwithin="#createtimespan(0,0,60,0)#">
...
<cfif action is "create">
	...
	<cfoutput>
		<form name="encumber" method="post" action="Encumbrances.cfm">
			...
			<input type="text" name="encumberingAgent" id="encumberingAgent" class="reqdClr minput"
...
<cfif action is "nothing">
	<cfoutput>
		<cfset title = "Search for records or encumbrances">
		<h2>Encumbrances</h2>
		Encumbrances mask information from public view.
		<!---- https://github.com/ArctosDB/dev/issues/130 ---->
		<div style="font-size: smaller; background-color: lightgray;">
			 Encumbrances which meet these conditions will be deleted once a year: 
			 <br>a) expiration that are over 1 year old; 
			 <br>b)  no attached records
		</div>
		<p>
			<cfif len(table_name) gt 0 or len(collection_object_id) gt 0>
				Now find an encumbrance to apply to or remove from the records below. If you need a new encumbrance, create it
...
<cfif action is "createEncumbrance">
	<cfoutput>
		<cfquery name="nextEncumbrance" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			select nextval('sq_encumbrance_id') nextEncumbrance
		</cfquery>
		<cfquery name="newEncumbrance" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
...
<cfif action is "listEncumbrances">
	...
	<script>
		function confirmRemoveSpecs(eid){
			if (confirm("Are you sure you want to remove all records from this encumbrance?")==true){
	...
	<cfoutput>
		<cfset s="select
			...
			from
				encumbrance
				left outer join coll_object_encumbrance on encumbrance.encumbrance_id = coll_object_encumbrance.encumbrance_id">
		<cfset q = " WHERE 1=1 ">
		<cfset sql=" ">
		...
		<cfif isdefined("made_date_before") and len(#made_date_before#) gt 0>
			<cfset sql = "#sql# AND made_date <= '#made_date_before#'">
		</cfif>
		<cfif isdefined("expiration_date_after") and len(#expiration_date_after#) gt 0>
			<cfset sql = "#sql# AND expiration_date >= '#expiration_date_after#'">
		</cfif>
		<cfif isdefined("expiration_date_before") and len(#expiration_date_before#) gt 0>
			<cfset sql = "#sql# AND expiration_date <= '#expiration_date_before#'">
		</cfif>
		...
		<cfset sql=s & q & sql & " group by encumbrance.encumbrance_id,
				...
				encumbrance.expiration_date,
				encumbrance.remarks">
		<cfquery name="getEnc" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
				...
				<form name="listEnc#i#" method="post" action="Encumbrances.cfm">
					...
					<tr #iif(i MOD 2,DE("class='evenRow'"),DE("class='oddRow'"))#>
						...
						<td nowrap>
							...
							<div>
								<a href="/search.cfm?encumbrance_id=#encumbrance_id#">[ See all Records in this Encumbrance ]</a>
							</div>
							<cfif listfindnocase(session.roles, "MANAGE_COLLECTION")>
								<div>
									<a href="/Admin/deleteSpecByEncumbrance.cfm?encumbrance_id=#encumbrance_id#">
										[ Delete all Records in this Encumbrance ]
									</a>
								</div>
							<cfelse>
								<div>
							...
							<div class="likeLink" onclick="confirmRemoveSpecs('#encumbrance_id#')">
								[ Remove (unencumber) All Records from this Encumbrance ]
							</div>
						</td>
					</tr>
				</form>
...
<cfif action is "removeAllSpecimens">
	<cfoutput>
		<cfquery name="encDetails" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
...
<cfif action is "remListedItems">
	<cfoutput>
		...
		<p>
			Listed records have been unencumbered.
		</p>
		<p>
...
<cfoutput>
	...
	<cfquery name="isUsed" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
		select count(*) as cnt from coll_object_encumbrance where encumbrance_id=#encumbrance_id#
	</cfquery>
	<cfif isUsed.cnt gt 0>
		<div class="error">
			You can't delete used encumbrances; unencumber records first.<cfabort>
		</div>
	</cfif>
	<cfquery name="deleteEnc" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
...
<cfoutput>
	...
	<cfif  len(table_name) is 0 AND LEN(collection_object_id) IS 0>
		Didn't get records<cfabort>
	</cfif>
	<CFIF LEN(table_name) GT 0>
		<cfquery name="unencumberedSpecimens" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
...
<cfif len(table_name) gt 0 or len(collection_object_id) gt 0>
	...
	<Cfset title = "Manage Encumbrances for these records">
		<cfoutput>
		...
		<br><strong>Cataloged Items being encumbered:</strong>
			<table width="95%" border="1">
				<tr>
					<td><strong>Catalog Record</strong></td>
