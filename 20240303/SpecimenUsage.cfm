...
<cfif action is "search">
	<script>
		...
		function copyPublicationIdentifier(pid){
			...
			$('<span class="copyalert">Copied to clipboard</span>').insertAfter('#fgcopypidbtn').delay(3000).fadeOut();
		}
	</script>
	<cfoutput>
	...
	<cfdump var=#projects#>
		...
		<cfif isdefined("current_Sci_Name") AND len(#current_Sci_Name#) gt 0>
			<cfset tbls = "#tbls# inner join citation CURRENT_NAME_CITATION on publication.publication_id = CURRENT_NAME_CITATION.publication_id
				inner join flat ci_current on CURRENT_NAME_CITATION.collection_object_id = ci_current.collection_object_id
			">
			<cfset thisrow={}>
			<cfset thisrow.l="false">
			<cfset thisrow.d="cf_sql_varchar">
			<cfset thisrow.t="upper(ci_current.scientific_name)">
			<cfset thisrow.o="LIKE">
		...
		<cfif isdefined("cited_Sci_Name") AND len(cited_Sci_Name) gt 0>
			<cfset tbls = "#tbls# inner join citation CURRENT_NAME_CITATION on publication.publication_id = CURRENT_NAME_CITATION.publication_id
				inner join	identification CitTaxa on CURRENT_NAME_CITATION.identification_id = CitTaxa.identification_id ">
			<cfset thisrow={}>
			<cfset thisrow.l="false">
			<cfset thisrow.d="cf_sql_varchar">
			<cfset thisrow.t="CitTaxa.scientific_name">
			<cfset thisrow.o="ilike">
			<cfset thisrow.v= '%#trim(cited_Sci_Name)#%'>
			<cfset arrayappend(qp,thisrow)>
		</cfif>
		<cfif isdefined("proj_pub_remark") AND len(proj_pub_remark) gt 0>
			<cfset thisrow={}>
...
limit #maxNumberOfRows#
		...
		<cfloop query="pubs">
			<div #iif(i MOD 2,DE("class='evenRow'"),DE("class='oddRow'"))#>
				...
				<div class="projCtl">
					<input class="lnkBtn" id="fgcopypidbtn" type="button" value="copy publication identifier" onclick="copyPublicationIdentifier('#Application.serverRootURL#/publication/#publication_id#');">
				...
				<div class="projMeta">
					<cfif len(doi) gt 0>
						...
						<div id='x#escdoi#' data-doi='#doi#'><a class="external" target="_blank" href="#doi#">#doi#</a></div>
					<cfelse>
						<cfif isdefined("session.roles") and listfindnocase(session.roles,"manage_publications")>
