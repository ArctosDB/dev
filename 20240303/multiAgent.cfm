...
<cfoutput>
	<cfquery name="getColls" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
		...
		FROM
			...
			left outer join agent on collector.agent_id=agent.agent_id
	</cfquery>
...
<cfloop query="ci">
	<cfquery name="c" dbtype="query">
		select preferred_agent_name from getColls where collector_role='collector' and guid=<cfqueryparam value="#guid#" cfsqltype="cf_sql_varchar"> order by COLL_ORDER
	</cfquery>
	<cfquery name="p" dbtype="query">
		select preferred_agent_name from getColls where collector_role='preparator' and guid=<cfqueryparam value="#guid#" cfsqltype="cf_sql_varchar"> order by COLL_ORDER
	</cfquery>
	<cfquery name="m" dbtype="query">
		select preferred_agent_name from getColls where collector_role='creator' and guid=<cfqueryparam value="#guid#" cfsqltype="cf_sql_varchar"> order by COLL_ORDER
	</cfquery>
    <tr>
	  <td>
