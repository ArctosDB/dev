	...
	<cfquery name="getMapData" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
		...
		FROM
			...
			WHERE
				...
				coalesce(specimen_event.verificationstatus,'NULL') != 'unaccepted' and 
				not exists (select locality_id from locality_attributes where locality_attributes.locality_id=locality.locality_id and attribute_type='locality access')
