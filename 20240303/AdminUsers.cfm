...
<cfif action is "edit">
	<cfoutput>
		...
		<cfquery name="theirCollectionRoles" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			...
			SELECT 
				cte.rolname as role_name,
				collection.guid_prefix
			from 
				cte
				inner join collection on cte.rolname=collection.collection_role
			order by rolname
		</cfquery>
		<cfquery name="myActionRoles" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			select role_name, sort_order from cf_ctuser_roles where role_name in (select unnest(has_roles) from current_user_roles)
		</cfquery>
		<cfquery name="theirActionRoles" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			...
			SELECT cte.rolname as role_name,
			cf_ctuser_roles.sort_order
			from cte 
			inner join cf_ctuser_roles on cte.rolname=cf_ctuser_roles.role_name order by sort_order
		</cfquery>
		<cfquery name="grantableActionRoles" dbtype="query">
			select role_name,sort_order from myActionRoles where role_name not in (
		...
		<ul>
			<li>Users must have both functional roles and access to collections to use Arctos. Accounts without both will be auto-locked.</li>
			<li>Users must have both agent and an operator email addresses. Accounts without will be auto-locked.</li>
					...
					<td valign="top">
						<table border>
							...
							<cfloop query="theirActionRoles">
								<tr>
									<td>#role_name#</td>
									<td><a href="/Admin/user_roles.cfm?action=defineRole&role_name=#role_name#" class="external">define</a></td>
									<td><a href="/Admin/user_roles.cfm?###role_name#" class="external">doc</a></td>
									<td>
						...
						<table border>
							...
							<cfloop query="theirCollectionRoles">
								<tr>
									...
									<td>
										<a href="/collection/#guid_prefix#" class="external">#guid_prefix#</a>
									</td>
									<td>
										<a href="AdminUsers.cfm?action=remrole&role_name=#role_name#&username=#username#">
											<input type="button" class="delBtn" value="revoke">
...
<cfif action is "makeNewDbUser">
	<cfoutput>
		...
		<cfelse>
			...
			<!---- ilike for nocase: https://github.com/ArctosDB/dev/issues/157 ---->
			<cfquery name="hem" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
				...
				attribute_value ilike <cfqueryparam value="#usrdata.EMAIL#" CFSQLType="cf_sql_varchar">
			</cfquery>
			<cfif hem.c is 0>
