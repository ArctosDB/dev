		...
		<table cellpadding="0" cellspacing="0">
	 		<tr>
				...
				<td>
					...
					<input name="barcode" type="text" value="#getCont.barcode#" id="barcode" readonly="readonly" class="readClr" title="change not allowed">
				</td>
			</tr>
				...
				<td colspan="2">
					...
					<table cellspacing="0" cellpadding="0" width="100%" id="dimTbl">
						<tr>
							...
							<td>
								<label for="height">Units</label>
								<select name="dimension_units" id="dimension_units">
									<option value=""></option>
									<cfloop query="ctlength_units">
										<option value="#ctlength_units.length_units#" <cfif getCont.dimension_units is ctlength_units.length_units> selected="selected" </cfif>>#ctlength_units.length_units#</option>
									</cfloop>
								</select>
							</td>
						</tr>
...
<td valign="top"><!---- right column ---->
	...
	<cfif listfindnocase(session.roles,'manage_transactions')>
		<cfquery name="trans_container" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			select 
				trans_container.transaction_id,
				collection.guid_prefix,
				trans.transaction_type,
				accn.accn_number as trans_id
			 from 
			 	trans_container
			 	inner join trans on trans_container.transaction_id=trans.transaction_id
		 	 ...
		 	 where 
		 	 	trans_container.container_id=<cfqueryparam value="#container_id#" cfsqltype="cf_sql_int">
		 	 union
		 	 select 
				trans_container.transaction_id,
				collection.guid_prefix,
		...
		<cfif trans_container.recordcount gt 0>
			<h4>Transactions</h4>
			<ul>
				<cfloop query="trans_container">
					<li>
		...
		<p>
			...
			<table border>
				...
				<tr>
					...
					<td>#donor_container.dimension_units#</td>
					...
					<td>#receiver_container.dimension_units#</td>
	...
	<cfoutput>
		...
		<cfquery name="updatecontainer" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			update container set
				...
				dimension_units=<cfqueryparam value="#dimension_units#" cfsqltype="cf_sql_varchar" null="#Not Len(Trim(dimension_units))#">,
...
<cfif action is "moveChillun">
	<cfoutput>
		...
		<cfif cidOfnewParentBarcode.recordcount is not 1 or len(cidOfnewParentBarcode.container_id) lt 1>
			bad barcode<cfabort>
		</cfif>
		<cfquery name="moveAllChildren" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			update container set parent_container_id=<cfqueryparam value="#cidOfnewParentBarcode.container_id#" cfsqltype="cf_sql_int">
				where parent_container_id=<cfqueryparam value="#container_id#" cfsqltype="cf_sql_int">
		</cfquery>
		<p>
