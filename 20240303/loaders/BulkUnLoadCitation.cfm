<!-------What this is and how to use it-------------
	...
	drop table cf_temp_unload_citation;
	...
	grant select, insert, update, delete on cf_temp_unload_citation to manage_collection;
...
delete from cf_component_loader where data_table='cf_temp_unload_citation';
...
insert into cf_component_loader (
	...
	'Citation: Bulk Unload',
	'This tool allows Arctos operators with the <a href="/Admin/user_roles.cfm">Manage Collection</a> role to DELETE citations. This process is irreversible; proceed with great caution. This form will unload ALL publication-at-specimen citations; a record may have multiple identifications using a publication, this will remove them all.',
	1, -- run_order is a nonunique integer; 1 is as good at anything
	'autounload_citation', -- this will resolve to /ScheduledTasks/componentLoaderComponents/{what you enter here}.cfm
	'/loaders/BulkUnLoadCitation.cfm', -- this should be /loaders/something.cfm - migration in process
	'cf_temp_unload_citation', -- the table used by the loader
...
<cfoutput>
	...
	<cfset templateHeader="guid,publicationID,doi,status">
</cfoutput>
...
<cfsavecontent variable = "defDocTableGuts">
	<tr>
		...
		<td>yes</td>
		<td>
			Record from which to remove the citation. Example: https://arctos.database.museum/guid/UWBM:PB:21291 (or UWBM:PB:21291)
		</td>
	</tr>
	<tr>
		<td>publicationID</td>
		<td>publicationID or doi is required</td>
		<td>
			Publication from which to remove the citation. Example: https://arctos.database.museum/publication/10009268
		</td>
	</tr>
	<tr>
		<td>doi</td>
		<td>publicationID or doi is required</td>
		<td>
			Publication from which to remove the citation. Example: https://doi.org/10.1111/bij.12833
		</td>
	</tr>
	<tr>
		<td>status</td>
