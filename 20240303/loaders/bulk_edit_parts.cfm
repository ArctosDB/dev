<!-------What this is and how to use it-------------
	...
	drop table cf_temp_bulk_edit_parts;
...
insert into cf_component_loader (
	...
	'Bulk Edit Parts', -- title
	'Bulk Edit Parts: Change core-stuff about parts', -- short description of the purpose
	1, -- run_order is a nonunique integer; 1 is as good at anything
	'autoload_edit_parts', -- this will resolve to /ScheduledTasks/componentLoaderComponents/{what you enter here}.cfm
	'/loaders/bulk_edit_parts.cfm', -- this should be /loaders/something.cfm - migration in process
	'cf_temp_bulk_edit_parts', -- the table used by the loader
	...
	'Username has manage_records access to corresponding collection' -- description of any in-loader checks
);
...
<cfoutput>
	...
	<!----
			...
			select string_agg(column_name,',') from information_schema.columns where table_name='cf_temp_bulk_edit_parts' and column_name not in ('key','last_ts','username','status');
	---->

	<cfset templateHeader=" partID,part_name,condition,disposition,part_count,remarks">
</cfoutput>

<!------------------------------------------ BEGIN: documentation table guts ---------------------------------------------------------->

<cfsavecontent variable = "defDocTableGuts">
	<!---- https://github.com/ArctosDB/arctos/issues/7999 ---->
	<tr>
		<td> </td>
		<td>SPECIAL NOTICE</td>
		<td>
			NULL below means nothing, empty, does not exist. This does not mean the string "NULL." Uploading a file with an empty value OR omitting the column from the upload will result in this condition.
		</td>
	</tr>
	<tr>
		<td>partID</td>
		<td>yes</td>
		<td>
			part identifier, available from Parts table/download (among other places), Format is <pre>https://arctos.database.museum/guid/{triplet}/PID{integer}</pre>
		</td>
	</tr>
	<tr>
		<td>part_name</td>
		<td>no</td>
		<td>
			NULL to do nothing; value from <a href="/info/ctDocumentation.cfm?table=ctspecimen_part_name">ctspecimen_part_name</a> to change.
		</td>
	</tr>
	<tr>
		<td>condition</td>
		<td>no</td>
		<td>
			NULL to do nothing; text to change.
		</td>
	</tr>
	<tr>
		<td>disposition</td>
		<td>no</td>
		<td>
			NULL to do nothing; value from <a href="/info/ctDocumentation.cfm?table=ctdisposition">ctdisposition</a> to change.
		</td>
	</tr>
	<tr>
		<td>part_count</td>
		<td>no</td>
		<td>
			NULL to do nothing; integer to change.
		</td>
	</tr>
	<tr>
		<td>remarks</td>
