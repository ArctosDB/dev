<!-------What this is and how to use it-------------
	...
	drop table cf_temp_move_container;

	create table cf_temp_move_container (
		key serial not null,
		barcode varchar not null,
		parent_barcode varchar not null,
	...
	grant select, insert, update, delete on cf_temp_move_container to manage_container;
	grant select, usage on cf_temp_move_container_key_seq to public;




delete from cf_component_loader where loader_template='autoload_move_container';
...
insert into cf_component_loader (
	...
	'Move Containers by barcode', -- short description of the purpose
	1, -- run_order is a nonunique integer; 1 is as good at anything
	'autoload_move_container', -- this will resolve to /ScheduledTasks/componentLoaderComponents/{what you enter here}.cfm
	'/loaders/move_container.cfm', -- this should be /loaders/something.cfm - migration in process
	'cf_temp_move_container', -- the table used by the loader
	10, -- 10 is a nice number; this should NOT tax the server, and must complete in under a minute
	null,
	'manage_container', -- list of roles required to set autoload, delete, etc.
...
<cfoutput>
	...
	<cfset templateHeader="barcode,parent_barcode">
</cfoutput>
...
<cfsavecontent variable = "defDocTableGuts">
	...
	<tr>
		...
		<td>yes</td>
		<td>
			barcode of new parent of container to move
		</td>
	</tr>
	<tr>
		<td>status</td>
