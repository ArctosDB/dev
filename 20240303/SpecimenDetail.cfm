...
<script src="/includes/specimenDetail.js?v=5"></script>
<cfif isdefined("session.roles") and listfindnocase(session.roles,"coldfusion_user")>
	<script src="/includes/specimenDetailOperator.js?v=1"></script>
</cfif>
<style>
	.taxonDets{
		font-size: small;
...
<cfoutput>
	...
	<div id="gp_record_summary">
		...
		<div id="guid_summary_data" class="summarycolumn">
			<div id="SDSummaryPart" class="summary_row">
				<cfif len(detail.partString) gt 0>
	...
	<cfquery name="colrs" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
		select
			...
			case when collector_role='creator' then 1 else 2 end primarysort
		from
			collector
		...
		order by primarysort,coll_order,preferred_agent_name
	</cfquery>	
	<!-------------------------------------   Identifications    -------------------------------------------------->
	<!-------------------------------------   Identifications    -------------------------------------------------->
	<!-------------------------------------   Identifications    -------------------------------------------------->
	...
	<div class="gp_twocolumn">
		...
		<div id="gp_collector_container" class="gp_one_section">
			...
			<div class="collectorGroup">					
				<table border id="vcollrtbl guidPageTable">
					<thead>
						<tr>
							<th scope="col"><div class="gp_v_col_hdr">Role</div></th>
							...
							<tr>
								...
								<td data-label="Agent: ">
									<cfif oneOfUs != 1 and detail.encumbranceDetail contains collector_role>
										Anonymous
									<cfelse>
										<a class="newWinLocal" href="/agent/#agent_id#">#preferred_agent_name#</a>
									</cfif>
								</td>
								<td data-label="Remark: ">#encodeForHTML(collector_remark)#</td>
							</tr>
						</cfloop>
					</tbody>
				</table>
	...
	<div class="gp_twocolumn">
		...
		<div id="cat_rec_transactions"  class="gp_one_section">
			<label for="cat_rec_transactions" class="gp_section_label">Transactions</label>
			<!-----
				https://github.com/ArctosDB/dev/issues/131: transaction identifiers are public
			---->
			<div id="all_transactions">
				<!---- https://github.com/ArctosDB/dev/issues/133 - run this as user_login but expect it to fail below ---->
				<cfquery name="transactions" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
					SELECT
						loan.transaction_id,
						loan.loan_type as trans_type,
						loan.loan_status as trans_status,
						to_char(loan.closed_date,'yyyy-mm-dd') closed_date,
						collection.guid_prefix,
						loan.loan_number as trans_identifier,
						trans.trans_date,
						permit.permit_id,
						permit.permit_num,
						permit.use_condition,project.project_name, 
						project.project_id,
						trans.is_public_fg
					FROM
						loan_item
					...
					select
						accn.transaction_id,
						'accession' as trans_type,
						accn_status as trans_status,
						accn.received_date as closed_date, -- useful lies....
						collection.guid_prefix,
						accn.accn_number as trans_identifier,
						trans.trans_date,
						permit.permit_id,
						permit.permit_num,
						permit.use_condition,
					...
					from
						...
						inner join collection on trans.collection_id=collection.collection_id
					where
						accn.transaction_id=<cfqueryparam value="#detail.accn_id#" cfsqltype="cf_sql_int">
				</cfquery>
				<div class="one_transaction">
					<cfquery name="thisAccn" dbtype="query">
						select
							transaction_id,
							trans_type,
							trans_status,
						...
						from
							transactions
						where
							trans_type=<cfqueryparam value="accession" cfsqltype="cf_sql_varchar">
						group by
							is_public_fg,
							transaction_id,
							trans_type,
					...
					<div class="transaction_main">
						<cfif thisAccn.recordcount is 1>
							Accession #thisAccn.guid_prefix# #thisAccn.trans_identifier#
							<cfif thisAccn.is_public_fg is 1>
								<a href="/viewAccn.cfm?transaction_id=#thisAccn.transaction_id#" target="_blank">
									<input type="button" class="lnkBtn" value="View">
								</a>
							</cfif>
							<cfif listfind(session.roles,'manage_transactions')>
								<a href="/accn.cfm?action=edit&transaction_id=#thisAccn.transaction_id#" target="_blank">
									<input type="button" class="lnkBtn" value="Edit">
								</a>
							</cfif>
							<cfif listfind(session.roles,'manage_specimens')>
								<input type="button" class="lnkBtn" value="Manage" onclick="loadEditApp('addAccn')">
							</cfif>
						</div>
						<cfif listfindnocase(session.roles,'coldfusion_user')>
							<div class="transaction_status">
								#thisAccn.trans_status#, #thisAccn.closed_date#
							</div>
						</cfif>
						<cfif listfindnocase(session.roles,'coldfusion_user')>
							<cfquery name="thisPermits" dbtype="query">
								select
									permit_id,
									permit_num,
									use_condition
								from
									transactions
								where
									permit_id is not null and
									transaction_id=<cfqueryparam value="#thisAccn.transaction_id#" cfsqltype="cf_sql_int">
								group by
									permit_id,
									permit_num,
									use_condition
							</cfquery>
							<cfif thisPermits.recordcount gt 0>
								<div class="trans_pp_label">
									Permits
								</div>
								<cfloop query="thisPermits">
									<div class="transaction_permit">
										#use_condition# <a class="external" href="/Permit.cfm?permit_id=#permit_id#">#permit_num#</a>
									</div>
								</cfloop>
							</cfif>
						</cfif>
						<cfquery name="thisProj" dbtype="query">
							select 
								project_id,
								project_name 
							from 
						...
						<cfif thisProj.recordcount gt 0>
							<div class="trans_pp_label">
								Projects
							</div>
							<cfloop query="thisProj">
								<div class="transaction_project">
									<a class="external" href="/project/#project_id#">#project_name#</a>
								</div>
							</cfloop>
						</cfif>
						<div id="SpecAccnMedia"></div>
					<cfelse>
						-accession not available to user-
					</cfif>
				</div>
				<cfquery name="thisDLoan" dbtype="query">
					select
						transaction_id,
						trans_type,
						trans_status,
					...
					from
						transactions
					where
						trans_type=<cfqueryparam value="data" cfsqltype="cf_sql_varchar">
					group by
						transaction_id,
						trans_type,
						trans_status,
				...
				<cfloop query="thisDLoan">
					<div class="one_transaction">
						<div class="transaction_main">
							Loan #guid_prefix# #trans_identifier#
							<cfif listfindnocase(session.roles,'manage_transactions')>
								<a href="/Loan.cfm?action=editLoan&transaction_id=#thisDLoan.transaction_id#" target="_blank">
									<input type="button" class="lnkBtn" value="Edit">
								</a>
							</cfif>
						</div>
						<cfif listfindnocase(session.roles,'coldfusion_user')>
							<div class="transaction_status">
								#thisDLoan.trans_type#: #thisDLoan.trans_status#, #thisDLoan.trans_date# <cfif len(thisDLoan.closed_date) gt 0>-#thisDLoan.closed_date#</cfif>
							</div>
						</cfif>
						<cfif listfindnocase(session.roles,'coldfusion_user')>
							<cfquery name="thisPermits" dbtype="query">
								...
								where
									permit_id is not null and
									transaction_id=<cfqueryparam value="#thisDLoan.transaction_id#" cfsqltype="cf_sql_int">
								group by
									permit_id,
									permit_num,
									use_condition
							...
							<cfif thisPermits.recordcount gt 0>
								...
								<cfloop query="thisPermits">
									<div class="transaction_permit">
										#thisPermits.use_condition# <a class="external" href="/Permit.cfm?permit_id=#thisPermits.permit_id#">#thisPermits.permit_num#</a>
									</div>
								</cfloop>
							</cfif>
						</cfif>
						...
						<cfif thisProj.recordcount gt 0>
							...
							<cfloop query="thisProj">
								<div class="transaction_project">
									<a class="external" href="/project/#thisProj.project_id#">#thisProj.project_name#</a>
								</div>
							</cfloop>
						</cfif>
					</div>
				...
				<cfquery name="thisLoan" dbtype="query">
					...
					from
						transactions
					where
						trans_type not in (<cfqueryparam value="data,accession" cfsqltype="cf_sql_varchar" list="true">)
					group by
						transaction_id,
						trans_type,
						trans_status,
				...
				<cfloop query="thisLoan">
					<div class="one_transaction">
						<div class="transaction_main">
							Loan #thisLoan.guid_prefix# #thisLoan.trans_identifier#
							<cfif listfindnocase(session.roles,'manage_transactions')>
								<a href="/Loan.cfm?action=editLoan&transaction_id=#thisLoan.transaction_id#" target="_blank">
									<input type="button" class="lnkBtn" value="Edit">
								</a>
							</cfif>
						</div>
						<cfif listfindnocase(session.roles,'coldfusion_user')>
							<div class="transaction_status">
								#thisLoan.trans_type#, #thisLoan.trans_status#, #thisLoan.trans_date# <cfif len(thisLoan.closed_date) gt 0>-#thisLoan.closed_date#</cfif>
							</div>
						</cfif>
						<cfif listfindnocase(session.roles,'coldfusion_user')>
							<cfquery name="thisPermits" dbtype="query">
								...
								where
									permit_id is not null and
									transaction_id=<cfqueryparam value="#thisLoan.transaction_id#" cfsqltype="cf_sql_int">
								group by
									permit_id,
									permit_num,
									use_condition
...
<div id="dialog" title="Would you like maps with that?">
	Map Contents
	<ul>
		<li>Red Markers are record georeference point</li>
