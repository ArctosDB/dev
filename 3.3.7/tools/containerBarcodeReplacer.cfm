<!-------What this is and how to use it-------------
	...
	create table cf_temp_barcodeswapper (
		key serial not null,
		container_id int not null,
		label varchar not null,
		replacement_barcode varchar not null,
		username varchar not null default session_user,
		last_ts timestamp default current_timestamp,
		status varchar
	);

	grant select, insert, update, delete on cf_temp_barcodeswapper to admin_container;
	grant select, usage on cf_temp_barcodeswapper_key_seq to public;


	once the new app is built, add it to the handler system
	* run_order is a nonunique integer - 1 is a nice number for testing, anyone can change it
	* 10 is the usual initual rec_per_run number; this should complete in ~30 seconds.
...
insert into cf_component_loader (
	...
	'Scary Barcode Swapper',
	99,
	'autoload_barcode_swapper',
	'/tools/containerBarcodeReplacer.cfm',
...
<cfset ComponentLoaderVersion="1.6">
...
--->
<cfset required_roles="admin_container">
<cfoutput>
	...
	<cfset thisFormFile="containerBarcodeReplacer.cfm">
	<cfset thisFormName="Scary Barcode Swapper">
	<cfset thisFormPurpose='This tool allows Arctos operators with <a href="/Admin/user_roles.cfm">role(s)</a> (#required_roles#) to swap barcodes. 
	<p>This form 
	<ul>
	<li>Destroys a `%label` container with a barcode, then</li>
	<li>Adds the barcode from the destroyed container to an existing non-`%label` container</li>
	</ul>
	<p>Any existing barcode from the "keeper" container will be permanently lost.</p>
	<p>
	There is no practical way to un-do what this form does. Do not use this form unless you know what you are doing. Most collections should have no use for this tool.
	</p>'>
	<cfset thisTable="cf_temp_barcodeswapper">
	<cfset thisTemplateName="containerBarcodeReplacer.csv">
	<cfset thisDownloadName="containerBarcodeReplacerData.csv">
	<cfset templateHeader="container_id,label,replacement_barcode">
...
<cfsavecontent variable = "defDocTableGuts">
	<tr>
		...
		<td>yes</td>
		<td>
			integer: container_id of the container which is getting a new barcode. "12345678" from "https://arctos.database.museum/EditContainer.cfm?container_id=12345678" Any existing barcode will be lost. 
		</td>
	</tr>
	<tr>
		<td>label</td>
		<td>yes</td>
		<td>
			label of the container identified by container_id; serves as a check that the correct container is being selected.
		</td>
	</tr>
	<tr>
		<td>replacement_barcode</td>
		<td>yes</td>
		<td>
			Barcode of the donor container. The associated container_type must be some kind of '% label'. The donor or replacement container will be destroyed, and the barcode will be assigned to the container identified by container_id (and accompanying label).
		</td>
	</tr>
	<tr>
		<td>status</td>
...
<cfif action is "nothing">
	<cfoutput>
		...
		<div class="inlinedocs">
			...
			<p>
				 Component Loaders are shared tools designed to minimize impact on the database infrastructure. Some operations may take days or even weeks to complete. Check the <a href="/info/component_loader_status.cfm">Component Loader Status</a> page for more information.
			</p>
			<p>
