...
<cfif action is "validate">
	<cftransaction>
		<cfquery name="agent_id" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			update cf_container_environment set agent_id=getAgentID(checked_by_agent)
		</cfquery>
		<cfquery name="container_id" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
		...
		<!----
		docs say this is ISO, all structures are DATE, just commenting out for now

		<cfquery name="isiso" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			update cf_container_environment set status=is_iso8601(check_date) where is_iso8601(check_date) != 'valid'
		</cfquery>
		---->
		<cfquery name="badagent" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			update cf_container_environment set status='bad agent' where agent_id is null
		</cfquery>
		<cfquery name="badbc" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			update cf_container_environment set status='bad barcode' where container_id is null
		</cfquery>
		<cfquery name="badp" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
		...
		<cfquery name="ins" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			insert into container_environment (
				...
				remark
			 	...
			 	select
			 		nextval('sq_container_environment_id'),
			 		container_id,
			 		check_date::date,
			 		agent_id,
			 		parameter_type,
