...
drop table cf_temp_bulk_identifier_assignedby;


	drop table cf_temp_bulk_identifier_issuedby;


create table cf_temp_bulk_identifier_issuedby (
	key serial not null,
	guid varchar,
	identifier_type varchar,
	identifier_value varchar,
	identifier_references varchar,
	issued_by varchar,
...
grant select, insert, update, delete on cf_temp_bulk_identifier_issuedby to manage_records;
grant select, usage on cf_temp_bulk_identifier_issuedby_key_seq to public;




delete from cf_component_loader where loader_template='/loaders/bulk_identifier_assignedby.cfm';
delete from cf_component_loader where loader_template='autoload_identifier_issuedby';
delete from cf_component_loader where loader_template='autoload_identifier_assignedby';
...
insert into cf_component_loader (
	...
	'Identifier IssuedBy Bulkloader', -- title
	'Bulk associate issued by agents with existing Identifiers', -- short description of the purpose
	1, -- run_order is a nonunique integer; 1 is as good at anything
	'autoload_identifier_issuedby', -- this will resolve to /ScheduledTasks/componentLoaderComponents/{what you enter here}.cfm
	'/loaders/bulk_identifier_issuedby.cfm', -- this should be /loaders/something.cfm - migration in process
	'cf_temp_bulk_identifier_issuedby', -- the table used by the loader
...
-- 202308: tune for MVZ load

update cf_component_loader set rec_per_run=75,remark='75 running in ~50s' where loader_template='autoload_identifier_issuedby';

--202311 mkoo email

update cf_component_loader set purpose='Add issued by agents to existing Identifiers when Issued By is missing.' where loader_template='autoload_identifier_issuedby';
...
<cfoutput>
	...
	<cfset templateHeader="guid,identifier_type,identifier_value,identifier_references,issued_by">
...
<cfsavecontent variable = "defDocTableGuts">
	...
	<tr>
		...
		<td>yes</td>
		<td>
			type of ID to update
		</td>
	</tr>
	<tr>
		<td>identifier_value</td>
		<td>yes</td>
		<td>
			value of ID to update
		</td>
	</tr>
	<tr>
		<td>identifier_references</td>
		<td>yes</td>
		<td>
			id_references of the ID to update
		</td>
	</tr>
	<tr>
		<td>issued_by</td>
		<td>yes</td>
		<td>
			Issuing Agent; existing record must be NULL for this to work.
		</td>
	</tr>
	<tr>
		<td>status</td>
