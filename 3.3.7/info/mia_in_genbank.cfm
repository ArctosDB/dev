...
<div style="border:3px solid green;margin:1em;">
	The following are potential specimen records that are in GenBank but not in Arctos.
	<ul>
		<li>This runs only for collections with a "genbank_collection" entry in Manage Collection</li>
		<li>Sometimes GenBank reports strange numbers (usually 0 or 600); don't trust the count, just click the link</li>
		<li>Data in the table below are far from perfect and require human verification.</li>
	</ul>
	<!----
	<br>
	<br>
	...
	<cfoutput>
		<cfquery name="gb" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
