	...
	<cfloop query="d">
	    ...
	    <cfquery name="ck_agent" datasource="uam_god"  cachedwithin="#createtimespan(0,0,60,0)#">
			select getAgentId('#d.agent_name#') as aid
		</cfquery>
		...
		<cftry>
			<cftransaction>
				...
				<cfset queryAddRow(existingCollectors,{agent_id=naid,collector_role=d.collector_role,coll_order=d.coll_order})>
				<cfif debug is true>
					<cfdump var="#existingCollectors#">
				</cfif>
				<cfset thisCO=1>
				<cfquery name="ordCls" dbtype="query">
					select agent_id,collector_role,coll_order from existingCollectors order by coll_order
				</cfquery>
				<cfif debug is true>
					<cfdump var="#ordCls#">
				</cfif>
