...
<!---- https://github.com/ArctosDB/dev/issues/85 - add container stuff ---->
			...
			<table border>
				...
				<tr>
					...
					<td>
						...
						<select name="use_condition" size="1" id="use_condition">
							<option value=""></option>
							<cfloop query="ctuse_condition">
								<option value = "#ctuse_condition.use_condition#">#ctuse_condition.use_condition#</option>
							</cfloop>
						</select>
						<span data-ctl="use_condition" class="ui-icon ui-icon-arrow-4-diag expandoSelect"></span>
	...
	<cfoutput>
		...
		<cfquery name="getLoanRaw" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#" timeout="55">
			select
				...
				concatTransactionContainers(trans.transaction_id)::varchar as transcontainers,
				...
				<div class="aRow">
					<table width="100%">
						<tr>
							...
							<td>
								<a class="external" href="/Reports/reporter.cfm?transaction_id=#transaction_id#">
									<input type="button" class="lnkBtn" value="Reporter">
								</a>
							</td>
						</tr>
					</table>
					...
					<cfif len(transcontainers) gt 0>
						<div class="transObjectRow">
							<cfif isJSON(transcontainers)>
								<cfset x=deSerializeJSON(transcontainers)>
					        <cfelse>
					            <cfset x="">
					        </cfif>
					        <h4>Containers</h4>
					        <ul>
								<cfloop array="#x#" item="i">
									<li><a class="newWinLocal" href="/findContainer.cfm?container_id=#i.container_id#">#i.containerPath#</a></li>
								</cfloop>
							</ul>
						</div>
					</cfif>
					...
					<cfif len(transpermits) gt 0>
						<div class="transObjectRow">
							...
							<cfloop array="#x#" item="i">
								<div class="itemTitle">Permit Number: <a class="newWinLocal" href="/Permit.cfm?permit_id=#i.permit_id#">#i.permit_num#</a></div>
								...
								<cfif not isNull(i.use_condition)>
									<div class="itemSubTitle">Use Condition: #i.use_condition#</div>
								</cfif>
								<cfif not isNull(i.use_condition_summary)>
									<div class="itemSubTitle">Use Condition Summary: #i.use_condition_summary#</div>
								</cfif>
