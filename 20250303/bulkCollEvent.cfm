...
<p>
	...
	This form is temporarily disabled, please see https://github.com/ArctosDB/arctos/issues/4033
...
<cfif action is "nothing" or action is "findCollEvent">
	<cfquery name="specimenList" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
		 SELECT
		...
		FROM
			flat
			left outer join specimen_event on flat.collection_object_id=specimen_event.collection_object_id
	...
	<div style="padding:1em; text-align:center; margin:1em; width:70%;border:2px solid red;">
		...
		<cfif allowReplace is true>
			...
			<div style=" text-align:left">
				<form name="loc" method="post" action="bulkCollEvent.cfm">
						...
						<cfloop query="thisEvents">
							<table #iif(i MOD 2,DE("class='evenRow'"),DE("class='oddRow'"))# border>
								...
								<cfif len(VERBATIM_COORDINATES) gt 0>
									<tr>
										<td align="right">AsEnteredCoordinates</td>
	...
	<cfoutput>
		<cftransaction>
			<cfloop list="#specimen_event_id#" index="i">
				<cfquery name="ctspecimen_event_type" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
					update
						specimen_event
					set
...
<cfoutput>
	<cftransaction>
		<cfloop list="#collection_object_id#" index="i">
			...
			<cfquery name="upEd" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
