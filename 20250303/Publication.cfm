...
<cfoutput>
	...
	<cfquery name="raw" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
		select
			...
			publication.edits pub_edits,
			publication.created_date pub_created_date,
			publication.created_agent_id pub_created_id,
			getPreferredAgentName(publication.created_agent_id) pub_created,
			publication.edit_date pub_edit_date,
			publication.edit_agent_id pub_edit_id,
			getPreferredAgentName(publication.edit_agent_id) pub_edit,
			publication_agent.publication_agent_id,
			publication_agent.agent_id,
			getPreferredAgentName(publication_agent.agent_id) agent_name,
			publication_agent.author_role
 		from 
 			publication 
 			left outer join publication_agent on publication.publication_id=publication_agent.publication_id
		where 
			publication.publication_id=<cfqueryparam value="#publication_id#" cfsqltype="cf_sql_int">
	</cfquery>	
	...
	<cfquery name="auth" dbtype="query">
		select
		...
		from
			raw
		where
			publication_agent_id is not null
		group by
			publication_agent_id,
			agent_id,
			agent_name,
	...
	<hr>
	<h4>History</h4>
	<cfif len(pub.pub_created) gt 0>
		<div>
			Publication Created: <a href="/agent/#pub.pub_created_id#">#pub.pub_created#</a> #pub.pub_created_date#
		</div>
	</cfif>
	<cfif len(pub.pub_edit) gt 0>
		<div>
			Publication Most Recent Edit: <a href="/agent/#pub.pub_edit_id#">#pub.pub_edit#</a> #pub.pub_edit_date#
		</div>
	</cfif>
	<cfif len(pub.pub_edits) gt 0>
		<cfset pedits=deSerializeJSON(pub.pub_edits)>
		<cfloop from="1" to="#arrayLen(pedits)#" index="i">
			<cfquery name="getAgentName" datasource="uam_god" cachedwithin="#createtimespan(0,0,60,0)#">
				select getPreferredAgentName(<cfqueryparam value="#pedits[i].agent_id#" cfsqltype="cf_sql_int"> ) as aname
			</cfquery>
			<div>
				Publication Edit: <a href="/agent/#pedits[i].agent_id# ">#getAgentName.aname#</a> #pedits[i].date#
			</div>
		</cfloop>
	</cfif>
</cfoutput>
...
<cfif action is "deletePub">
	<cftransaction>
		<cfquery name="dpublication_agent" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			delete from publication_agent where publication_id=<cfqueryparam value="#publication_id#" cfsqltype="cf_sql_int">
		</cfquery>
		<cfquery name="dpublication" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			delete from publication where publication_id=<cfqueryparam value="#publication_id#" cfsqltype="cf_sql_int">
		</cfquery>
	</cftransaction>
	it's gone.
</cfif>
	...
	<cftransaction>
		...
		<cfquery name="pub" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			update publication set
				...
				edit_agent_id=<cfqueryparam value="#session.myAgentId#" cfsqltype="cf_sql_int">,
