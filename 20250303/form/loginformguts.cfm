...
<cfif action is "signOut">
	...
	<p>
		<a href="loginformguts.cfm?action=nothing">Back to Log In</a>
	</p>	
</cfif>
<!------------------------------------------------------------>
	...
	<cfoutput>
		...
		<cfif isdefined("session.roles") and listfindnocase(session.roles,'coldfusion_user')>
			...
			<cfif listfindnocase(session.roles,'manage_agents') or listfindnocase(session.roles,'manage_collection')>
				...
				<cfif pwage lte 0>
					<cfset session.force_password_change=true>
				</cfif>
			</cfif>
			<cfif listfindnocase(session.roles,'manage_collection')>


				<!---- https://github.com/ArctosDB/dev/issues/73 ----->
				<cfquery name="coll_no_license" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#" cachedwithin="#createtimespan(0,0,60,0)#">
					select guid_prefix from collection where internal_license_id is null or external_license_id is null
				</cfquery>
				<cfif coll_no_license.recordcount gt 0>
					<cfset attnNeeded=listappend(attnNeeded,'license')>
				</cfif>
		...
		<cfif len(attnNeeded) gt 0>
			<script>
				$(document).ready(function() {
					$("##click_continue").focus();
				});
			</script>
			<p>
				You have successfully logged in. 
				<input id="click_continue" type="button" value="Click here to continue!" class="lnkBtn"  onclick="reloadParent();"> 
			</p>
			<p>
				<a href="/directory.cfm" target="_top">Directory</a>
			</p>
			...
			<cfif listfind(attnNeeded,'passwordChange')>
				<div class="friendlyNotification">
					Your password expires in less than 7 days. You may change it <a href="/ChangePassword.cfm" target="_top">here</a> or
			...
			<cfif listfind(attnNeeded,'contacts')>
				<div class="importantNotification">
					...
					<ul>
						<cfloop query="cnc">
							<li>
								<a href="/Admin/Collection.cfm?guid_prefix=#guid_prefix#" target="_blank">
									#cnc.guid_prefix#
								</a>
							</li>
						</cfloop>
			...
			<!---- https://github.com/ArctosDB/dev/issues/73 ----->
			<cfif listfind(attnNeeded,'license')>
				<div class="importantNotification">
					<p>
						You have manage_collection access for collection(s) which do not have a license! Please add appropriate licenses to these collections.
					</p>
					<ul>
						<cfloop query="coll_no_license">
							<li>
								<a href="/Admin/Collection.cfm?guid_prefix=#guid_prefix#" target="_blank">
									#coll_no_license.guid_prefix#
								</a>
							</li>
						</cfloop>
					</ul>
