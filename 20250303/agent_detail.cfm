...
<cfoutput>
    ...
    <cfif structkeyexists(agnt,"data_checks")>
		<h3>
			Data Checks
		</h3>
    	<cfset ts=agnt.data_checks.problems>
    	<table border="1">
    		<tr>
    			<th>Consideration</th>
    			<th>Related Agent</th>
    		...
    		<cfloop collection="#ts#" item="key">
    			<tr>
    				<td>#ts[key].subject#</td>
    				<td>
    					<cfif len(ts[key].agent_id) gt 0>
    						<a href="/agent/#ts[key].agent_id#">#ts[key].preferred_agent_name#</a> (#ts[key].agent_type#)
    					</cfif>
    				</td>
    			</tr>
    		</cfloop>
