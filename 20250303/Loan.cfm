...
<cfif action is "editLoan">
	...
	<form name="editloan" id="editloan" action="Loan.cfm" method="post">
		...
		<ul>
			<cfif loanDetails.loan_type is "data">
				...
				<li><a href="search.cfm?data_loan_trans_id=#transaction_id#">[ view involved catalog records ]</a></li>
			<cfelse>
				...
				<cfquery name="hasCanned" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
					...
					and URL like <cfqueryparam value="%search.cfm%" cfsqltype="cf_sql_varchar">
				...
				<cfquery name="hasArchive" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
					select archive_name from archive_name where  creator=<cfqueryparam value="#session.username#" cfsqltype="cf_sql_varchar">
			...
			<li><a href="search.cfm?loan_trans_id=#transaction_id#">[ view involved catalog records ]</a></li>
			</cfif>
		</ul>
	...
	<td valign="top"><!---- right cell ---->
		...
		<cfquery name="projSugg" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			...
			where
				trans_agent.transaction_id=<cfqueryparam value="#transaction_id#" cfsqltype="cf_sql_int"> and
				project.project_id not in  (select project_id from project_trans where transaction_id=<cfqueryparam value="#transaction_id#" cfsqltype="cf_sql_int">)
			group by
				project.project_id,
				project.project_name,
				project.project_description
		...
		<cfquery name="getPermits" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			...
			WHERE
				...
				permit_trans.transaction_id = <cfqueryparam value="#loanDetails.transaction_id#" cfsqltype="cf_sql_int">
	</cfquery>
	...
	<p>
		...
		<p>
			<input type="button" value="Add a permit" class="picBtn" onClick="openOverlay('/picks/PermitPick.cfm?transaction_id=#transaction_id#&callbackfunction=addNewPermitsPicked','Choose Permits');">
		</p>
	</p>
...
<cfif Action is "deleLoan">
	<cftransaction>
		<cfquery name="killLoan" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			delete from loan where transaction_id=<cfqueryparam value="#transaction_id#" cfsqltype="cf_sql_int">
		</cfquery>
...
<cfif Action is "delePermit">
	<cfquery name="killPerm" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
		DELETE FROM permit_trans WHERE transaction_id = <cfqueryparam value="#transaction_id#" cfsqltype="cf_sql_int"> and
...
<cfif action is "unlinkProject">
	<cfoutput>
		<cfquery name="c" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			delete from project_trans where PROJECT_ID=<cfqueryparam value="#project_id#" cfsqltype="cf_sql_int"> and TRANSACTION_ID=<cfqueryparam value="#transaction_id#" cfsqltype="cf_sql_int">
		</cfquery>
		<cflocation url="Loan.cfm?action=editLoan&transaction_id=#transaction_id#" addtoken="false">
	</cfoutput>
</cfif>
