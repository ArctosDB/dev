...
create table cf_temp_media (
		...
		media_uri varchar,
		preview_uri varchar,
		mime_type varchar not null references ctmime_type(mime_type),
		media_type varchar not null references ctmedia_type(media_type),
		media_license varchar references ctmedia_license(display),
		media_relationship_1 varchar references ctmedia_relationship(media_relationship),
		media_related_term_1 varchar,
		media_relationship_2 varchar references ctmedia_relationship(media_relationship),
		media_related_term_2 varchar,
		media_relationship_3 varchar references ctmedia_relationship(media_relationship),
		media_related_term_3 varchar,
		media_relationship_4 varchar references ctmedia_relationship(media_relationship),
		media_related_term_4 varchar,
		media_relationship_5 varchar references ctmedia_relationship(media_relationship),
		media_related_term_5 varchar,
		media_label_1 varchar references ctmedia_label(media_label),
		media_label_value_1 varchar,
		media_label_2 varchar references ctmedia_label(media_label),
		media_label_value_2 varchar,
		media_label_3 varchar references ctmedia_label(media_label),
		media_label_value_3 varchar,
		media_label_4 varchar references ctmedia_label(media_label),
		media_label_value_4 varchar,
		media_label_5 varchar references ctmedia_label(media_label),
		media_label_value_5 varchar,
		media_label_6 varchar references ctmedia_label(media_label),
		media_label_value_6 varchar,
		media_label_7 varchar references ctmedia_label(media_label),
		media_label_value_7 varchar,
		media_label_8 varchar references ctmedia_label(media_label),
		media_label_value_8 varchar,
		media_label_9 varchar references ctmedia_label(media_label),
		media_label_value_9 varchar,
		media_label_10 varchar references ctmedia_label(media_label),
		media_label_value_10 varchar
	);

	create unique index ix_u_cf_temp_media_media_uri on cf_temp_media(media_uri);

	grant select, insert, update, delete on cf_temp_media to manage_records;
	grant select, usage on cf_temp_media_key_seq to public;

--https://github.com/ArctosDB/dev/issues/166
alter table cf_temp_media add media_terms varchar(255);



delete from cf_component_loader where data_table='cf_temp_media';
...
insert into cf_component_loader (
	...
	'Bulkload Media', -- title
	'media metadata loader', -- short description of the purpose
	1, -- run_order is a nonunique integer; 1 is as good at anything
	'autoload_media', -- this will resolve to /ScheduledTasks/componentLoaderComponents/{what you enter here}.cfm
	'/loaders/BulkloadMedia.cfm', -- this should be /loaders/something.cfm - migration in process
	'cf_temp_media', -- the table used by the loader
	10, -- 10 is a nice number; this should NOT tax the server, and must complete in under a minute
	null,
	'manage_media,manage_records', -- list of roles required to set autoload, delete, etc.
	'manage_media', -- list of roles required to insert data
...
<cfoutput>
	...
	<cfset templateHeader="media_uri,preview_uri,mime_type,media_type,media_license,media_terms">
	<cfloop from="1" to="5" index="i">
		<cfset templateHeader=templateHeader & ",media_relationship_#i#,media_related_term_#i#">
	</cfloop>
	<cfloop from="1" to="10" index="i">
		<cfset templateHeader=templateHeader & ",media_label_#i#,media_label_value_#i#">
	</cfloop>
	<cfset templateHeader=templateHeader & ",status">
...
<cfsavecontent variable = "defDocTableGuts">
	<tr>
		...
		<td>yes</td>
		<td>
			URI (generally URL) of the media file
		</td>
	</tr>
	<tr>
		<td>preview_uri</td>
		<td>no</td>
		<td>
			URI (generally URL) of the media preview. See documentation for limitations and requirements. Leave blank for auto-generation.
		</td>
	</tr>
	<tr>
		<td>mime_type</td>
		<td>yes</td>
		<td>
			How machines handle the file; <a href="/info/ctDocumentation.cfm?table=ctmime_type">ctmime_type</a>
		</td>
	</tr>
	<tr>
		<td>media_type</td>
		<td>yes</td>
		<td>
			What humans should expect; <a href="/info/ctDocumentation.cfm?table=ctmedia_type">ctmedia_type</a>
		</td>
	</tr>
	<tr>
		<td>media_license</td>
		<td>yes</td>
		<td>
			Text from the "license" column of the code table. <a href="/info/ctDocumentation.cfm?table=ctmedia_license">ctmedia_license</a>
		</td>
	</tr>
	<tr>
		<td>media_terms</td>
		<td>no</td>
		<td>
			Text from the "license" column of the code table. <a href="/info/ctDocumentation.cfm?table=ctcollection_terms">ctcollection_terms</a>
		</td>
	</tr>
	<tr>
		<td>media_relationship_n</td>
		<td>no</td>
		<td>
			If given, must be paired with media_related_term_n. <a href="/info/ctDocumentation.cfm?table=ctmedia_relationship">ctmedia_relationship</a>
		</td>
	</tr>
	<tr>
		<td>media_related_term_n</td>
		<td>no</td>
		<td>
			If given, must be paired with media_relationship_n. The following are the only handled values.
			<div>Note: % denotes "any string"; for example, a "% agent" relationship may be used for "created by agent" or "shows agent".</div>
			<table border>
				<tr>
					<th>Relationship</th>
				...
				<tr>
					<td>% project</td>
					<td>ID from project URLs, eg 10000356 from http://arctos.database.museum/project/10000356</td>
					<td>10000356</td>
				</tr>
				<tr>
					<td>% cataloged_item</td>
					<td>GUID (or triplet)</td>
					<td>https://arctos.database.museum/guid/UAM:Mamm:12</td>
				</tr>
				<tr>
					<td>% agent</td>
					<td>Any unique name (preferred is always unique).</td>
					<td>Leslie Viereck</td>
				</tr>
				<tr>
					<td>% media</td>
					<td>ID from media URLs, eg 10625360 from https://arctos.database.museum/media/10625360</td>
					<td>10625360</td>
				</tr>
				<tr>
					<td>% collecting_event</td>
					<td>collecting event name</td>
					<td>SPP01-001</td>
				</tr>
				<tr>
					<td>% loan</td>
					<td>loan.transaction_id, "1001718" (from edit loan URL eg, "https://arctos.database.museum/Loan.cfm?action=editLoan&transaction_id=1001718")</td>
					<td>1001718</td>
				</tr>
			</table>
		</td>
	...
	<tr>
		...
		<td>no</td>
		<td>
			If given, must be paired with media_label_value_n. <a href="/info/ctDocumentation.cfm?table=ctmedia_label">ctmedia_label</a>
		</td>
	</tr>
	<tr>
		<td>media_label_value_n</td>
		<td>no</td>
		<td>
			If given, must be paired with media_label_n. Most are free-text.
		</td>
	</tr>
	<tr>
		<td>status</td>
