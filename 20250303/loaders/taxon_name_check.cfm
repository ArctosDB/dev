...
https://github.com/ArctosDB/arctos/issues/3258



drop table cf_temp_taxon_check;

create table cf_temp_taxon_check (
	key serial not null,
	taxon_name varchar(255) not null,
	suggested_taxon_name varchar,
	name_type varchar,
...
grant select, insert, update, delete on cf_temp_taxon_check to manage_taxonomy;
grant select, usage on cf_temp_taxon_check_key_seq to public;


--https://github.com/ArctosDB/arctos/issues/7548 - also clarify what's going on here

delete from cf_component_loader where data_table='cf_temp_taxon_check';
...
insert into cf_component_loader (
	...
	'Taxon Name Checker', -- title
	'Checks if names exist in Arctos. 
	<div><div class="importantNotification">This tool does not check validity, format, or anything other than existence in Arctos.</div></div>
	<a href="https://verifier.globalnames.org/" class="external">https://verifier.globalnames.org/</a> may be a useful verification service.', -- short description of the purpose
	1, -- run_order is a nonunique integer; 1 is as good at anything
	'autoload_taxon_check', -- this will resolve to /ScheduledTasks/componentLoaderComponents/{what you enter here}.cfm
	'/loaders/taxon_name_check.cfm', -- this should be /loaders/something.cfm - migration in process
	'cf_temp_taxon_check', -- the table used by the loader
...
<cfoutput>
	...
	<cfset templateHeader="taxon_name">
</cfoutput>
...
<cfsavecontent variable = "defDocTableGuts">
	<tr>
		...
		<td>yes</td>
		<td>
			name (not identification) to check
		</td>
	</tr>
	<tr>
		<td>suggested_taxon_name</td>
		<td>SPECIAL</td>
		<td>
			return field: Will return Arctos name match or possible match. Note that name type is a creation flag, not usage - eg https://arctos.database.museum/name/Iron could have been created for Linnean, cultural, or mineral usage. quarantine names exist but are not available for use.
		</td>
	</tr>
	<tr>
		<td>name_type</td>
		<td>SPECIAL</td>
		<td>
			return field: Name type is a creation flag and does not prescribe usage - eg https://arctos.database.museum/name/Iron could have been created for Linnean, cultural, or mineral usage. quarantine names exist but are not available for use.
		</td>
	</tr>
</cfsavecontent>
