<!-------
	...
	drop table cf_temp_container_create;

	create table cf_temp_container_create (
		key serial not null,
		username varchar not null default session_user,
		last_ts timestamp default current_timestamp,
		status varchar,
		container_type varchar(30) not null references ctcontainer_type(container_type),
		barcode varchar(50),
		label varchar(255) not null check (checkfreetext(label)),
		description varchar(255) check (checkfreetext(description)),
		container_remarks varchar(4000) check (checkfreetext(container_remarks)),
		width double precision,
		height double precision,
		length double precision,
		dimension_units varchar(30),
		institution_acronym varchar(20) not null,
		number_rows int,
		number_columns int,
		orientation varchar(25) check (orientation in ('horizontal','vertical')),
		positions_hold_container_type varchar references ctcontainer_type(container_type),
		weight numeric,
		weight_units varchar references ctweight_units(weight_units),
		weight_capacity numeric,
		weight_capacity_units varchar references ctweight_units(weight_units)
	);

	grant select, insert, update, delete on cf_temp_container_create to admin_container;
	grant select, usage on cf_temp_container_create_key_seq to public;




delete from cf_component_loader where loader_template='autoload_container_create';
...
insert into cf_component_loader (
	...
	'Create Containers', -- title
	'Create containers. Should be used after reading documentation and staking a claim.', -- short description of the purpose
	1, -- run_order is a nonunique integer; 1 is as good at anything
	'autoload_container_create', -- this will resolve to /ScheduledTasks/componentLoaderComponents/{what you enter here}.cfm
	'/loaders/bulkCreateContainer.cfm', -- this should be /loaders/something.cfm - migration in process
	'cf_temp_container_create', -- the table used by the loader
	1000, -- 10 is a nice number; this should NOT tax the server, and must complete in under a minute
	null,
	'admin_container', -- list of roles required to set autoload, delete, etc.
	'admin_container', -- list of roles required to insert data
	'Username has access to institution' -- description of any in-loader checks
);
...
<cfoutput>
	...
	<cfset templateHeader="container_type,barcode,label,description,container_remarks,length,width,height,dimension_units,institution_acronym,number_rows,number_columns,orientation,positions_hold_container_type,weight,weight_units,weight_capacity,weight_capacity_units">
...
<cfsavecontent variable = "defDocTableGuts">
	...
	<tr>
		...
		<td>yes</td>
		<td>
			<a href="https://handbook.arctosdb.org/documentation/container.html">handbook</a>
			<br>Special Caution: When creating containers without barcodes, note that there is no key on label so it is possible to create any number of functional duplicate containers with this tool.
		</td>
	</tr>
	<tr>
		<td>description</td>
	...
	<tr>
		...
		<td>conditionally</td>
		<td>
			length  of container in dimension_units; decimal; (width,height,length,dimension_units) must be given together or not at all
		</td>
	</tr>
	<tr>
		<td>width</td>
		<td>conditionally</td>
		<td>
			width of container in dimension_units; decimal; (width,height,length,dimension_units) must be given together or not at all	
		</td>
	</tr>
	<tr>
		<td>height</td>
		<td>conditionally</td>
		<td>
			height  of container in dimension_units; decimal; (width,height,length,dimension_units) must be given together or not at all	
		</td>
	</tr>
	<tr>
		...
		<td>conditionally</td>
		<td>
			<a href="/info/ctDocumentation.cfm?table=ctlength_units">ctlength_units</a>
			dimension_units; "0" (no quotes) will update to NULL; blank will be ignored (no updates); (width,height,length,dimension_units) must be given together or not at all	
		</td>
	</tr>
	<tr>
		...
		<td>yes</td>
		<td>
			Must reference collection.institution_acronym
		</td>
	</tr>
	<tr>
		<td>number_rows</td>
		<td>conditionally</td>
		<td>
			integer; position layout; (number_rows,number_columns,orientation,positions_hold_container_type) must be given together or not at all	
		</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td>conditionally</td>
		<td>
			vertical or horizontal; position layout; (number_rows,number_columns,orientation,positions_hold_container_type) must be given together or not at all	
		</td>
	</tr>
	<tr>
		...
		<td>conditionally</td>
		<td>
			<a href="/info/ctDocumentation.cfm?table=CTCONTAINER_TYPE">CTCONTAINER_TYPE</a>; position layout; (number_rows,number_columns,orientation,positions_hold_container_type) must be given together or not at all	
		</td>
	</tr>
	<tr>
		<td>weight</td>
		<td>conditionally</td>
		<td>
			(weight, weight_units) must be given together. Numeric.
		</td>
	</tr>
	<tr>
		<td>weight_units</td>
		<td>conditionally</td>
		<td>
			(weight, weight_units) must be given together. Accepts <a href="/info/ctdocumentation.cfm?table=ctweight_units">ctweight_units</a>.
		</td>
	</tr>
	<tr>
		<td>weight_capacity</td>
		<td>conditionally</td>
		<td>
			(weight_capacity, weight_capacity_units) must be given together. Numeric.
		</td>
	</tr>
	<tr>
		<td>weight_capacity_units</td>
		<td>conditionally</td>
		<td>
			(weight_capacity, weight_capacity_units) must be given together. Accepts <a href="/info/ctdocumentation.cfm?table=ctweight_units">ctweight_units</a>.
		</td>
	</tr>
