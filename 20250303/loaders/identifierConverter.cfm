<!-------
	...
	drop table cf_temp_identifier_converter;

	create table cf_temp_identifier_converter (
		key serial not null,
		...
		identifier varchar,
		new_other_id_type varchar,
		new_other_id_number varchar,
		new_other_id_references varchar,
		issued_by varchar,
		remarks varchar
	);


	grant select, insert, update, delete on cf_temp_identifier_converter to manage_records;
	grant select, usage on cf_temp_identifier_converter_key_seq to public;

	


delete from cf_component_loader where loader_template='autoload_identifier_converter';
...
insert into cf_component_loader (
	...
	'Identifier Converter', -- title
	'Automatically convert old-style type-based identifiers to more useful agent-based identifiers', -- short description of the purpose
	1, -- run_order is a nonunique integer; 1 is as good at anything
	'autoload_identifier_converter', -- this will resolve to /ScheduledTasks/componentLoaderComponents/{what you enter here}.cfm
	'/loaders/identifierConverter.cfm', -- this should be /loaders/something.cfm - migration in process
	'cf_temp_identifier_converter', -- the table used by the loader
	10, -- 10 is a nice number; this should NOT tax the server, and must complete in under a minute
	null,
	'coldfusion_user', -- list of roles required to set autoload, delete, etc.
	'coldfusion_user', -- list of roles required to insert data
	'Open access - this does not create core data.' -- description of any in-loader checks
);
...
<cfoutput>
	...
	<cfset templateHeader="guid,identifier_type,identifier,new_other_id_type,new_other_id_number,new_other_id_references,issued_by,remarks">
</cfoutput>
...
<cfsavecontent variable = "defDocTableGuts">
	<tr>
		...
		<td>
			...
			<ol>
				<li>
					status=autoload wil parse the input data into the modern form. The data may then be downloaded, reviewed, and used in the identifier bulkloader.
				</li>
				<li>
					status=autoload_passthrough will pass the converted data directly to the identifier loader with a status of 'autoload.' Use this only in simple 
					situations, and only after testing with the first option. GenBank numbers are a good candidate for this use case. Note that this option will fail 
					(usualy in the identifier loader) if GUID is not provided. You must watch the identifier loader
					after using this option, and as always you are responsible for the final data.
				</li>
			</ol>
		</td>
	</tr>
	<tr>
		...
		<td>no</td>
		<td>
			"DWC Triplet" Record identifier - not used by this form, passed on to final loader
		</td>
	</tr>
	<tr>
		...
		<td>yes</td>
		<td>
			"legacy type" - values in <a href="/tools/identifier_helper.cfm" class="external">Identifier Helper</a> are recognized. Example: GenBank
		</td>
	</tr>
	<tr>
		<td>identifier</td>
		<td>yes</td>
		<td>
			Input value, "bare number" legacy format. Example: "ON148024"
		</td>
	</tr>
	<tr>
		...
		<td>no</td>
		<td>
			Output for this form, input for identifier loader. Usually 'identifier.'
		</td>
	</tr>
	<tr>
		...
		<td>no</td>
		<td>
			Output for this form, input for identifier loader. Example: "https://www.ncbi.nlm.nih.gov/nuccore/ON148024"
		</td>
	</tr>
	<tr>
		...
		<td>no</td>
		<td>
			Not used by this form, passed on to final loader
		</td>
	</tr>
	<tr>
		<td>issued_by</td>
		<td>no</td>
		<td>
			Output for this form, passed on to final loader. Example: "NCBI Nucleotide"
		</td>
	</tr>
	<tr>
		<td>remarks</td>
