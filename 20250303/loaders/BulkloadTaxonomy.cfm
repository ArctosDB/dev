<!-------What this is and how to use it-------------
	...
	create table cf_temp_taxon_name (
		...
		name_type varchar(255) not null references cttaxon_name_type(taxon_name_type)
	);


	grant select, insert, update, delete on cf_temp_taxon_name to manage_taxonomy;
	grant select, usage on cf_temp_taxon_name_key_seq to public;



--https://github.com/ArctosDB/arctos/issues/7548 - be super clear re what's expected here

delete from cf_component_loader where data_table='cf_temp_taxon_name';
...
insert into cf_component_loader (
	...
	'Taxon Name Loader', -- title
	'This tool creates taxon names. 
		<h2>Extra caution is required to use this tool.</h2>
		<div class="importantNotification">
			<ul>
				<li><b>Do not use this tool unless you are intimately familiar with the appropriate documentation.</b> This tool is dangerous for you and everyone else. Particularly important to understand are the purposes of name type and the distinctions between taxonomy, identification, and classification, including that "traditionally" embedded in taxon names. File an Issue with questions or for assistance.</li>
				<li><b>Do not attempt to use this tool as a verification service.</b> This tool will load pattern-appropriate strings, and make huge messes in the process. Internal and external verification services exist; use them.</li>
				<li><b>Do not use this tool before first checking if names exist.</b> There are tools available, use them or file an Issue for assistance.</li>
				<li><b>Do not use this tool for classifications.</b> This tool creates names.</li>
				<li><b>Do not use this tool before using the taxon name checker.</b> This tool should never return errors!</li>
			</ul>
		</div>', -- short description of the purpose
	1, -- run_order is a nonunique integer; 1 is as good at anything
	'autoload_taxon_name', -- this will resolve to /ScheduledTasks/componentLoaderComponents/{what you enter here}.cfm
	'/loaders/BulkloadTaxonomy.cfm', -- this should be /loaders/something.cfm - migration in process
	'cf_temp_taxon_name', -- the table used by the loader
	10, -- 10 is a nice number; this should NOT tax the server, and must complete in under a minute
	null,
	'manage_taxonomy', -- list of roles required to set autoload, delete, etc.
	'manage_taxonomy', -- list of roles required to insert data
...
<cfoutput>
	...
	<cfset templateHeader="scientific_name,name_type,status">
</cfoutput>
...
<cfsavecontent variable = "defDocTableGuts">
	...
	<tr>
		...
		<td>yes</td>
		<td>
			<a href="/info/ctDocumentation.cfm?table=cttaxon_name_type">cttaxon_name_type</a>
		</td>
	</tr>
	<tr>
		<td>status</td>
