...
<cfoutput>
	<cfquery name="me" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
...
<cfquery name="archive" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
	select
	...
	from
		...
		left outer join doi on archive_name.archive_id=doi.archive_id
	where
		upper(creator)=<cfqueryparam value="#ucase(session.username)#" CFSQLType="cf_sql_varchar">
	group by
		archive_name.archive_id,
		archive_name,
		create_date,
