...
<br>
Many Arctos fields will not accept certain characters, including
...
<p>This form will accept text which may contain prohibited characters, and return the following:
<ol>
	<li>
		Prohibited characters replaced with [X]. This is primarily for visual reference, but may also be used for further processing. For example,
		you could paste this into your favorite word processor (one which does not add prohibited characters, please!), search for [X], and replace
		all found values with "\n" ("newline" in javascript) to format your text for javascript display.
	</li>
	<li>
		Prohibited characters replaced with a user-specified string. Just type something into the "Enter a replacement string here" box. The default is the HTML linebreak,
		"&lt;br&gt;".
	</li>
	<li>
		Prohibited characters replaced with nothing. You may be able to replace your original value with this if it contains control characters that
		do not affect layout.
	</li>
	<li>
		Prohibited characters replaced with a space. You may be able to replace your original value with this if it contains control characters that
		do affect layout.
	</li>
</ol>
</p>
