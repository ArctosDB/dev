...
<cfset title='Agent NameString Formatter'>
...
<p>
	...
	<form name="atts" method="post" enctype="multipart/form-data" action="split_agent_namestring.cfm">
	...
	<cfoutput>
		...
		<cftransaction>
			<cfinvoke component="/component/utilities" method="uploadToTable">
		    	<cfinvokeargument name="tblname" value="ds_temp_agent_namesplit">
			</cfinvoke>
		</cftransaction>
...
<cfquery name="ds_ct_agentabbreviations" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#" cachedwithin="#createtimespan(0,0,60,0)#">
			...
			<cfloop query="d">
				...
				<cfquery name="d" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
					update ds_temp_agent_namesplit set
						FORMATTED_NAME=<cfqueryparam value="#tname#" CFSQLType="cf_sql_varchar">,
						REMARK=<cfqueryparam value="#cmnt#" CFSQLType="cf_sql_varchar">
					where key=#key#
				</cfquery>
			</cfloop>
		</table>
