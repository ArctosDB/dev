...
<cfelseif isdefined("niceProjName")>
	<cfquery name="redir" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
		select project_id from project where niceURL(project_name)=<cfqueryparam value="#niceProjName#" cfsqltype="cf_sql_varchar">
	</cfquery>
...
<script type="text/javascript" language="javascript">
	...
	<cfquery name="a" dbtype="query">
		...
		group by
			...
			leave_date
		order by
			agent_position,preferred_agent_name
	</cfquery>
	...
	<cfloop query="a">
		<div class="proj_agent">
			<a target="_blank" href="/agent/#agent_id#">#preferred_agent_name#</a>: #project_agent_role#
	...
	<cfif len(p.funded_usd) gt 0>
		<!---- https://github.com/ArctosDB/dev/issues/172 ---->
		<cfset f="This project was funded for #DollarFormat(p.funded_usd)#">
		<cfif len(supported_research_value.supported_research_value) gt 0>
			<cfset f=f & ", and has supported projects funded for #DollarFormat(supported_research_value.supported_research_value)#">
		</cfif>
