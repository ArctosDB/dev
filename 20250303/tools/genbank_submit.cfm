...
<cfif action is "edbatch">
	<cfoutput>
		...
		<cfquery name="s" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			...
			from
				genbank_sequence
				left outer join flat on genbank_sequence.COLLECTION_OBJECT_ID=flat.COLLECTION_OBJECT_ID
			where
				genbank_batch_id=#batch_id#
		</cfquery>
		<!----
		<cfif s.recordcount is 0>
			<a href="genbank_submit.cfm?action=preDeleteBatch&batch_id=#batch_id#"><input type="button" class="delBtn" value="Delete Batch"></a>
		<cfelse>
			Found sequences; delete not available.
		</cfif>
		----->
		<h3>Sequences</h3>
		<div class="newRec">
...
<cfif action is "edit_sequence">
	<cfoutput>
		<cfif isdefined("source_material_id") and len(source_material_id) gt 0>
			...
			<cfquery name="gid" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
				select
...
<cfif action is "edit_batch">
	<cfoutput>
		...
		<cfif make_addr is "yes">
			...
			<cfquery name="dAddr" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
				delete from address where ADDRESS_TYPE='formatted JSON' and AGENT_ID=#CONTACT_AGENT_ID#
			</cfquery>
			<cfquery name="addAddr" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
				insert into address (
					ADDRESS_ID,
					AGENT_ID,
					ADDRESS_TYPE,
...
<cfif action is "prepfiles">
	<cfoutput>
		...
		<cfset dir="#application.webDirectory#/download/#d.batch_name#">
...
<cfloop query="s">
	...
	<cfquery name="sd" datasource="uam_god">
		select
			guid,
			COLLECTORS,
			case began_date when ended_date then began_date else began_date  || '/' || ended_date end as cdate,
			country,
...
<p>
	...
	<ul>
		<li><a href="/download/#d.batch_name#/#d.batch_name#.sbt">/download/#d.batch_name#/#d.batch_name#.sbt</a></li>
		<li><a href="/download/#d.batch_name#/#d.batch_name#.fsa">/download/#d.batch_name#/#d.batch_name#.fsa</a></li>
		<li><a href="/download/#d.batch_name#/#d.batch_name#.src">/download/#d.batch_name#/#d.batch_name#.src</a></li>
	</ul>
</p>
...
<cfif action is "pkgfiles">
	...
	<cfoutput>
		<cfexecute
			...
			name="#Application.webDirectory#/bin/linux64.tbl2asn"
			arguments="-t #application.webDirectory#/download/#b.batch_name#/#b.batch_name#.sbt -p #application.webDirectory#/download/#b.batch_name#/ -V vb -a s" />
		<p>
			This should have just executed:

			<code>
				/usr/local/bin/linux.tbl2asn -t #application.webDirectory#/download/#b.batch_name#/#b.batch_name#.sbt -p #application.webDirectory#/download/#b.batch_name#/ -V vb -a s
			</code>
		</p>
		<p>
			Execution Results:
...
<cfif action is "reviewResults">
	...
	<cfoutput>
		...
		<cfdirectory name="d" action="list" directory="#application.webDirectory#/download/#b.batch_name#">
		...
		<cfloop query="d">
<!----
			<cffile action="rename" mode="777" source="#Application.webDirectory#/download/#b.batch_name#/#name#" destination="#Application.webDirectory#/download/#b.batch_name#/#name#.txt">
			<li><a href="/download/#b.batch_name#/#name#" target="blank">/download/#b.batch_name#/#name#</a> (new tab/window)</li>

			---->

			<li><a href="/download.cfm?file=#b.batch_name#/#name#" target="blank">download #name#</a></li>
...
<cfif action is "add_agent">
	<cfoutput>
		<cfif len(first_name) gt 0>
			<cfset fn=first_name>
		<cfelse>
			<cfquery name="gad" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
...
<cfif action is "create_batch">
	<cfquery name="k" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
		select nextval('someRandomSequence') k
	</cfquery>

	<!--- see if we can get address info; fake it if not --->
	<cfquery name="adrs" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
		select * from address where agent_id=#contact_agent_id#
	</cfquery>
	<cfquery name="fj" dbtype="query">
		select address from adrs where address_type='formatted JSON' and end_date is null
	</cfquery>

	<cfquery name="an" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
	...
	<cfelse>
		...
		<cfquery name="e" dbtype="query">
			select address from adrs where address_type='email' and end_date is null
		</cfquery>
	...
	<cfelse>
		<!--- see if we can find it --->
		<cfquery name="e" dbtype="query">
			select address from adrs where address_type like '%phone%' and end_date is null
		</cfquery>
		<cfif e.recordcount gt 0>
	...
	<cfelse>
		<!--- see if we can find it --->
		<cfquery name="e" dbtype="query">
			select address from adrs where address_type = 'fax' and end_date is null
		</cfquery>
		<cfif e.recordcount gt 0>
