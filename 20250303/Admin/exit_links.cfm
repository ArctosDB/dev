...
<cfoutput>
	<cfquery name="ctstatus" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
	...
	<cfif isdefined("form.fieldnames") and len(form.fieldnames)>
		<cfquery name="exit" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			select
				<cfif format is "summary">
					...
					count(distinct(media_id)) numberOfMedia
				<cfelse>
					*
				</cfif>
			 from exit_link
			...
			<table border>
				...
				<cfloop query="exit">
					<tr>
						<td>#EXIT_LINK_ID#</td>
						<td>#FROM_PAGE#</td>
						<td><a target="_blank" href="/media/#media_id#">#media_id#</a></td>
						<td>#STATUS#</td>
						<td>#USERNAME#</td>
						<td>#IPADDRESS#</td>
						<td>#WHEN_DATE#</td>
		...
		<cfelseif format is "csv">
			...
			<cfscript>
				...
				variables.joFileWriter.writeLine(ListQualify("ID,Referrer,Media_ID,Status,Username,IP,Date",'"'));
			</cfscript>
			<cfloop query="exit">
				<cfset oneLine = '"#EXIT_LINK_ID#","#FROM_PAGE#","#media_id#","#STATUS#","#USERNAME#","#IPADDRESS#","#WHEN_DATE#"'>
				<cfscript>
