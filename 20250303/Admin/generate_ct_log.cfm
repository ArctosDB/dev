...
CREATE TRIGGER tr_log_#tabl.table_name#
  ...
  EXECUTE PROCEDURE trigger_fct_tr_log_#tabl.table_name#();
		...
		select sum(c) c from (
			...
			union
			select count(*) c from ctpart_attribute_type where lower(value_code_table)='#tabl.table_name#'
			union
			select count(*) c from ctpart_attribute_type where lower(unit_code_table)='#tabl.table_name#'
			union
