...
update bulkloader set attribute_#i#_type='age' where attribute_#i#_type='age class';
...
<cfif action is "bulkloader_cleanup_junk">

<cfsavecontent variable="sc">

<cfloop from ="1" to="20" index="i">
	<cfloop from ="1" to="4" index="a">
		update bulkloader set part_#i#_attribute_value_#a#=r.new_preservation where part_#i#_attribute_type_#a#='preservation' and part_#i#_attribute_value_#a#=r.part_preservation;
</cfloop>
...
BEGIN
	...
	if rec.part_#i#_name is not null THEN
		...
		if rec.part_#i#_barcode is not null then
			...
			select checkUserContainerAccessByBarcode(rec.part_#i#_barcode,rec.guid_prefix,rec.enteredby) into bool_val;
			...
			SELECT count(*) INTO STRICT numRecs FROM container 
				inner join collection on container.institution_acronym=collection.institution_acronym
				WHERE 
					container.container_type LIKE '%label%' AND 
					container.barcode = rec.part_#i#_barcode and
					...
					if rec.part_#i#_barcode is not null then
						...
						UPDATE container SET
							parent_container_id = (
								...
								inner join collection on container.institution_acronym=collection.institution_acronym
								where container.barcode=rec.part_#i#_barcode and collection.guid_prefix=rec.guid_prefix)
