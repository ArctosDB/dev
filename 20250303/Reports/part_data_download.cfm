	...
	<cfif view is "part_edit_view">
		<cfquery name="raw" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			select
				...
				'https://arctos.database.museum/guid/' || #table_name#.guid || '/PID' || specimen_part.collection_object_id as partID,
	...
	<cfif view is "merge_view">
		...
		<cfquery name="raw" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			select
				...
				'https://arctos.database.museum/guid/' || #table_name#.guid || '/PID' || specimen_part.collection_object_id as partID,
				'https://arctos.database.museum/guid/' || #table_name#.guid || '/PID' || specimen_part.sampled_from_obj_id as parentPartID,
