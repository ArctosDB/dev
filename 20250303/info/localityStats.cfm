...
<cfoutput>
	...
	<table border class="sortable" id="t">
		...
		<cfquery name="d" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#" cachedwithin="#createtimespan(0,0,60,0)#">
			select count(*) c from locality where locality_id in
				(
					...
					union
					select locality_id from media_relations where media_relationship like '% locality'
				)
		</cfquery>
		<tr>
		...
		<cfquery name="d" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#" cachedwithin="#createtimespan(0,0,60,0)#">
			select count(*) c from locality where  s_dec_lat is not null
		</cfquery>
		<tr>
		...
		<cfquery name="d" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#" cachedwithin="#createtimespan(0,0,60,0)#">
			select count(*) c from locality where s_geography is not null
		</cfquery>
		<tr>
