...
<cfoutput>
	...
	<cfif len(collecting_event_id) is 0 and len(sdate) is 0 and len(edate) is 0 and len(who) is 0 and len(locality_id) is 0>
		...
		select
		 ...
		 where
		 	...
		 	<cfif len(collecting_event_id) gt 0>
				and collecting_event_id in ( <cfqueryparam value = "#collecting_event_id#" CFSQLType = "cf_sql_int" list = "yes" separator = ","> )
			</cfif>
			<cfif len(sdate) gt 0>
			...
			<cfif len(locality_id) gt 0>
				and collecting_event_id in (
					select collecting_event_id from collecting_event_archive where locality_id in ( <cfqueryparam value = "#locality_id#" CFSQLType="cf_sql_int" list="true"> )
				)
			</cfif>
	</cfquery>
	...
	<table border>
		...
		<cfloop query="dlocid">
			...
			<tr class="datarow" data-lid="#dlocid.collecting_event_id#">
				...
				<td class="original">
					<a target="_blank" href="/editEvent.cfm?collecting_event_id=#dlocid.collecting_event_id#">
						#orig.collecting_event_id#
					</a>
					<br><a target="_blank" href="/search.cfm?collecting_event_id=#dlocid.collecting_event_id#">specimens</a>
