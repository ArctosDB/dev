	...
	<cfoutput>
		...
		<cftransaction>
			...
			<cfquery name="makeMedia" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
		        ...
		        media_terms_id = <cfqueryparam value="#media_terms_id#" CFSQLType="cf_sql_int" null="#Not Len(Trim(media_terms_id))#">
		...
		<form name="newMedia" method="post" action="media.cfm" enctype="multipart/form-data">
			...
			<select name="media_terms_id" id="media_terms_id">
				...
				<cfloop query="ctcollection_terms">
					<option <cfif media.media_terms_id is ctcollection_terms.collection_terms_id> selected="selected"</cfif> value="#ctcollection_terms.collection_terms_id#">#ctcollection_terms.media_terms#</option>
				</cfloop>
			</select>
