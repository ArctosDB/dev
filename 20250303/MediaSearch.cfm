...
----->
	...
	<cfoutput>
		...
		<form name="newMedia" method="post" action="">
			...
			<label for="media_terms_id">Terms</label>
			<select name="media_terms_id" id="media_terms_id">
				<option value=""></option>
				<cfloop query="ctcollection_terms">
					<option value="#ctcollection_terms.collection_terms_id#">#ctcollection_terms.media_terms#</option>
				</cfloop>
			</select>
	...
	<cfif action is "search">
		<script>
			...
			// get_document_media_pageinfo is linked to this code. Do not change one without changing the other.
				...
				https://github.com/ArctosDB/arctos/issues/6698
				...
				$.each($("div[id^='docInfoDiv_']"), function() {
					...
					if (runcount<=10){
						var ptl='/component/functions.cfc?method=getMediaDocumentInfo&returnformat=plain&urltitle=' + theID + '&page=' + 1;
						var containerid="docInfoDiv_"+theID;
						$("#" + containerid).html('<img src="/images/indicator.gif">');
				 		$.get(ptl, function(data){
							$("#" + containerid).html(data);
						});
						...
						var containerid="docInfoDiv_"+theID;
						$("#" + containerid).html('More information is not available at this time.');
					}
	...
	<cfoutput>
		...
		<cfif (isdefined("media_terms_id") and len(media_terms_id) gt 0)>
			<cfif tabls does not contain " media ">
				<cfset tabls = "#tabls# inner join media on media_flat.media_id = media.media_id ">
			</cfif>
			<cfif media_terms_id is 'NULL'>
				<cfset thisrow={}>
				<cfset thisrow.l="false">
				<cfset thisrow.d="isnull">
				<cfset thisrow.t="media.media_terms_id">
				<cfset thisrow.o="">
				<cfset thisrow.v="">
				<cfset arrayappend(qp,thisrow)>
			<cfelse>
				...
				<cfset thisrow.t="media.media_terms_id">
				<cfset thisrow.o="=">
				<cfset thisrow.v=media_terms_id>
				<cfset arrayappend(qp,thisrow)>
			</cfif>
			<cfset mapurl="#mapurl#&media_terms_id=#media_terms_id#">
		</cfif>
		...
		<cfquery timeout="10" name="raw" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#" cachedwithin="#createtimespan(0,0,60,0)#">
			...
			<cfif (isdefined("taxon_name_id") and len(taxon_name_id) gt 0)>
				...
				from
					media_relations
					inner join identification on media_relations.cataloged_item_id = identification.collection_object_id
		...
		<tr #iif(rownum MOD 2,DE("class='evenRow'"),DE("class='oddRow'"))#>
			<cfif media_type is "multi-page document">
				<cfquery name="qhastags" dbtype="query">
					select media_id from raw where urltitle=<cfqueryparam value="#urltitle#" cfsqltype="cf_sql_varchar"> and hastags>0 group by media_id
				</cfquery>
				<td align="middle">
					...
					<br><span style = "font-size:small;">#terms#</span>
			...
			<cfelse><!--- not MPD --->
				...
				<td align="middle">
					<cfif mime_type is "audio/mpeg3">
						...
						<audio controls>
							...
							<a href="/media/#media_id#?open" target="_blank" class="#addThisClass#" title="#alt_text#">
								<img src="#thumbnail#" alt="#alt_text#" style="max-width:150px;max-height:150px;">
							</a>
						</audio>
				...
				---->
				<td>
					<!---- this can probably be removed for better performance when everything's refreshed and happy ---->
					<cftry>
						<div style="max-height: 20em; overflow:auto;">
							<cfif len(relationships) gt 0>
								<cfset rlns=deSerializeJSON(relationships)>
								<cfloop from="1" to="#arrayLen(rlns)#" index="i">
									<div>
										#rlns[i].rln#: <a class="newWinLocal" href="#rlns[i].lnk#">#rlns[i].dsp#</a>
									</div>
								</cfloop>
							</cfif>
							<cfif len(labels) gt 0>
								<cfset lbls=deSerializeJSON(labels)>
					...
					<cfif isdefined("session.roles") and listcontainsnocase(session.roles,"manage_media")>
				        <a href="/media.cfm?action=edit&media_id=#media_id#"><input type="button" class="lnkBtn" value="edit media"></a>
				        <a href="/TAG.cfm?media_id=#media_id#"><input type="button" class="lnkBtn" value="add or edit TAGs"></a>
				    </cfif>
				</td>
			</tr>
			</cfif>
