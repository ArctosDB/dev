...
<cfif action is "nothing">
	...
	function addLocAttrRow(){
		...
		async function populateLocAttrs(id) {
			...
			// do this so we can call this as a promise, don't return until everything's done
	...
	<cfoutput>
		...
		<table width="100%">
			<tr>
				<td>
					...
					<div id="hlinks">
						<a target="_blank" href="/info/localityArchive.cfm?locality_id=#locality_id#">Edit History</a>
						<a target="_blank" href="/place.cfm?action=detail&locality_id=#locality_id#">Detail Page</a>
						<a target="_blank" href="place.cfm?sch=collecting_event&locality_id=#locDet.locality_id#">Child Events</a>
					</div>
				</td>
				<td>
					<div class="importantNotification">
						...
						<cfquery name="vstat" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
							...
							where
								collecting_event.locality_id=<cfqueryparam value="#locDet.locality_id#" cfsqltype="cf_sql_int">
							group by
								verificationstatus,
								guid_prefix
						</cfquery>
						...
						<form name="x" method="post" action="editLocality.cfm">
							...
							<label for="VerificationStatus" class="helpLink" id="_verification_status">
								Update Verification Status for ALL record-events in this Locality to....
							...
							<br>
							<input type="submit" class="lnkBtn" value="Update Verification Status for all of your record-events in this locality to value in pick above">
						</form>
					</div>
				</td>
			</tr>
		...
		<form name="locality" id="locality" method="post" action="editLocality.cfm">
			<div id="editStuffWrapper">
				<div id="editCellDiv">
					<div id="editLocDiv">
						<p>
							<strong>Locality</strong>
					        <input type="submit" value="Save Locality Edits" class="savBtn">
					        <input type="button" value="Save Locality Edits, push my agent + today's date to record-event" class="savBtn"
						...
						<cfif session.roles contains "manage_geography">
							<a href="/Admin/geography.cfm?geog_auth_rec_id=#locDet.geog_auth_rec_id#">[ Edit Geography]</a>
						</cfif>
						...
						<fieldset id="fs_coordinates">
							...
							<div id="editSpatialButtons">
								<input type="button" value="GeoLocate (georeference)" class="insBtn" onClick="geolocate();">
								<cfif len(locDet.DEC_LONG) gt 0>
									<input type="button" value="GeoLocate (modify)" class="insBtn" onClick="geolocate('adjust');">
								</cfif>
								<input type="button" onclick="convertCoords('dec_lat','dec_long');" value="coordinate converter" class="picBtn">
								<a target="_blank"  href="/bnhmMaps/bnhmPointMapper.cfm?locality_id=#locality_id#">BerkeleyMapper</a>
					...
					<div id="editLocMapDiv">
				    	...
				    	<div style="border:1px dashed red; padding:1em;background-color:lightgray;font-size:small;">
							...
							Last refresh: #locDet.s_lastdate#
							<div>
							...
							<div style="font-size:small;font-style:italic; max-height:6em;overflow:auto;border:2px solid red;">
								...
								<span style="font-style:bold;">
									Data in this box come from various webservices. They are NOT "record data," are derived from entirely automated processes,
								...
								<p>
									Automated georeferencing comes from either higher geography and locality or higher geography alone.
							...
							<br>
							Coordinates:
							<input type="text" id="s_dollar_dec_lat" value="#locDet.s_dec_lat#" size="6">
							<input type="text" id="s_dollar_dec_long" value="#locDet.s_dec_long#" size="6">
							+/-<input type="text" id="s_dollar_error_meters" value="#locDet.s_error_meters#" size="6">m
							...
							<br>Elevation (m):
							<input type="text" id="s_dollar_elev" value="#locDet.s_elevation#" size="6">
							<span style="font-style:italic;">
								<cfif len(locDet.min_elev_in_m) is 0>
									There is no curatorially-supplied elevation.
								<cfelseif locDet.min_elev_in_m gt locDet.s_elevation or locDet.s_elevation gt locDet.max_elev_in_m>
									Automated georeference is outside the curatorially-supplied elevation range.
								<cfelseif  locDet.min_elev_in_m lte locDet.s_elevation and locDet.s_elevation lte locDet.max_elev_in_m>
									Automated georeference is within the curatorially-supplied elevation range.
								</cfif>
							</span>
							<br>Tags:
							<span style="font-weight:bold;">#locDet.s_geography#</span>
				...
				<div id="saveButtons">
					<cfif canEdit.c is 0>
			        	...
			        	<input type="button" value="Save Locality Edits, push my agent + today's date to record-event" class="savBtn" onclick="$('##pushMeToEvent').val('push');submit();">
			...
			</div><!-- editStuffWrapper -->
		</form>
	
		<hr>
		<h4>Cache</h4>
		<p>
			These data are drawn from asserted and derived data, generally though spatial intersections, and are used in various queries. Users do not need to do anything here, but the link below may be used	to refresh if desired.
		</p>
		<cfquery name="place_terms" datasource="uam_god">
			select 
				term_type,
				term_value,
				source,
				last_date,
				case 
					when source='geography' then 1              -- asserted
					when source='locality' then 2               -- asserted
					when source='locality attribute' then 3     -- asserted
					when source='geography metadata' then 4     -- indirectly asserted
					when source='spatial intersection' then 5   --derived from asserted data
					when source='geolocate' then 6              -- derived from derived data
					else 99
				end sort_order
			from
				place_terms
			...
			order by sort_order,source, term_type,term_value
		</cfquery>
		<table border id="ptt" class="sortable">
			<tr>
				<th>Source</th>
				<th>Type</th>
				<th>Term</th>
		...
		<a href="editLocality.cfm?action=requestPlacetermRefresh&locality_id=#locality_id#">request refresh</a>
		<br>NOTE: Refresh can be slow, timeouts are not unexpected, try again, wait, or file an Issue for assistance. Use the pull/refresh button near the map *before* this for a fll refresh.
	</cfoutput>
	<cfinclude template="/includes/_footer.cfm">
</cfif>
<!------------------------------------------------------------------------------------------------------>
<cfif action is "requestPlacetermRefresh">
	<cfoutput>
		<cfquery name="place_terms" datasource="uam_god">
			select update_locality_cache(<cfqueryparam value="#locality_id#" cfsqltype="cf_sql_int">,<cfqueryparam value="true" cfsqltype="cf_sql_boolean">)
		</cfquery>
		spiffy
		<a href="editLocality.cfm?locality_id=#locality_id#">back to edit</a>
	</cfoutput>
</cfif>
<!------------------------------------------------------------------------------------------------------>
...
<cfoutput>
	...
	<cfquery name="isColl" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
		select collecting_event_id from collecting_event where locality_id=<cfqueryparam value="#locality_id#" cfsqltype="cf_sql_int">
	</cfquery>
	...
	<cftransaction>
		<cfquery name="deleLocalityAttrs" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			delete from locality_attributes where locality_id=<cfqueryparam value="#locality_id#" cfsqltype="cf_sql_int">
		</cfquery>
		<cfquery name="deleLocality" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
			delete from locality where locality_id=<cfqueryparam value="#locality_id#" cfsqltype="cf_sql_int">
		</cfquery>
	</cftransaction>
	You deleted it.
	</cfoutput>
	...
	<cfoutput>
		<cftransaction>
			...
			<cfquery name="gaseed" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey,'AES/CBC/PKCS5Padding','hex')#">
				select * from locality_attributes where LOCALITY_ID=<cfqueryparam value="#locality_id#" cfsqltype="cf_sql_int">
			</cfquery>
			<cfloop query="gaseed">
