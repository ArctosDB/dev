	...
	<cfloop query="d">
		...
		<cfif debug is true>
	   	...
	   	<cfif len(d.sensu_publication_id) gt 0>
			<cfif left(trim(d.sensu_publication_id),43) neq 'https://arctos.database.museum/publication/'>
	    		<cfset errs=listappend(errs,'sensu_publication_id invalid')>
			</cfif>
			<cfif not isNumeric(listlast(d.sensu_publication_id,'/'))>
	    		<cfset errs=listappend(errs,'sensu_publication_id invalid')>
			</cfif>
			<cfif len(errs) is 0>
				<!---- this will throw an error if the previous check caught something, don't bother if we're not going to load anyway ---->
		   		<cfquery name="ck_pub" datasource="uam_god"  cachedwithin="#createtimespan(0,0,60,0)#">
		   			select publication_id from publication where publication_id=<cfqueryparam value="#listlast(d.sensu_publication_id,'/')#" CFSQLType="cf_sql_int">
		   		</cfquery>
