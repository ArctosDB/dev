<!---- temporarily disabled for debugging <cfabort> ---->
	...
	<cfloop query="d">
		<cftry>
			...
			<cfif len(d.media_terms) gt 0>
				<cfquery name="getTerm" datasource="uam_god" cachedwithin="#createtimespan(0,0,60,0)#">
					select collection_terms_id from ctcollection_terms where display=<cfqueryparam value="#d.media_terms#" CFSQLType="CF_SQL_VARCHAR">
				</cfquery>
				<cfif len(getTerm.collection_terms_id) gt 0>
					<cfset mtid=getTerm.collection_terms_id>
				<cfelse>
					<cfset errs=listappend(errs,'Invalid terms',"; ")>
				</cfif>
			</cfif>
			<cfif debug>
				<br>d.media_terms: #d.media_terms#
				<br>mtid==#mtid#
			</cfif>
			...
			<cftransaction>
				...
				<cfquery name="makeMedia" datasource="uam_god">
					insert into media (
						...
						<cfqueryparam value="#mtid#" CFSQLType="cf_sql_int" null="#Not Len(Trim(mtid))#">
					)
				</cfquery>
				<cfloop from="1" to="5" index="i">
